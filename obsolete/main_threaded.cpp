...
int main(int argc, char** argv) {
    ...
    /* does not work for the moment
          ...
          //rc = pthread_create(&threads[t], NULL, seapodym_threaded, (void
    *)t); ad_comm::pthread_manager->write_lock_buffer(t);
...
int OptionToCode(char* op, int& sub_option) {
    ...
    char* cmdop[N] = {
        ...
        "--twin-experiment=1"};
    int cmpCode[N] = {0, 1,  2,  3, 4, -3, -2, 0, 1, 2, 3,
                      4, -3, -2, 3, 4, 3,  4,  3, 4, 3, 4};
    for (int i = 0; i < N; i++)
...
void help(char* argv0) {
    ...
    cout << "  -sa[=FLAG]  \t\t\t Make sensitivity analysis. By default[=0] "
            "sensitivity function takes model predictions only.\n";
...
int main(int argc, char** argv) {
        ...
        if ((argc > 3)||(argc < 2)) {
                cerr << "Usage["__FILE__ << ':' << __LINE__ << "]: " << argv[0]
<< " parfile\n"; return EXIT_SUCCESS;
        }
