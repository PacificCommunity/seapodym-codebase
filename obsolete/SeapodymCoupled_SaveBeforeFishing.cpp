...
/// This is the main loop function for the SAVE-BEFORE-FISHING simulation mode.
/// The default function includes the following calls:
/// 1- Initialising population density
/// 2- Reading all forcing data (once in optimization mode, at every time step
/// in simulation mode) 3- Reading fisheries data 4- Reading tagging data if
/// tag_like is activated 5- Age/lifestage loop calling the ADRE solvers and
/// ageing 6- Predicting observed variables (catch, LF and density of tags) 7-
/// Likelihood computation 8- Writing outputs (in simulation mode only) Here to
/// the default function added the second ADRE solver in order to: 1) Solve the
/// ADREs without fishing using the state vector of model with fishing at T-1 2)
/// Save the outputs, which correspond to the model solution without fishing
/// mortality 3) Restore the model-with-fishing state vector and get the ADRE
/// solution for T. Note, to activate this simulation mode, use this file
/// instead of SeapodymCoupled_OnRunCoupled in Makefile or Makefile.clt. If
/// latter, only simulation mode is supported for this run.
...
double SeapodymCoupled::OnRunCoupled(
    dvar_vector x, const bool writeoutputfiles) {
    InitializeAll();
    SUM_CATCH = 0.0;
    ...
    */
    for (; t_count <= nbt_total; t_count++) {
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            // 1.5 Connectivity simulations by setting the density in donor
            // region to zero
            if (connectivity_comp) Set_density_region_age_zero(sp);
            ...
            // SAVE-BEFORE-FISHING code starts here, keep this block commented
            // when NOT USED
            ...
            for (int age = 0; age < agemax_ad; age++) {
                Pop_copy(age).allocate(
                    map.imin1, map.imax1, map.jinf1, map.jsup1);
                Pop_copy(age).initialize();
            }
            // Solve the ADREs twice - 1) without fishing -> save outputs -> 2)
            // with fishing
            for (int icounter = 0; icounter < 2; icounter++) {
                ...
                /// for (int age=0; age<=nb_age_built[sp]; age++){///TO BE
                /// FIXED!!!
                for (int n = 0; n < param->sp_nb_cohort_ad[sp]; n++) {
                    ...
                    // NOTE: currently current averaging doesn't depend on
                    // seasonal migrations
                    ...
                    // 2014: catch at age computation for fisheries without
                    // effort data pop.Ctot_no_effort_sp_age_comp(map, *param,
                    // mat, value(mat.dvarDensity(sp,age)), tcur, sp, age); this
                    // section will work only if seasonality switch is ON and
                    // for <1 maturity at age parameter
                    if (param->migrations_by_maturity_flag &&
                        param->seasonal_migrations[sp]) {
                        cout << "In this version no smooth maturity; enter the "
                                "age at first maturity. Exit now!"
                             << endl;
                        exit(1);

                    }  // end of section with seasonality switch and <1 maturity
                       // at age parameter
                    else {
                        migration_flag = 0;
                        ...
                        if (param->age_compute_habitat[sp][age] !=
                            ...
                            if (!tags_only || tags_age_habitat(age))
                                func.Feeding_Habitat(
                                    *param, mat, map, Habitat, sp, age, jday,
                                    tcur, migration_flag);
                        }
                        double mean_age = mean_age_cohort[sp][age];
                        ...
                        if (!param->food_requirement_in_mortality(sp)) {
                            func.Mortality_Sp(
                                *param, mat, map, Mortality, Habitat, sp,
                                mean_age, age, tcur);  // checked
                        } else {
                            Food_Requirement_Index(
                                IFR, FR_pop, ISR_denom, sp, age, tcur, jday);
                            func.Mortality_Sp(
                                *param, mat, map, Mortality, IFR, sp, mean_age,
                                age, tcur);  // checked
                        }
                        ...
                        if (param->age_compute_habitat[sp][age] !=
                            ...
                            if (!tags_only || tags_age_habitat(age))
                                pop.Precaldia_Caldia(
                                    map, *param, mat, Habitat, Total_pop, sp,
                                    age, tcur, jday);  // checked
                        }
                        if (!param->gcalc()) {
                        ...
                        for (int spop = spop0; spop < nb_tagpops + 1; spop++) {
                            ...
                            // SAVE-BEFORE-FISHING: keep the two lines below
                            // COMMENTED if USED: 	if
                            //(sum(param->mask_fishery_sp) && !tags_only)
                            //		fishing = true;
                            ...
                            if (spop > 0) {
                                if (!tagpop_age_solve(
                                        spop - 1, t_count - 1, age))
                                    continue;
                            }
                            pop.Precalrec_Calrec_adult(
                                map, mat, *param, rw,
                                mat.dvarDensity[spop][age], Mortality, tcur,
                                fishing, age, sp, year, month, jday,
                                step_fishery_count,
                                mortality_off);  // checked 20150210
                        }
                        // SAVE-BEFORE-FISHING: keep the line below COMMENTED if
                        // USED: if (sum(param->mask_fishery_sp) && !tags_only)
                        // fishing = true;
                    }
                    age++;
                }
            ...
            if (writeoutputfiles) {
                ...
                // SAVE-BEFORE-FISHING: in this mode the outputs are prepared
                // above CalcSums();
            }
