...
///Here to the default function added the second ADRE solver in order to:
///1) Solve the ADREs without fishing using the state vector of model with fishing at T-1
///2) Save the outputs, which correspond to the model solution without fishing mortality
///3) Restore the model-with-fishing state vector and get the ADRE solution for T.
///Note, to activate this simulation mode, use this file instead of SeapodymCoupled_OnRunCoupled
///in Makefile or Makefile.clt. If latter, only simulation mode is supported for this run. 
...
//				ISR_denom_comp(ISR_denom, sp, tcur);
			...
			//Solve the ADREs twice - 1) without fishing -> save outputs -> 2) with fishing
			for (int icounter=0; icounter<2; icounter++){
				...
				for (int n=0; n<param->sp_nb_cohort_ad[sp]; n++){
					...
					if (param->migrations_by_maturity_flag && param->seasonal_migrations[sp]){
						...
						//SAVE-BEFORE-FISHING: keep the two lines below COMMENTED if USED:						
						...
						//SAVE-BEFORE-FISHING: keep the line below COMMENTED if USED:						
			...
			if (writeoutputfiles){
				...
				//SAVE-BEFORE-FISHING: in this mode the outputs are prepared above
				//CalcSums();
			}
