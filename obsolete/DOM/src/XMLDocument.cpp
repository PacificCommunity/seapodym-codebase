...
string XMLDocument::get(const string& element, const string& attribute) const {
    ...
    for (DOMNode* node = iterator->nextNode(); value.empty() && node != 0;
...
string XMLDocument::getValue(
    const DOMNode* node, const string& attribute) const {
    string value;
...
int write(const char* parfile) throw(runtime_error) { return 0; }
