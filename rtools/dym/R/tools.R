# A set of functions to manipulate DYM files: do arithmetic operations on multiple files, subset files, aggregate through space (to time series) or through time (to 2D distribution).
	...
	#' @examples 
	#' Example 1. Get the early larvae that are accessible to the sampling gear by multiplying the density by the MLD function as used in SEAPODYM
	#' dir.in  <- paste0(Sys.getenv("SEA_FORCING_HOME"),"/run-jra55np_2x30d-po/")
	#' dym.1 <- list.files(dir.in,pattern="mld",full.names=TRUE)
	#  if (length(dym.1)>1){
	#  	f.rm <- grep("clm",dym.1); 
	#  	dym.1 <- dym.1[-f.rm]
	#  }
	#  dym.2 <- paste0("output/",sp,"_early_larvae.dym")
	#' dym.out <- gsub("early","qmld_early_larvae",dym.2)
	#' mult.dyms(c(dym.1,dim.2),dym.out); # writing the result to DYM file
	#'
	#' @export mult.dyms
