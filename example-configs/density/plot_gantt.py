...
for fname in sorted(glob("log_worker*.txt")):
    ...
    for task in init_start:
        if task in init_end:
            ...
            records.append(dict(task_id=task, step=-1, t_init=t_init, t_step=None, t_send=None))
    ...
    for (task, step), t0 in step_start.items():
        ...
        if (task, step) in send_start and (task, step) in send_end:
            ...
            t_send = None
        records.append(dict(task_id=task, step=step, t_init=None, t_step=t_step, t_send=t_send))
...
def agg_task(group):
    ...
    return pd.Series(dict(num_steps=num_steps, t_init=t_init, t_step=t_step, t_send=t_send))
...
summary.to_csv("gantt_summary.csv", index=False)