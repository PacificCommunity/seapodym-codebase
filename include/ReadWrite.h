...
\details Fishing effort redistributed to the model resolution will be read and used in Calpop class for each i,j to compute fishing mortality rates.
*/
...
class fishing_effort
{
  ...
  public:
	...
	void set_effort(int ii, int jj, double ee){i = ii; j = jj; efrt=ee;}
};

/*!
\brief Class that reads and stores all fishing data
\details Fishing data to be stored in SEAPODYM: i,j indices, lon/lat coordinates (center of the fishing area), effort and catch
*/
...
class fishery_record
{
  ...
  private:
    ...
    double efrt; ///fishing effort
    double ctch;///2015: catch for a single species, can be modified to multispecies if needed
  ...
  public:
	...
	void set_record(double longitude, double latitude, int ii, int jj, double ee, double cc){
		lon = longitude; lat = latitude; i = ii; j = jj; efrt=ee; ctch=cc;}
	//void add_record(double ee, dvector cc){efrt+=ee; ctch+=cc;}
	void change_coord(double longitude, double latitude, int ii, int jj){
		lon = longitude; lat = latitude; i = ii; j = jj;}
};


/*!
\brief IO class
\details Class functions are accessible through all computational classes. All types of input data are read here, any new output writing routines must be placed here as well.
*/
class CReadWrite  
{
public:
...
private:
	...
	dvector flon, flat;	//the grid for degraded fishing data (to be used in likelihood and catch_pred)
...
public:
	...
	//string wtxt_headpar(string varname, const int nb_reg, ivector area_sp_B, string sp_name);//, string& header_part);
...
//	void rbin_input2d(string file_in, const imatrix& carte, DMATRIX& mat2d, int nbi, int nbj, int nbytetoskip) ;
	...
	void SaveSepodymFileTxt (CParam& param, CMatrices& mat, PMap& map,
					dvector sumP, DVECTOR& sumF, DVECTOR& sumFprime, DVECTOR& sumF_area_pred, DVECTOR& sumF_required_by_sp, 
					DVECTOR& mean_omega_sp, int day, int mois2, int yr2, int t_total, 
					int qtr1, int qtr2, int nbi, int nbj);
...
//	void rbin_fishery(CParam& param, PMap& map, dmatrix& effort, float date, const int f);
	...
	void rtxt_fishery_data(CParam& param, const PMap& map, const int nbt, const int jday_spinup);
	void set_effort_rm(CParam& param, PMap& map, const int nbt, const int jday_spinup);
	void degrade_fishery_reso(CParam& param, PMap& map, const int nbt, const int jday_spinup);
	//void frec_rm(CParam& param, const PMap& map, fishery_record *frec, const int f, const float lon, const float lat, const int y, const int m, const float E, const dvector C);
	void set_frec_rm(CParam& param, const PMap& map, const int nbt, const int jday_spinup);
	void set_frec_rm_no_effort_fisheries(CParam& param, const PMap& map, const int nbt, const int jday_spinup);
	void delete_fisheries_rec(void);
	void get_catch(CParam& param, dmatrix& catch_obs, const int f, int y, const int m, const int sp);
	void get_effort(CParam& param, dmatrix& effort, const int f, int y, const int m);
	void get_effort_lonlat(CParam& param, dmatrix& effort, dmatrix& efflon, dmatrix& efflat, const int f, int y, const int m);
	void get_effort_rm(CParam& param, dmatrix& effort, const int f, int y, const int m);
	//void get_effort_rm(CParam& param, dmatrix& effort, const imatrix carte, const int f, int y, const int m);
	void get_fishery_data(CParam& param, D3_ARRAY& effort, D4_ARRAY& catch_obs, int y, const int m);
	void get_fishery_data(CParam& param, D3_ARRAY& effort, D4_ARRAY& catch_obs, D3_ARRAY& efflon, D3_ARRAY& efflat, int y, const int m);
	void get_average_effort(CParam& param, D3_ARRAY& effort, D3_ARRAY& efflon, D3_ARRAY& efflat, const int nby, const int m);
	void get_average_effort_rm(CParam& param, dmatrix& effort, const int f, const int nby, const int m);
	void get_average_selectivity(PMap& map, CParam& param, dvector& swa, const ivector fisheries, const int nbf, const int nbt, const int nb_ages, const int sp, const int step_count);
	//void get_fishery_data_mpa(PMap& map, CParam& param, D3_ARRAY& effort, D4_ARRAY& catch_obs, int y, const int m);
