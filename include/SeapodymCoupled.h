...
class SeapodymCoupled : public SeapodymDocConsole {
   public:
    ...
    double run_coupled(dvar_vector x, const bool writeoutputfiles = false) {
        return OnRunCoupled(x, writeoutputfiles);
    }
    double run_habitat(dvar_vector x, const bool writeoutputfiles = false) {
        return OnRunHabitat(x, writeoutputfiles);
    }
    double run_density(dvar_vector x, const bool writeoutputfiles = false) {
        return OnRunDensity(x, writeoutputfiles);
    }
    double run_flux(dvar_vector x, const bool writeoutputfiles = false) {
        return OnRunFlux(x, writeoutputfiles);
    }
    dvariable reset(dvar_vector x) { return param->reset(x); }

    void write(const char* parfile) { param->write(parfile); }
    void save_statistics(
        ...
        param->save_statistics(
            dirout, x_names, likelihood, g, elapsed_time, status, iter, nvars);
    }
    ...
    double get_total_time_reading() {
        double total_time = elapsed_time_reading + pop.elapsed_time_reading +
   ...
   private:
    double dnum1, t_yrdd;
    double cell_area;
    int day, month, qtr, year, past_month, past_qtr, jday_run,
        jday_spinup;  // date variables
    int nbt_building, nbt_spinup_tuna, nbt_forecast, nbt_total,
        nbt_start_series;
    ...
    void Food_Requirement_Index(
        dvar_matrix& IFR, dvar_matrix FR_pop, dvar_matrix ISR_denom,
        const int sp, const int age, const int t_count, const int jday);
    void IFR_age_comp(
        dvar_matrix& IFR, dmatrix FR_pop, dmatrix ISR_denom, const int age,
        const int sp, const int t);
    ...
    void FluxesComp_polygons(
        dvar_matrix Density, dvar_matrix Habitat, dvar_matrix Mortality,
        const int age, const bool fishing, const int year, const int month,
        const int jday, const int step_fishery_count, const int tcur);

    // autodif function
    // void starvation_penalty(dvar_matrix& mortality, dvar_matrix&
    // total_pop,dvar3_array& nF_ratio,const int sp, const int age);
    ...
    void Total_Pop_comp(
        dvar_matrix& total_pop, const int sp, const int jday,
        const int t_count);
    void Total_Stock_comp(dvariable& total_stock, const int sp);
    void SpawningBiomass_comp(dvar_matrix& total_pop, const int sp);

    /// void Age_class_survivals(dvar_matrix& N_a, const dmatrix N_a_1, const
    /// dmatrix Mort_a, const dmatrix Mort_a_1, const int nt_a, const int
    /// nt_a_1);
    // void Age_class_survivals(dvar_matrix& N_a, const dmatrix N_a_1, const int
    // nt_a, const int nt_a_1);
    /// void Survival(dvar_matrix& N_a, dvar_matrix& N_a_1, dvar_matrix& Mort_a,
    /// dvar_matrix& Mort_a_1, const int a, const int sp, const bool adult);
    void Survival(
    ...
    void Spawning(
        dvar_matrix& J, dvar_matrix& Hs, dvar_matrix& N_a, const int jday,
        const int sp, const int pop_built, const int t_count);
    void spawning_spinup_comp(
        dmatrix& J, dmatrix& Hs, dmatrix& SST, double nb_recruitment,
        double Tmin);
    void spawning_built_comp(
        dmatrix& J, dmatrix& Hs, const dmatrix N, double nb_recruitment,
        double a_adults_spawning);
...
\details It stores and returns the information about release position and fish
age at release.
*/
class tag_release {
   public:
