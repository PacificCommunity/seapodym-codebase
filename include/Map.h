...
enum type_de_bords { TERRE = -1, SANS = 0, G_FERME, D_FERME };

/*!
\brief Class managing spatial domain and grid: the land mask, the indexing and
the boundaries. \details This class reads land mask, EEZ mask (if exist) and
topographic indices. The boundary conditions are defined here as well using the
land mask information. Also, the ragged array indices are computed and stored in
this class.
*/
class PMap {
   public:
   ...
   private:
    void definit_cell_bords(
        const int nti, const int ntj);  // définition des bords des cellules
    void definit_lim_infsup(
        const int nti,
        const int
            ntj);  // définition des limites inférieures
...
\details For a given pair of indices (i,j) structure cote stores the type of
cell's borders, two in x and two in y direction - left-closed (G_FERME),
right-closed (D_FERME) or open (SANS) for ocean cells, and land (TERRE) if the
land is next to the land cell.
*/
class CBord {
   public:
    union {
