...
class CMatrices {
   public:
    ...
    // environmental data
    DMATRIX u;  // courant zonal moyen selon temps passe dans les differentes
                // couches, utilise dans calpop
    DMATRIX v;
    ...
    DMATRIX advection_y;
    DMATRIX
        speed;  // magnitude of species velocity (without passive component);

    DVECTOR lastlat;         // Latitude of cell j
    DVECTOR lat_correction;  // Correction for cell area at latitude j
    //	DVECTOR maxGD_lat;	// Maximal gradient of the daylength at a given
    //latitude
    ...
    D3_ARRAY np1;
    D3_ARRAY sst;
    D3_ARRAY ph1;  // Inna 16/01/2017: adding PH (upper layer only) variable
                   // that will impact juvenile mortality
    D3_ARRAY vld;
    ...
    // D3_ARRAY mort_by_sp;	// mort_by_sp: mortality on forage due to all
    // species described in the model DVECTOR nF_access;	// coeff
    // d'accessibilite (absolue) a chacune des composantes forage [nb_forage]
    // DVECTOR nF_ratio;
    ...
    dmatrix mean_speed;  // weigthed (by cohort distribution) average of cohort
                         // speed
    dmatrix mean_diffusion;  // weigthed (by cohort distribution) average of
                             // cohort diffusion rate
    dmatrix mean_mortality;  // weigthed (by cohort distribution) average of
                             // mortality-at-age
    dmatrix
        mean_temperature;  // weigthed average of cohorts ambient temperature
    D3_ARRAY larvae;       // larvae: biomasse des larves (age0 ->1 mois)
    D3_ARRAY
        juvenile;  // juvenile: biomasse des juveniles (age1 ->age_ autonomous)
    ...
    D3_ARRAY total_pop;  // total_pop: biomasse totale = somme de toutes les
                         // classes d'ages
    D3_ARRAY PEB;  // Population Exploitable Biomass: sum of age classes B x
                   // average selectivity function
    ...
    d4_array Ctot_proportion_fishery;  // proportion of C(f) in total catch,
                                       // defined for all (i,j)
    ...
    D5_ARRAY
        density_before;  // fish density before solving ADRE, used in adjoint
    D5_ARRAY adult_habitat;  // store adult habitat index to be used in adjoint
    D4_ARRAY density_after;  // fish density after solving ADRE, used for
                             // computation of weigthed mean variables
    ...
    D3_ARRAY effort;          // observed fishing effort by fishery and space
                              // [nb_fishery][i][j]
    D3_ARRAY efflon, efflat;  // lon-lat coordinates of observed fishing effort
                              // by fishery and space [nb_fishery][i][j]
    D4_ARRAY catch_obs;       // catch observed by
                              // [nb_species][nb_fishery_by_sp[sp]][i][j]
    D4_ARRAY catch_est;       // catch predicted by
                              // [nb_species][nb_fishery_by_sp[sp]][i][j]
    D4_ARRAY
        C_N_sp_age_fishery;  // somme des captures estimees
                             // [nb_species][nb_age_class[sp]][nb_fishery_by_sp[sp]]
    D4_ARRAY C_tot_no_effort_sp_age;  // total catch at age for fisheries
                                      // without effort data, i.e. flagged with
                                      // '1' in mask_fishery_no_effort
    D4_ARRAY LF_qtr_obs;  // observed length frequencies at age in the quarter,
                          // for given fleet and region
    D4_ARRAY
        C_N_sp_age_fishery_qtr;  // somme par trimestre des captures estimees
                                 // [nb_species][nb_age_class[sp]][nb_fishery_by_sp[sp]]
    D5_ARRAY Sum_C_N_sp_age_fishery_area;  // somme totale par trimestre des
                                           // captures estimees

    void createMatHeader(const CParam& param);
    // void createMatHeader(const CParam& int nlong, int nlat, int nlevel);
    void createMatOcean(
        const PMap& map, int t0, int nbt, int nbi, int nbj, int nb_layer,
        int dt);
    ...
    void createMatForage(
        const PMap& map, int nforage, int t0, int nbt, int nbi, int nbj);
    void createMatHabitat(
        const PMap& map, const int nb_forage, const int nb_species, int t0,
        int nbt, const ivector sp_adult_age0, const ivector sp_nb_age_class,
        const imatrix age_compute_habitat);
    void createMatHabitat_input(
        const PMap& map, const int nb_ages, const int nbt_total);
    void createMatSpecies(
        const PMap& map, int t0, int nbt, int nbi, int nbj, int nb_species,
        const ivector a0_adult, const ivector sp_nb_age_class);
    ...
    void createMatCatch(
        const PMap& map, int nbi, int nbj, int nb_species,
        const IVECTOR& nb_fleet, const ivector a0_adult,
        const IVECTOR& nb_cohorts, const IVECTOR& nb_region);
    void createMatMortality(int nforage, int nbi, int nbj);
    void MeanVarMovement(
        const PMap& map, const dmatrix& Adv_x, const dmatrix& Adv_y,
        const dmatrix& Diff, const double mss, const double sigma_species,
        const double length_age, const double length_age_max, const int dT,
        const int sp, const int age);
    void MeanVarMortality(
        const PMap& map, const dmatrix& M, const double Mp_max,
        const double Ms_max, const double Mp_exp, const double Ms_slope,
        const double mean_age_in_month, const int sp, const int age);
    void MeanVarTemperature(
        const PMap& map, const int sp, const int sp_nb_cohort_lv,
        const int a0_adult, const int t_count);

    double comp_waverage(
        const PMap& map, const dmatrix& var, const int sp, const int age);
    double comp_waverage2(
        const PMap& map, const dmatrix& var1, const dmatrix& var2, const int sp,
        const int age);
   ...
   private:
    ...
    d3_array create3d(
        int n1, const int imin, const int imax, ivector& jinf, ivector& jsup);
...
class dim
{
  ...
  private:
        ...
        unsigned nbi;		// nb cell dans la direction est-west (+ 2) zone
1 unsigned nbj;		// nb cell dans la direction nord-sud (+ 2) zone 1
        ...
        ivector nb_cohorts;	// nb of cohorts by species (should be a
vector!) ivector nbc_lv; ivector nbc_jv; ivector nbc_yn; ivector nbc_ad; ivector
i0_ad;

  public:
        void set_dim(const int ni, const int nj, const int nz, const int nt,
                    const int nbf, const int nbs, const int nbc, const int
nc_lv, const int nc_jv, const int nc_yn, const int nc_ad, const int a0_ad, const
int nb_region, const int nb_fisheries){
                ...
                //cohorts
                nbc_lv = nc_lv; nbc_jv = nc_jv; nbc_yn = nc_yn; nbc_ad = nc_ad;
i0_ad = a0_ad;
...
/*
        ...
        static inline dmatrix create2d(int imin, int imax, ivector jinf, ivector
   jsup)
        {
                dmatrix dmat;
        ...
        static inline d3_array create3d(int n1, const int imin, const int imax,
   ivector& jinf, ivector& jsup)
        {
                d3_array d3arr;
                d3arr.allocate(0,n1-1);
                for (int n=0; n<n1; n++){
