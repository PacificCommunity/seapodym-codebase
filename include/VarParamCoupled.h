...
\details In this class we read the XML parameter file, initialize and reset
variable parameters.
*/
class VarParamCoupled : public CParam {
   ...
   public:
    ...
    //int nbi, int nbj, int nbytetoskip){ 		rw.rbin_input2d(file_in, carte, mat2d,
    //nbi, nbj, nbytetoskip);
    //	}
    //	void rbin_input2d(string file_in, DMATRIX& mat2d, int nbi, int nbj, int
    //nbytetoskip); 	void rbin_mat2d(string file_in, const imatrix& carte,
    //DMATRIX& mat2d, int nlat, int nlong, int nbytetoskip); 	void
    //rbin_mat2d(string file_in, DMATRIX& mat2d, int nlat, int nlong, int
    //nbytetoskip);

    /*
            void allocate_dvmatr(const int imin, const int imax, const ivector
       jinf, const ivector jsup){ dvarsU.allocate(imin, imax, jinf,
       jsup);dvarsU.initialize(); dvarsV.allocate(imin, imax, jinf,
       jsup);dvarsV.initialize();
            }
    */
