   ...
   public:
    ...
    bool set(const string& xpath, const string& value) {
        ...
        xmlXPathObject* xpathObj =
            xmlXPathEvalExpression((xmlChar*)xpathfull.c_str(), context);
        if (xpathObj == 0) {
            return false;
        }
     ...
     */
    bool set(
        const string& xpath, const string& attribute, const string& value) {
        string xpathfull = "/par" + xpath;
        xmlXPathObject* xpathObj =
            xmlXPathEvalExpression((xmlChar*)xpathfull.c_str(), context);
        if (xpathObj == 0) {
        ...
        if (xpathObj->type == XPATH_NODESET) {
            ...
            for (int i = 0; i < max; i++) {
                xmlNode* xmlnode = xmlnodeset->nodeTab[i];
                if (xmlnode != 0) {
                    for (xmlAttr* attributenode = xmlnode->properties;
                         attributenode != 0;
                         attributenode = attributenode->next) {
                        if (attribute.compare(
                                string((char*)attributenode->name)) == 0) {
                            attributenode->children->content =
     ...
     */
    int getInteger(
        const string& xpath, const int idx) /*const throw (runtime_error)*/ {
        int i = 0;
        ...
        if (!value.empty()) {
            ...
            throw runtime_error(
                "Error: XPath=\"" + xpath + "\"  with index=\"" +
                string(itoa(idx)) + "\" does not exist.\n");
        }
        return i;
    }
            ...
            for (int i = 0; i < max; i++) {
                ...
                if (xmlnode != 0) {
                    for (xmlAttr* attributenode = xmlnode->properties;
                        ...
                        if (attribute.compare(
                                string((char*)attributenode->name)) == 0 &&
                            attributenode->children != 0) {
                            value +=
                                string((char*)attributenode->children->content);
                        }
                    }
                }
            }
    ...
    double getDouble(const string& xpath) /*const throw (runtime_error)*/ {
        ...
        if (!s.empty()) {
            ...
            throw runtime_error(
                "Error: XPath=\"" + xpath + "\" does not exist.\n");
        }
        return d;
    }
    ...
    double getDouble(
        const string& xpath,
        const string& attribute) /*const throw (runtime_error)*/ {
        double d = 0;
        ...
        if (!s.empty()) {
            ...
            throw runtime_error(
                "Error: XPath=\"" + xpath + "\"  with attribute=\"" +
     ...
     * @param xpath absolute element xpath
     */
    int write(const char* parfile) { return xmlSaveFile(parfile, doc); }
