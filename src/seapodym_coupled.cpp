...
*/
int seapodym_coupled(
    const char* parfile, int cmp_regime, int FLAG, const bool reset_buffers) {
    time_t time_sec;
    time(&time_sec);
    ...
    if (cmp_regime == 0 || sc.param->flag_coupling) {
        if (sc.param->flag_coupling && compute_gradient != 0) {
            cout << "Warning: COUPLING regime is ON, no parameter estimation "
                    "will be performed!"
                 << endl;
        }
    ...
    if (compute_gradient) {
        ...
        // test: write the optimisation stats on exit with final values of like
        // and parameters when it exists the 'while' loop right after gradcalc
        // returns exit code = 1
        if (fmc.ireturn == -1) {
            time(&time_sec);
            time_t time2 = time_sec;
...
int seapodym_phases(const char* parfile) {
    ...
    if (littxt) {
        cout << endl
             << "Reading sensitivity analysis data from the file: "
             << filename.c_str() << endl;
        // skip one line
        string s;
        getline(littxt, s, '\n');
        ...
        cerr << "The file sa_res_all.txt was not found! Will exit now..."
             << endl;
        exit(1);
    }
    littxt.close();
    ...
    for (int phase_no = 0; phase_no < 4; phase_no++) {
        ...
        g.initialize();
        cout << "Total number of variables in phase " << phase_no
             << " is: " << nvar_phase << '\n'
             << '\n';
        ...
        if (out == -1)
            cerr << "Could not copy the file " << file1 << " to " << file2
                 << endl;
...
void Sensitivity_analysis(const char* parfile, const int sftype) {
    ...
    if (sftype == 0) {
        ...
        cout << "\nstarting computing likelihoods for OAT sensitivity analysis"
             << endl;
        string dirout = get_path(parfile);
        ...
        double fmin = 1e10;  // uncomment to always have non-increasing (<=)
                             // fmin between iterations

        for (int i = 1; i <= nvar; i++) {
            // for (int i=nvar; i>=1; i--){
            // for (int i=1; i<=2; i++){

            // double fmin = 1e10; //uncomment to allow increase in function
            // value in iterations
            ...
            for (int k = 0; k < nxr; k++) {
                ...
                cout << i << "." << k + 1 << " \t" << x_names[i] << " \t"
                     << sc.param->get_parval(i) << " " << like << endl;
            }
        ...
        sc.write(newparfile.c_str());
    } else if (sftype == 3) {  // just a forward run, usually to be used in
                               // ALL-AT-a-TIME sensitivity analysis
