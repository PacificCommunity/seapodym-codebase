...
variables;
2. Running the application in one of three modes
   a) (default) running the model in forward (simulation) and backward (gradient
computation) mode optimizing the likelihood; b) simulation computing either
spawning or feeding habitat functions; c) Hessian calculation;
*/
int seapodym_habitats(
    ...
    //'run_habitat' runs the habitat model in forward (simulation) mode
    ...
    if (compute_gradient) {
        ...
        while (fmc.ireturn >= 0) {
            ...
            if (fmc.ireturn > 0) {
                likelihood = sc.run_habitat((dvar_vector)x);
                gradcalc(nvar, g);
    ...
    sc.run_habitat((dvar_vector)x, true);
