		...
		if (param.mask_fishery_sp[sp][f]){
			if (param.mask_fishery_sp_no_effort[sp][f]){
				...
				for (int age=a0; age<nb_ages; age++){
					...
					save_identifier_string2((char*)"predicted_catch_no_effort_begin");
					...
					save_identifier_string2((char*)"predicted_catch_no_effort_end");

					gradient_structure::GRAD_STACK1->set_gradient_stack(dv_predicted_catch_without_effort_fishery);
				}
			}
			k++;
		}
...
void dv_predicted_catch_without_effort_fishery()
{
	...
	pop->Recomp_C_fishery_proportion_in_Ctot(*map,*param,*rw,Ctot_proportion_fishery,year,month,sp,k);
	...
	for (int i = imax; i >= imin; i--){
		...
		for (int j = jmax; j >= jmin; j--){
			...
			if (map->carte(i,j) && C_proportion_f){
				...
				//double C_f_age_est_cell = value(mat.dvarCtot_age_est(sp,age,i,j)) * 
				//	* mat.Ctot_proportion_fishery(k,i,j) * area / mat.lat_correction(j);
				dfC_age_est(i,j) += dfC_f_age_est_cell * C_proportion_f * area/lat_correction(j);
