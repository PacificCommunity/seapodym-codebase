...
//   char* parfile = "/homelocal/isenina/SEAPODYM/run/YFT/ECCO/LikeComplete/I_tests/threads/initparfile.xml";
...
new_thread_data * tptr = (new_thread_data *) threadid;
	...
	for (int n=0; n<4; n++){
		...
		if (wtxt){
			wtxt << ipar << "\t" << sc.param->parfile_names[ipar]<< "\t" << sc.param->get_parval(ipar) << "\t";
