...
NishikawaCategories::NishikawaCategories(): Hs_cat{0, 1, 5, 10}, Hs_diff{1, 4, 5, 40}, dl(0.1) {}

dvariable NishikawaCategories::categorical_poisson_comp(int H_obs, dvariable N_pred){
    // 1 - Computes the numerical integral of a Poisson distribution function between the two H_obs bounds
    // 2 - applies -log
    ...
    if (H_obs==0.0){
        ...
        // Numerical integral
        int nbl;
        nbl =  (int)(Hs_diff[H_obs-1] / dl);
        for (int il = 1; il <= nbl; il++){
            double l = Hs_cat[H_obs-1] + dl * il - dl/2;
...
void NishikawaCategories::dv_categorical_poisson_comp(){
    ...
    const int H_obs = restore_int_value();
...
void NishikawaCategories::print_fields(){
    ...
    int length = sizeof(Hs_cat) / sizeof(Hs_cat[0]);
