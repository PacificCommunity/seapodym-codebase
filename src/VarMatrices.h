...
public:
	...
	void CreateMatHabitat(PMap& map, const int nb_species, const int nforage, const int nblayer, const int nb_ages, int t0, int nbt, const int nbi, const int nbj, const ivector sp_adult_age0, const ivector sp_nb_age_class, const imatrix age_compute_habitat) {
		CMatrices::createMatHabitat(map,nforage,nb_species,t0,nbt,sp_adult_age0,sp_nb_age_class,age_compute_habitat);

		dvarSeasonSwitch.allocate(0, nb_species);
		dvarSigmaSeason.allocate(0, nb_species);
		for (int sp=0; sp < nb_species; sp++){
			dvarSeasonSwitch(sp).allocate(map.imin,map.imax, map.jmin, map.jmax);
			dvarSigmaSeason(sp).allocate(map.imin,map.imax, map.jmin, map.jmax);
		...
		for (int a=0; a<nb_ages; a++){
			...
			for (int l=0; l<nblayer; l++){
				dvarZ_access(l,a).allocate(map.imin, map.imax, map.jinf, map.jsup);
...
/*
		...
		for (int a = 0; a<age_max; a++){
			dvarDensity_age(a).allocate(map.imin1, map.imax1, map.jinf1, map.jsup1);
			dvarDensity_age(a).initialize();
		}
*/
	...
	void CreateMatCatch(PMap& map, int nbi, int nbj, int nb_species, const IVECTOR& nb_fleet, const ivector a0_adult, const IVECTOR& nb_cohorts, const IVECTOR& nb_region) {
		...
		for (int sp = 0; sp < nb_species; sp++){
			...
			for (int age = a0; age < agemax; age++){
				dvarCtot_age_obs(sp,age).allocate(map.imin, map.imax, map.jinf, map.jsup);
				dvarCtot_age_est(sp,age).allocate(map.imin, map.imax, map.jinf, map.jsup);
		...
		for (int sp = 0; sp < nb_species; sp++) {
			...
			for (int fleet = 0; fleet < fleetmax; fleet++) {
				dvarCatch_est(sp, fleet).allocate(map.imin, map.imax, map.jinf, map.jsup);
