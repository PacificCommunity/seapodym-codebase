...
void VarSimtunaFunc::Seasonal_switch(VarParamCoupled& param, VarMatrices& mat, const PMap& map, int jday, int sp)
{
	...
	Seasonal_switch_comp(param,mat,map,value(season_peak),value(season_start),jday,sp);
...
void dffdaylength(double& dfDL, double& dfJday, double lat, double jday, double pi)
{//adjoint for daylength function(double,double,double)
	...
	for (int j = jmax; j >= jmin; j--){
		...
		for (int i = imax; i >= imin; i--){   
			if (map->carte(i,j)){
				...
				//mat.SeasonSwitch(sp).elem_value(i,j) = tetafunc(dn_ratio-season_start,tau2);
