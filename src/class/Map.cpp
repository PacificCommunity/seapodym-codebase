...
void PMap::lit_map(CParam& param) {
    ...
    if (!litcarte) {
        cout << endl
             << "WARNING : Cannot read file: " << param.str_file_mask.c_str()
             << endl;
    }
    ...
    if (!littopo) {
        cout << endl
             << "WARNING : Cannot read file: " << param.str_file_topo.c_str()
             << endl;
    }
    for (int j = 1; j < ntj - 1; j++) {
        for (int i = 1; i < nti - 1; i++) {
    ...
    if (param.mpa_simulation) {
        ...
        if (!rtxt) {
            cout << endl
                 << "WARNING : Cannot read file: "
                 << param.str_file_maskMPA.c_str() << endl;
        }
        for (int j = 1; j < ntj - 1; j++)
            for (int i = 1; i < nti - 1; i++) {
                rtxt >> maskMPA[i][j];
    ...
    // global = 0;//ATTN: put global to zero to be able to run with summy
    // fisheries. Re-set when needed.
...
void PMap::reg_indices(CParam& param) {
    ...
    for (int a = 0; a < param.nb_region; a++) {
        ...
        // cout << a+1 << "\t" << param.area[a]->lgmin << "\t" <<
        // param.area[a]->lgmax << "\t" << param.area[a]->ltmin << "\t" <<
        // param.area[a]->ltmax << endl; cout << a+1 << "\t" << regimin[a] <<
        // "\t" << regimax[a] << "\t" << regjmin[a] << "\t" << regjmax[a] <<
        // endl;
    }
}
/*
...
*/
void PMap::definit_cell_bords(
    const int nti,
    const int ntj)  // définition des bords des cellules de la zone
{
    ...
    // cout << "i=1 " << (int)bordures.cote.x <<" " <<(int)bordures.cote.y << "
    // "<< bordures.b << endl;
    for (j = 1; j < ntj - 1; j++) {
        bord_cell[i][j] = bordures.b;
        nbl_bord_cell[i][j] = bordures.b;
    ...
    // cout << "i=nbi-2 " <<(int)bordures.cote.x <<" " <<(int)bordures.cote.y <<
    // " " << bordures.b << endl;
    for (j = 1; j < ntj - 1; j++) {
        bord_cell[i][j] = bordures.b;
        nbl_bord_cell[i][j] = bordures.b;
    ...
    // cout << "j=1 " <<(int)bordures.cote.x <<" " <<(int)bordures.cote.y << "
    // "<< bordures.b << endl;
    for (i = 1; i < nti - 1; i++) {
        bord_cell[i][j] = bordures.b;
        nbl_bord_cell[i][j] = bordures.b;
    ...
    // cout << "j=nbj-2 " <<(int)bordures.cote.x <<" " <<(int)bordures.cote.y <<
    // " "<< bordures.b << endl;
    for (i = 1; i < nti - 1; i++) {
        bord_cell[i][j] = bordures.b;
        nbl_bord_cell[i][j] = bordures.b;
    ...
    // cout << "i=1; j=1 " <<(int)bordures.cote.x <<" " <<(int)bordures.cote.y
    // << " "<< bordures.b << endl;
    ...
    // cout << "i=1; j=nbj-2 " <<(int)bordures.cote.x <<" "
    // <<(int)bordures.cote.y << " "<< bordures.b << endl;
    ...
    // cout << "i=nbi-2; j=1 " <<(int)bordures.cote.x <<" "
    // <<(int)bordures.cote.y << " "<< bordures.b << endl;
    ...
    // cout << "i=nbi-2; j=nbj-2 " <<(int)bordures.cote.x <<" "
    // <<(int)bordures.cote.y << " "<< bordures.b << endl; exit(1);
    //************************
    ...
    for (i = 1; i < nti - 1; i++) {
        for (j = 1; j < ntj - 1; j++) {
            if (!carte[i][j]) {  // si la valeur de la carte = 0 (donc terre)
                if (i >
                    1) {  // si cellule n'est pas sur le bord horizontal haut
                ...
                if (i <
                    nti -
                        2) {  // si cellule n'est pas sur le bord horizontal bas
                ...
                if (j >
                    1) {  // si cellule n'est pas sur le bord vertical gauche
                ...
                if (j <
                    ntj -
                        2) {  // si cellule n'est pas sur le bord vertical droit
    ...
    for (i = 1; i < nti - 1; i++) {
        for (j = 1; j < ntj - 1; j++) {
            if (carte[i][j] <
                nbl) {  // if number of layers in the cell < total nbl
                if (i >
                    1) {  // si cellule n'est pas sur le bord horizontal haut
...

{
    ...
    // cout << rowmin << " " << rowmax << " " << colmin << " " << colmax <<
    // endl; cout << imin << " " << imax << " " << imin1 << " " << imax1 <<
    // endl; cout << "jinf " << jinf << endl; cout << "jsup" << jsup << endl;
