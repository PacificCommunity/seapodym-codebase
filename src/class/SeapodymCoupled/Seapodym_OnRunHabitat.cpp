...
/// This is the main loop function for Habitat simulations. It is based on the
/// default OnRunCoupled function, with modifications to compute either spawning
/// of feeding habitats. See SeapodymCoupled_OnRunCoupled.cpp for the
/// description of the main function.

void HabitatConsoleOutput(
    int t_count, int flag_simulation, string date_str,
    double norm_habitat_input, double norm_spawning_habitat, double like);
...
*/
double SeapodymCoupled::OnRunHabitat(
    ...
    if (!param->gcalc()) {
        if (!param->habitat_run_type)
            ...
            cout << endl
                 << "Running Feeding Habitat model for ages: "
                 << param->habitat_run_age << endl
                 << endl;
    ...
    if (param->habitat_run_type > 0)
        fileout = param->strdir_output + param->sp_name[0] +
                  "_feeding_habitat_output.dym";
    else
        fileout = param->strdir_output + param->sp_name[0] +
                  "_spawning_habitat_output.dym";

    // Write DYM headers
    if (writeoutputfiles) {
        WriteFileHeaders_submodel(fileout);
        if (!param->gcalc()) HabitatConsoleOutput(0, 0, date_str, 0, 0, 0);
    }
    /////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////
    ////------------------------------------------------------------/////
    ...
    // Not used currently, but can be useful to constrain eF parameters in
    ...
    for (; t_count <= nbt_total; t_count++) {
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            if (param->habitat_run_type > 0) {
                ...
                func.Faccessibility(
                    *param, mat, map, sp, jday, tcur, pop_built, 0,
            ...
            if (param->habitat_run_type == 0) {
                ...
                func.Spawning_Habitat(
                    *param, mat, map, Habitat, 1.0, sp, tcur, jday);
            }
            ...
            if (param->habitat_run_type > 0) {
                ...
                if (rr_x < 1 || rr_y < 1) {
                    cerr
                        << "Model resolution should be divisible without "
                ...
                for (int ages = 0; ages < param->nb_habitat_run_age; ages++) {
                    ...
                    for (a = 1; a < param->sp_nb_cohorts[sp] - 1; a++) {
                        if (mean_age_cohort(sp, a) >
                            param->habitat_run_age[ages]) {
                            age = mean_age_cohort(sp, a);
                            break;
                        }
                    }
                    if (a == param->sp_nb_cohorts[sp] - 1 && age == 0)
                        age = param->sp_nb_cohorts[sp] - 1;
                    ...
                    for (int i = 1; i < nlon_input; i++) {
                        for (int j = 1; j < nlat_input; j++) {
                            ...
                            if (H_obs > 0) {
                                ...
                                if (ncount) Hmean_pred /= ncount;
                                likelihood +=
                                    (H_obs - Hmean_pred) * (H_obs - Hmean_pred);
                            }
                        }
                    }
                }
        ...
        if (writeoutputfiles) {
            if (!param->gcalc())
                HabitatConsoleOutput(
                    t_count, 1, date_str, norm(mat.habitat_input(0, t_count)),
                    norm(value(Habitat)), value(likelihood));
            ...
            for (int i = map.imin; i <= map.imax; i++) {
                for (int j = map.jinf[i]; j <= map.jsup[i]; j++) {
                    if (map.carte[i][j]) {
                        ...
                        mat2d(i - 1, j - 1) =
                            value(Habitat(i, j));  // if multiple, always the
                                                   // last habitat is written!
                    }
                }
            }
            double minval = 0.0;
...
void SeapodymCoupled::ReadHabitat() {
    ...
    if (param->habitat_run_type == 0)
        file_input = param->strdir_output + param->sp_name[0] +
                     "_spawning_habitat_input.dym";
    else
        file_input = param->strdir_output + param->sp_name[0] +
                     "_feeding_habitat_input_age1.dym";
    ...
    for (; t_count <= nbt_total; t_count++) {
        ...
        if (t_count > nbt_building) {
            ...
            // will need to prepare the input habitat that contains the data
            // only for the selected time period
            int nbytetoskip = (9 + (3 * nlat_input * nlon_input) + nlevel +
                               ((nlat_input * nlon_input) * (t_count - 1))) *
                              4;
            if (param->habitat_run_type == 0) {
                file_input = param->strdir_output + param->sp_name[0] +
                             "_spawning_habitat_input.dym";
                // rw.rbin_input2d(file_input, map,
                // mat.habitat_input[0][t_count], nlon_input+2, nlat_input+2,
                // nbytetoskip);
                ...
                for (int n = 0; n < param->nb_habitat_run_age; n++) {
                    ...
                    ostr << n + 1;
                    file_input = param->strdir_output + param->sp_name[0] +
                                 "_feeding_habitat_input_age" + ostr.str() +
                                 ".dym";
                    // rw.rbin_input2d(file_input, map,
                    // mat.habitat_input[n][t_count], nbi, nbj, nbytetoskip);
                    ifstream litbin(file_input.c_str(), ios::binary | ios::in);
                    if (!litbin) {
                        cerr << "Error[" << __FILE__ << ':' << __LINE__
                             << "]: Unable to read file \"" << file_input
...
void HabitatConsoleOutput(
    int t_count, int flag_simulation, string date_str,
    double norm_habitat_input, double norm_Habitat, double like) {
    if (flag_simulation) {
        cout << setw(4) << left << t_count << "| " << setw(14) << date_str
             << "| " << setw(14) << norm_habitat_input << " | " << setw(14)
             << norm_Habitat << " | " << like << endl;
    } else {
        // CalcSums();
        //  Comment: Larvae biomass is the result of spawning function S(t-1)
        //  and ADRE at time t we have Larvae(t=1)=0 because S(t-1) is computed
        //  after CalcSums()
