...
void SeapodymCoupled::create_tag_recaptures() {
    ...
    deltax = param->dx_tags;
    deltay = param->dy_tags;
    xr_tags = deltax / DX;
    yr_tags = deltay / DY;
    if (xr_tags < deltax / DX) {
        deltax = xr_tags * DX;
        cout << "WARNING: tagging data resolution should be divisible of model "
                "resolution, set dx = "
             << deltax << " degree(s) " << endl;
    }
    if (yr_tags < deltay / DY) {
        deltay = yr_tags * DY;
        cout << "WARNING: tagging data resolution should be divisible of model "
                "resolution, set dy = "
             << deltay << " degree(s) " << endl;
    }
    ...
    if (param->longitudeMin + lon_i * dx_deg != param->lonmin_tags ||
        ...
        cout << "\n Reset tagging data grid corner to " << param->lonmin_tags
             << ", " << param->latmax_tags << endl;
        cout << " so that the larger tagging data cells would include model "
                "cells entirely."
             << endl;
    }
...
void SeapodymCoupled::ReadTaggingData(imatrix& nb_rel, ivector& t_count_rec) {
    ...
    if (param->date_mode < 3) jday_ini = Date::julday(ddini, mmini, yyini);

    int nbtot_tags_files = 0;
    for (int p = 0; p < nb_tagpops; p++) {
        string file_in = param->file_tag_data[p];
        // date of all recaptures in the cohort will be read from the name of
        // the file
        ...
        if (param->date_mode < 3) jdate_rec = Date::julday(drec, mrec, yrec);
        int tt_rec = jdate_rec - jday_ini;  // in days
        if (tt_rec > 0) {
            t_count_rec(p) = (int)(tt_rec / deltaT) + 1;  // in time steps
        } else {
            cerr << "Something wrong with reading the date from file with "
                    "recaptures "
                 << file_in.c_str() << ": " << date_rec << endl;
            exit(1);
        }
        ...
        if (rtxt) {
            int n = 0;
            for (; n < nbtot_tags; n++) {
                rtxt >> id >> tag_no >> yy >> mm >> dd >> lat >> lon >>
                    yy_rec >> mm_rec >> dd_rec >> lat_rec >> lon_rec >>
                    len_rel >> len_rec;
                ...
                if (lon_rec > xlon[0] && lon_rec <= xlon[nx_obs] &&
                    ...
                    if (map.carte(i_mod, j_mod)) rec_added = 1;
                }
                ...
                if (i_mod >= map.imin && i_mod <= map.imax &&
                    j_mod >= map.jmin && j_mod <= map.jmax) {
                    if (map.carte(i_mod, j_mod)) rel_added = 1;
                    // attn: generic way is to search over 8 surrounding cells
                    // if (!map.carte(i_mod,j_mod) && map.carte(i_mod,j_mod-1))
                    // { 	j_mod = j_mod-1; rel_added = 1; } else if
                    // (!map.carte(i_mod,j_mod) && map.carte(i_mod+1,j_mod)) {
                ...
                if (tt < 0) {
                    if (!param->gcalc())
                        cout
                            << "WARNING: tag releases out of model time range: "
                            << p << " " << yy << " " << mm << " " << yy_rec
                            << " " << mm_rec << endl;
                    continue;
                }
                ...
                if (!rec_added) {
                    ...
                    if (!param->gcalc()) {
                        if (lon_rec > xlon[0] && lon_rec <= xlon[nx_obs] &&
                            lat_rec <= ylat[0] && lat_rec > ylat[ny_obs]) {
                            cout << "WARNING: tag recapture is on land: " << p
                                 << " " << id << " " << yy_rec << " " << mm_rec
                                 << " " << lat_rec << " " << lon_rec
                                 << "; mask: " << map.carte(i, j) << endl;
                        } else
                            cout << "WARNING: tag recapture out of "
                                    "observational space: "
                                 << p << " " << id << " " << yy_rec << " "
                                 << mm_rec << " " << lat_rec << " " << lon_rec
                                 << endl;
                    }
                    continue;
                }
                ...
                if (!rel_added) {
                    if (!param->gcalc())
                        cout
                            << "WARNING: tag release out of model domain: " << p
                            << " " << id << " " << yy << " " << mm << " " << lat
                            << " " << lon
                            << "; mask: " << map.carte(i_mod, j_mod) << endl;
                    continue;
                }
                // One more check might be necessary: discrepancy between
                // length(age) at recapture which depends on the implemented
                // growth model
                if (age_mod < a0_adult(0) || age_mod > aN_adult(0)) {
                    if (!param->gcalc())
                        cout << "WARNING: age of tag release out of modelled "
                                "lifespan "
                             << p << " " << id << " " << yy_rec << " " << mm_rec
                             << " " << age_mod << endl;
                    continue;
                ...
                if (days_liberty < 0) {
                    if (!param->gcalc())
                        cout << "WARNING: tag releases time is post time "
                                "recapture: "
                             << p << " " << yy << " " << mm << " " << yy_rec
                             << " " << mm_rec << endl;
                    continue;
                ...
                if (!gauss_kernel) {
                    ...
                    double sigx = sqrt(
                        2 * pow(lon_distance(lon, lon_rec, lat, lat_rec), 2) /
                        (2 * (dtlib + 1)));
                    double sigy =
                        sqrt(2 * pow(lat_rec - lat, 2) / (2 * (dtlib + 1)));
                    ...
                    // finally, do not allow latitudinal error to be larger than
                    // longitudinal
                    if (sigy > sigx) sigy = sigx;
                    sigx /= cos(lat_rec * pi / 180.0);
                    gaussian_kernel(
                        gkernel, Xlon, Ylat, lon_rec, lat_rec, sigx, sigy);
    ...
    if (!param->gcalc()) {
        cout << "Number of tag releases being read in files: "
             << nbtot_tags_files << endl;
        cout << "Number of tag releases being used in the model: "
             << nbtot_tags_used << endl;
        if (gauss_kernel != 0)
            cout << "TOTAL number of tag recaptures after Gaussian kernel "
                    "transform: "
                 << sum(rec_obs) << endl;
    }
    ...
    for (int p = 0; p < nb_tagpops; p++) {
        for (int n = 0; n < nbt_total; n++) delete[] rel_rtxt[p][n];
        delete[] rel_rtxt[p];
    }
    delete[] rel_rtxt;
}

double SeapodymCoupled::lon_distance(
    const double lon_rel, const double lon_rec, const double lat_rel,
    const double lat_rec) {
    // computed the longitudinal distance by grid method integration with 0.1deg
    // latitudinal step returns longitudinal distance in 'degrees' corrected by
    // latitude.

    if (lon_rel == lon_rec) return 0;
    if (lat_rel == lat_rec)
        return (lon_rec - lon_rel) * cos(lat_rel * pi / 180.0);
...
void SeapodymCoupled::get_tag_releases(
    dvar4_array& density, i3_array& tagpop_age_solve, imatrix& nb_rel) {
    for (int p = 0; p < nb_tagpops; p++) {
...
void SeapodymCoupled::delete_tag_releases() {
    for (int p = 0; p < nb_tagpops; p++) {
        for (int n = 0; n < nbt_total; n++) delete[] rel[p][n];
        delete[] rel[p];
    }
    delete[] rel;
}

void SeapodymCoupled::gaussian_kernel(
    dmatrix& gauss_kernel, dvector x, dvector y, double lon, double lat,
    double rx, double ry) {
    ...
    for (int i = imin; i <= imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            double G =
                A * exp((-(x(i - 1) - lon) * (x(i - 1) - lon) / (rx * rx) -
                         (y(j - 1) - lat) * (y(j - 1) - lat) / (ry * ry)) /
                        2.0);
            if (map.carte[i][j]) {
                gauss_kernel[i][j] = G;
            } else
    ...
    //	cout << "for (lon,lat) = ("<< lon <<  "," << lat << ") and (sigx,sigy) =
    //(" << rx<<","<< ry <<"), A = "<< A <<"; at sea: " << gsum << " on land: "
    //<< outside << endl;
