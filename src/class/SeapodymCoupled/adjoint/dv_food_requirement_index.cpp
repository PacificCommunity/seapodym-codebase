...
void SeapodymCoupled::Food_Requirement_Index(
    ...
    gradient_structure::GRAD_STACK1->set_gradient_stack(
        dv_food_requirement_index_comp);
}
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                // F_access_sum(i,j) = F_access_sum_pr(i,j) +
                // mat.dvarF_access(n,a,i,j);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                for (int f = 0; f < n; f++) {
                    F_access_sum(f) =
                        mat->F_access_sum_age(sp, f, t_count, i, j);
                    ISR_denom_pr *= F_access_sum(f);
                }
                F_access_sum(n) = mat->F_access_sum_age(sp, n, t_count, i, j);

                // ISR_denom.elem_value(i,j) = ISR_denom_pr *
                // dvarF_access_sum_age(i,j);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nlayer > 0) {
                ...
                // recompute F_access, F_accessible and F_required, sumF and
                // sumF_access then FR_pop, IFR_pop, IFR_age, IFR_age_pop,
                // ISR_denom, ISR_age and ISR_age_pr
                // 1. F_access
                F_access.initialize();
                ...
                for (int n = 0; n < nb_forage; n++)
                    if (day_layer[n] < nlayer)
                        F_access(n) = l_access[day_layer[n]] * DL +
                ...
                */
                ...
                // 4. Finally scaled IFR, which will be used in Mortality
                // function IFR.elem_value(i,j)
                // = 1.0/(1.0+pow(slope,IFR_age_pop-1.0));
                ...
                // 3. IFR of cohort taking account competition for resource with
                // others
                /*				//IFR_age_pop = IFR_age *
                   pow(IFR_pop,1.0-ISR_age); double expr =
                   pow(IFR_pop,1.0-ISR_age); dfIFR_age  += expr * dfIFR_age_pop;
                                                dfIFR_pop  += IFR_age *
                   (1.0-ISR_age) * pow(IFR_pop,-ISR_age) * dfIFR_age_pop;
                                                dfISR_age  -= IFR_age * expr *
                   log(IFR_pop) * dfIFR_age_pop; dfIFR_age_pop  = 0.0;
                */
                // IFR_age_pop = IFR_age * IFR_pop;
                double expr = IFR_pop;
                ...
                /*
                                                //2. compute IRS
                                                //ISR_age =
                   ISR_age_pr/(ISR_denom(i,j)+1e-4); expr = ISR_denom+1e-4;
                                                dfISR_denom(i,j) -= dfISR_age *
                   ISR_age_pr/(expr*expr); dfISR_age_pr += dfISR_age/expr;
                                                dfISR_age = 0.0;
                */
                ...
                for (int n = nb_forage - 1; n >= 0; n--) {
                    //					double ISR_age_pr_pr
                    //= 1.0; 					for (int nn=0; nn<n; nn++) 						ISR_age_pr_pr *=
                    //F_access[n];

                    // ISR_age_pr *= value(dvarF_access(n,age,i,j));
                    //					dfF_access(n,i,j) +=
                    //ISR_age_pr_pr*dfISR_age_pr;
