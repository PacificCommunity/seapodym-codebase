...
void SeapodymCoupled::Total_Pop_comp(dvar_matrix& total_pop, const int sp, const
int jday, const int t_count)
{
                ...
                for (int i = map.imin; i <= map.imax; i++){
                        ...
                        for (int j = jmin; j <= jmax; j++){
                                if (map.carte(i,j)){
                                        ...
                                        total_pop.elem_value(i,j) = total_pr +
value(mat.dvarDensity(sp,a,i,j));
                                }
                        }
                }
...
void SeapodymCoupled::Total_Pop_comp(
    dvar_matrix& total_pop, const int sp, const int jday, const int t_count) {
    total_pop.initialize();
    for (int a = param->sp_a0_adult[sp] + 1; a < param->sp_nb_cohorts[sp];
         a++) {
        ...
        for (int i = map.imin; i <= map.imax; i++) {
            ...
            for (int j = jmin; j <= jmax; j++) {
                if (map.carte(i, j)) {
                    double access0 =
                        value(mat.dvarZ_access(0, age_adult_habitat, i, j));
                    double total_pr = value(total_pop(i, j));
                    total_pop.elem_value(i, j) =
                        total_pr +
                        access0 * value(mat.dvarDensity(sp, a, i, j));
                }
            }
        }
...
void SeapodymCoupled::SpawningBiomass_comp(
    dvar_matrix& total_pop, const int sp) {
    total_pop.initialize();
        ...
        for (int i = map.imin; i <= map.imax; i++) {
            ...
            for (int j = jmin; j <= jmax; j++) {
                if (map.carte(i, j)) {
                    ...
                    total_pop.elem_value(i, j) =
                        total_pr +
                        maturity_age[a] * value(mat.dvarDensity(sp, a, i, j));
                }
            }
        }
...
void SeapodymCoupled::Total_Stock_comp(dvariable& total_stock, const int sp) {
    ...
    int imax = param->lontoi(lonmax_stock_area) -
               1;  //-1 to match regional computations (see SumQArea)
    ...
    for (int a = param->sp_a0_adult[sp]; a < param->sp_nb_cohorts[sp]; a++) {
        ...
        for (int i = imin; i <= imax; i++) {
            ...
            const int jmax = map.jsup[i];
            for (int j = jmin; j <= jmax; j++) {
                if (map.carte(i, j) && j > jmin_stock_area &&
                    j < jmax_stock_area) {
                    double lat_corrected_area = area / mat.lat_correction[j];

                    // Units of Total Stock = thous.mt
                    Total_Stock += 1e-3 * value(mat.dvarDensity(sp, a, i, j)) *
                                   W_mt * lat_corrected_area / nbt;
                }
            }
        }
        // total_stock = nograd_assign(Total_Stock);//attn: nograd_assign
        // doesn't work!
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j) && j > jmin_stock_area &&
                ...
                // sum_ij += 1e-3*value(mat.dvarDensity(sp,a,i,j)) * W_mt *
                // lat_corrected_area/nbt_total;
                dfNa(i, j) +=
                    1e-3 * dfsum_ij * W_mt * lat_corrected_area(j) / nbt_total;
            }
        }
    }
...
void dv_total_pop_comp()
{
        ...
        unsigned long int pos_map   = restore_long_int_value();
        const dvar_matrix_position total_pop_pos  =
restore_dvar_matrix_position(); const dvar_matrix_position Na_pos         =
restore_dvar_matrix_position(); verify_identifier_string2("total_pop_begin");
        ...
        for (int i = imax; i >= imin; i--){
                ...
                for (int j = jmax; j >= jmin; j--){
                        if (map->carte(i,j)){
                                ...
                                //total_pop(i,j) = total_pr(i,j) +
mat.dvarPop_species(sp,a,i,j); dftotal_pr += dftotal_pop(i,j); dfNa(i,j)  +=
dftotal_pop(i,j); dftotal_pop(i,j) = 0.0;
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nlayer) {
                ...
                for (int l = 0; l < nbl; l++) {
                    if (Tfunc_Gaussian) {
                        l_access = f_accessibility_layer(
                    ...
                    for (int n = 0; n < nbf; n++) {
                        if (day_layer[n] == l)
                            lf_access(l) += l_access * forage(n, i, j) * DL;
                        if (night_layer[n] == l)
                            lf_access(l) +=
                                l_access * forage(n, i, j) * (1 - DL);
                    }
                    sumL += lf_access(l);
                }
                ...
                // total_pop(i,j) = total_pr(i,j) +
                // access0*mat.dvarPop_species(sp,a,i,j);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                // total_pop(i,j) = total_pr(i,j) + maturity_age[a]*
                // mat.dvarPop_species(sp,a,i,j);
                dftotal_pr += dftotal_pop(i, j);
