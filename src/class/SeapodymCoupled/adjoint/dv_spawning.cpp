...
void SeapodymCoupled::Spawning(
    ...
    if (pop_built) {
        ...
        Nmature.save_dvar_matrix_value();  // TODO: recompute using
                                           // density_after
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                // J(i,j) = nbr * Hs(i,j) * 1/(1+exp(teta*(SST(i,j)- mu +
                // 2*sigma)));
                dfNbr(i, j) += (Hs(i, j) / expr1) * dfJ(i, j);
                dfHs(i, j) += (nbr / expr1) * dfJ(i, j);
                dfSigma(i, j) -=
                    ((nbr * Hs(i, j) * 2.0 * teta * expr0) / expr2) * dfJ(i, j);
                dfMu(i, j) +=
                    ((nbr * Hs(i, j) * teta * expr0) / expr2) * dfJ(i, j);
                dfJ(i, j) = 0.0;
            }
        }
    }
