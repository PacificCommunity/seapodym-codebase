...
/// Likelihood computation 8- Writing outputs (in simulation mode only)
...
double SeapodymCoupled::OnRunCoupled(
    ...
    */
    for (; t_count <= nbt_total; t_count++) {
        ...
        //		LIKELIHOOD SECTION			//
        //------------------------------------------------------//
        // I. Total abundance likelihood: augmented only if param->stock_like =
        // true
    ...
    if (writeoutputfiles) {
        SaveDistributions(year, month);
        cout << "total catch in simulation (optimization): " << SUM_CATCH
             << endl;
    }
    // if (param->tag_like[0]){
    //	delete_tag_releases();
