...
/// in simulation mode) 3- Age/lifestage loop calling the ADRE solvers and
/// ageing 4- Computing model density -> used as predictions 5- Likelihood
/// computation using input density as observations 6- Writing outputs (in
/// simulation mode only) Note, there is no modelling of tagged cohorts here!
    ...
    */
    for (; t_count <= nbt_total; t_count++) {
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            func.Faccessibility(
                *param, mat, map, sp, jday, tcur, pop_built, false,
            ...
            for (int n = 0; n < param->sp_nb_cohort_lv[sp]; n++) {
                ...
                func.Mortality_Sp(
                    *param, mat, map, Mortality, Spawning_Habitat, sp, mean_age,
                    age, tcur);
                   ...
                   // age parameter
                    ...
                    if (param->age_compute_habitat[sp][age] !=
                        ...
                        pop.Precaldia_Caldia(
                            map, *param, mat, Habitat, Total_pop, sp, age, tcur,
                            jday);  // checked
                    }
                    if (!param->gcalc()) {
                        ...
                        mat.MeanVarMovement(
                            map, mat.advection_x, mat.advection_y,
                    ...
                    pop.Precalrec_Calrec_adult(
                        map, mat, *param, rw, mat.dvarDensity[sp][age],
                        Mortality, tcur, fishing, age, sp, year, month, jday,
                        step_fishery_count, 0);  // checked 20150210
                }
                age++;
            }
            ...
            for (int age = a0_adult[sp]; age < aN_adult[sp]; age++) {
                Density_pred +=
                    mat.dvarDensity[sp][age] * param->weight[sp][age] * 0.001;
            }
            ...
            if (nt_dtau == dtau) {
                for (int a = param->sp_nb_cohorts[sp] - 1; a >= 1; a--) {
                    Survival(
                        mat.dvarDensity[sp][a], mat.dvarDensity[sp][a - 1], a,
                        sp);
                }
                nt_dtau = 0;
            }
            // 7. Spawning
            Spawning(
                mat.dvarDensity[sp][0], Spawning_Habitat, Total_pop, jday, sp,
                pop_built, tcur);  // checked
        }                          // end of 'sp' loop
        ...
        // true Can be useful to constrain the total abundance if the spatial
        // distribution fitting alone cannot provide the same abundance as in
        // density_input.
        if (t_count == nbt_total)
            stocklike += get_stock_like(total_stock, likelihood);

        // II. Biomass density likelihood. Note, degrade it to the resolution of
        // the density_input
        if (t_count > nbt_building + nbstoskip) {
            ...
            TTRACE(rr_x, rr_y)
            if (rr_x < 1 || rr_y < 1) {
                cerr << "Model resolution should be divisible without "
                        "remainder by the resolution of input density field."
                     << endl
                     << "Currenly nlon/nlon_input = " << rr_x
                     << ", and nlat/nlat_input = " << rr_y << endl
                     << "Will exit now...";
                exit(1);
            }
            int i0 = 0;
            int iN = nlon_input;  // put 40-160 for 135E-110W in PO-2deg
                                  // configuration
            int j0 = 0;
            int jN =
                nlat_input;  // put 40-90 for 25S-25N in PO-2deg configuration
            for (int sp = 0; sp < nb_species; sp++) {
                for (int i = i0; i < iN; i++) {
                    for (int j = j0; j < jN; j++) {
    ...
    cout << "end of forward run, likelihood: " << value(likelihood) - eFlike
         << " " << eFlike << endl;
...
void SeapodymCoupled::ReadDensity() {
    ...
    string file_input;
    file_input =
        param->strdir_output + param->sp_name[0] + "_density_input.dym";
    ...
    for (; t_count <= nbt_total; t_count++) {
        ...
        if (t_count > nbt_building) {
            ...
            //----------------------------------------------//
            int nbytetoskip = (9 + (3 * nlat_input * nlon_input) + nlevel +
                               ((nlat_input * nlon_input) * (t_series - 1))) *
                              4;

            // rw.rbin_input2d(file_input, map, mat.density_input[t_count],
            // nlon_input+2, nlat_input+2, nbytetoskip);
            ifstream litbin(file_input.c_str(), ios::binary | ios::in);
            if (!litbin) {
                cerr << "Error[" << __FILE__ << ':' << __LINE__
                     << "]: Unable to read file \"" << file_input << "\"\n";
                exit(1);
            }
