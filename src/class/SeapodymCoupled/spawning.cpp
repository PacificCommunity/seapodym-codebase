...
void SeapodymCoupled::spawning_spinup_comp(
    dmatrix& J, dmatrix& Hs, dmatrix& SST, double nb_recruitment, double Tmin) {
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                ...
                J(i, j) =
                    nb_recruitment * Hs(i, j) * tetafunc(2.0, SST(i, j) - Tmin);
            }
        }
    }
}
...
void SeapodymCoupled::spawning_built_comp(
    dmatrix& J, dmatrix& Hs, const dmatrix Nmature, double R, double b) {
    //////////////////////////////////////////////////////////
