...
void SeapodymCoupled::OnRunFirstStep() {
    sumFprime.allocate(0, nb_forage - 1);
    sumFprime.initialize();
    sumF.allocate(0, nb_forage - 1);
    sumF.initialize();
    sumF_area_pred.allocate(0, nb_forage - 1);
    sumF_area_pred.initialize();
    sumF_required_by_sp.allocate(0, nb_forage - 1);
    sumF_required_by_sp.initialize();
    mean_omega_sp.allocate(0, nb_forage - 1);
    mean_omega_sp.initialize();

    int Tr_step = 0;
    Date::init_time_variables(
        *param, Tr_step, nbt_spinup_tuna, jday_run, jday_spinup, nbt_total, 0,
        0);
    ...
    mat.CreateMatSpecies(
        map, t0, nbt, nbi, nbj, nb_pops, a0_adult, param->sp_nb_cohorts);
    mat.CreateMatHabitat(
        map, nb_species, nb_forage, nb_layer, max(param->sp_nb_cohorts), t0,
        nbt, nbi, nbj, a0_adult, param->sp_nb_cohorts,
        param->age_compute_habitat);
    past_month = 0;
    past_qtr = 0;
    sumP = 0;
    ...
    if (!param->flag_no_fishing) {
        ...
        // put all data on the model resolution in case of MPA simulations or
        // EEZ extractions
        ...
        if (param->file_frq_data[0] != "") {
            for (int sp = 0; sp < nb_species; sp++)
                rw.read_frq_data(
                    *param, map, param->save_first_yr, param->save_last_yr, sp);
        }
    } else {
    ...
    // cout << "Just a WARNING: the sum of eF parameters = " << eF_sum << " will
    // be used in the optimization" << endl;
