...
int SeapodymCoupled::EditRunCoupled(const char* parfile) {
    if (parfile == 0) {
        cerr << "Error[" __FILE__ << ':' << __LINE__
             << "]: parameter parfile is NULL in \"int "
                "SeapodymCoupled::EditRunCoupled(const char* parfile)\"\n";
        return 0;
    }
    runtype = 'C';
    ...
    // Notice, here param variables will be rewritten, it's better to add the
    // test to check their identity rw.rbin_headpar(param->strfile_pp,
    // param->nlong, param->nlat, param->nlevel);
    rw.rbin_headpar(
        param->strfile_u[0], param->nlong, param->nlat, param->nlevel);
    if (nbi - 2 != param->nlong || nbj - 2 != param->nlat) {
        cerr << "INPUT files: dimensions do not match: " << nbi - 2
             << " != " << param->nlong << "; " << nbj - 2
             << " != " << param->nlat << ". Please correct. Will Stop now!"
             << endl;
        exit(1);
    }
    ...
    double mval;
    rw.rbin_header(
        param->strfile_pp, param->idformat, param->idfunc, mval, mval, nlon,
        nlat, param->nlevel, param->startdate, param->enddate, mat.xlon,
        mat.ylat, mat.zlevel, mat.mask);
    ...
    if (nb_fishery && !param->flag_no_fishing) {
        ...
        mat.CreateMatCatch(
            map, nbi, nbj, nb_species, param->nb_fishery_by_sp, a0_adult,
            aN_adult, param->nb_region_sp_B);
    }
