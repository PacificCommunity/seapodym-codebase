...
void SeapodymCoupled::WriteOutput(int t, bool fishing) {
    ...
    if (t_count > nbt_building) {
        /*to rewrite as the old code with binary fishing data files is now
        desactivated for (int sp=0; sp< nb_species;sp++){ if (param->wbin_flag){
                        // Write files with predicted fisheries data
                        rw.write_fishery_data(*param, map, mat, sp, year, month,
        true); if (month==3 || month==6 || month==9 || month==12)
                                rw.write_frq_data(*param, sp, year, qtr,
        value(mat.dvarLF_est(sp)), true);
                }
        }*/
        ...
        if (!param->flag_no_fishing) {
            if (qtr != past_qtr) mat.C_N_sp_age_fishery_qtr.initialize();
                    ...
                    for (int age = a0_adult[sp]; age < aN_adult[sp]; age++)
                        for (int k = 0; k < param->nb_fishery_by_sp[sp]; k++)
                            for (int r = 0; r < param->nb_region_sp_B[sp]; r++)
                                mat.C_N_sp_age_fishery_qtr[sp][age][r][k] =
                                    mat.C_N_sp_age_fishery[sp][age][k][r];
            }
        }
    }
    // save ascii file with sums
    rw.SaveSepodymFileTxt(
        *param, mat, map, sumPP, sumF, sumFprime, sumF_area_pred,
        sumF_required_by_sp, mean_omega_sp, day, month, year, t_count, past_qtr,
        qtr, nbi, nbj);
...
void SeapodymCoupled::WriteFluxes() {
    // Note, to put frq_like=0 and stop if regions are not defined and activated
    // in Fluxes comp mode, then remove this condition
...
void SeapodymCoupled::WriteFileHeaders_submodel(const string fileout) {
    ...
    double maxval = 1.0;
    rw.wbin_header(
        fileout, param->idformat, param->idfunc, minval, maxval, param->nlong,
        param->nlat, nbt_total, zlevel[0], zlevel[nbt_total - 1], mat.xlon,
        mat.ylat, zlevel, mat.mask);
}

void SeapodymCoupled::InitFileFluxes() {
    // Create and initialize (txt) files with cohort fluxes through regional
    // boundaries WARNING: temporal - single matrix for all species! To be
    // modified in case of multi-species simulations
    int nb_reg = param->nb_EEZ;
    // by default will compute fluxes between polygons, except if the nb_EEZ =
    // 0, then use regional structure
...
void SeapodymCoupled::SaveCohortsDym(int sp, bool WriteHeader, dvector zlevel) {
    ...
    for (int a = 0; a < nb_ages; a++) {
        ...
        ostr << a + 1;
        string fileout = param->strdir_output + param->sp_name[sp] + "_age" +
                         ostr.str() + ".dym";
        if (WriteHeader) {
            // Write file headers during the first time step
            rw.wbin_header(
                fileout, param->idformat, param->idfunc, minval, maxval,
                param->nlong, param->nlat, nbt_total, zlevel[0],
                zlevel[nbt_total - 1], mat.xlon, mat.ylat, zlevel, mat.mask);
...
void SeapodymCoupled::SaveOneCohortDym(
    int sp, bool WriteHeader,
    dvector zlevel) {  // First created for Joe (SimpoDym) at 20160229
    double minval = min(value(mat.dvarDensity(sp, 3)));
    double maxval = max(value(mat.dvarDensity(sp, 3)));

    string fileout =
        param->strdir_output + param->sp_name[sp] + "_cohort_density.dym";
    if (WriteHeader) {
        // Write file headers during the first time step
        rw.wbin_header(
            fileout, param->idformat, param->idfunc, minval, maxval,
        ...
        for (int age = 0; age < nb_ages; age++) {
            ...
            if (t_count + 2 >= param->sp_nb_cohorts[sp] - 1 &&
        ...
        for (int i = map.imin; i <= map.imax; i++) {
            for (int j = map.jinf[i]; j <= map.jsup[i]; j++) {
                if (map.carte[i][j]) {
                    // Units: Nb/sq.km
                    mat2d(i - 1, j - 1) =
                        value(mat.dvarDensity(sp, age_extract, i, j));
                }
            }
        }
        rw.wbin_transpomat2d(fileout, mat2d, nbi - 2, nbj - 2, true);
