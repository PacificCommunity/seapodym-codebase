...
void SeapodymCoupled::IFR_age_comp(
    dvar_matrix& IFR, dmatrix FR_pop, dmatrix ISR_denom, const int age,
    const int sp,
    const int
        t) {  // IFR_age - index of food requirement by age (accessible food /
              // required food) IFR_pop - index of population food requirement
              // (total food resource / total requirement) ISR_age - index of
              // sharing the food resource, magnifies the intensity of
              // competition or its absence residual_competition - percentage of
              // food resource available for species given the presence of other
              // species in the habitat (considered constant in mono-species
              // model)
    ...
    for (int i = imin; i <= imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte[i][j]) {
                ...
                for (int n = 0; n < nb_forage; n++)
                    theta(n) =
                        value(mat.dvarF_access(n, age, i, j)) / sumF_access;
