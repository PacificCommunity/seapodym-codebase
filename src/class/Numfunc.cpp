...
#define TINY \
    1.0e-20  // Will regularize the unusual case of complete correlation
...
// fonction  de calcul de correlation entre les captures observees (xx) et
// predites (yy) les zero ne sont pas pris en compte dans le calcul

void CNumfunc::corcatch(
...
void CNumfunc::corcpue(
    DMATRIX &xx, DMATRIX &yy, dmatrix &eff, const int imin, const int imax,
    const ivector jinf, const ivector jsup, int &nn, double &cor, double &z,
    double &prob, const IMATRIX &mask, double missval) {
    dmatrix xxx, yyy;
    xxx.allocate(imin, imax, jinf, jsup);
    xxx.initialize();
    yyy.allocate(imin, imax, jinf, jsup);
    yyy.initialize();
    for (int i = imin; i <= imax; i++)
        for (int j = jinf[i]; j <= jsup[i]; j++)
...
// fonction qui retourne la somme des carres des differences a la moyenne de x
// et y et le produit des differences a la moyenne de x et y

void CNumfunc::sumdif(
    const DMATRIX &xx, const DMATRIX &yy, const int imin, const int imax,
    const ivector jinf, const ivector jsup, const int nn, double sx, double sy,
    double &sxx, double &syy, double &sxy, const double missval) {
...
// correlation is disproved (prob whose small value indicates a significant
// correlation), and Fisher's z (returned as z), whose value can be used in
// further statistical tests as described in 'Numerical Recipes in C, Press and
// al. 1994 (14.5 p. 638)

void CNumfunc::pearsn(
    ...
    z = (double)(0.5 * log((1.0 + (r) + TINY) /
                           (1.0 - (r) + TINY)));  // Fisher'sz transformation
    double df = n - 2;
    double t = (r)*sqrt(
        df / ((1.0 - (r) + TINY) * (1.0 + (r) + TINY)));  // Equation (14.5.5)
    prob = (double)(betai(
        0.5 * df, 0.5, df / (df + t * t)));  // Student's t probability */

    // for large n, the easier computation of prob below, using the short
    // routine erfcc, would give approximatively the same value
...
// omit if five-figure accuracy is good enough	Ref.: Numerical Recipes p.214
...
double CNumfunc::betai(double a, double b, double x) {
    ...
    if (x == 0.0 || x == 1.0)
        ...
        bt =
            exp(gammln(a + b) - gammln(a) - gammln(b) + a * log(x) +
                b * log(1.0 - x));
    if (x < (a + 1.0) / (a + b + 2.0))
