...
/// They compute local proportions of catch by fishery in the total catch over
/// 'no effort' fisheries. Note, the catches being used need to have the same
/// units.
                ...
                for (int i = map.imin; i <= map.imax; i++) {
                    ...
                    for (int j = jmin; j <= jmax; j++) {
                        if (map.carte(i, j) && catch_obs_total(i, j)) {
                            mat.Ctot_proportion_fishery(sp, k, i, j) =
...
// call in run_coupled
// Recomp_C_fishery_proportion_in_Ctot(map, *param, rw,
// Ctot_proportion_fishery(sp), year, month, sp, k);
void CCalpop::Recomp_C_fishery_proportion_in_Ctot(
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j) && catch_obs_total(i, j)) {
                Ctot_proportion_fishery(i, j) =
                    catch_obs(k, i, j) / catch_obs_total(i, j);
            }
        }
    }
}

void CCalpop::predicted_catch_fishery_no_effort_comp(
    const PMap& map, CParam& param, VarMatrices& mat, const int f, const int k,
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (map.carte(i, j) && C_proportion_f) {
                ...
                double C_f_age_est_cell =
                    C_est_age * C_proportion_f * area / mat.lat_correction(j);

                mat.dvarCatch_est(sp, k).elem_value(i, j) +=
                    C_f_age_est_cell * weight;
