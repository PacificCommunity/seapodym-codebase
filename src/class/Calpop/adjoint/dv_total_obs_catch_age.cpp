...
/// age. Forward functions are in total_obs_catch_age.cpp
...
void dv_total_obs_catch_age_comp(void) {
    ...
    pop->Recomp_total_exploited_biomass(
        *map, *param, *mat, EB, catch_obs, pop->Selectivity(sp, f), f, sp,
        t_count);
    ...
    for (int j = jmax; j >= jmin; j--) {
        ...
        for (int i = imax; i >= imin; i--) {
            if (map->carte(i, j) && EB(i, j)) {
                ...
                for (int a = nb_region_sp - 1; a >= 0; a--) {
                    ...
                    if (nb_region) {
                        if ((i >= map->regimin[a]) && (i < map->regimax[a]) &&
                            ...
                            // mat.C_N_sp_age_fishery[sp][age][k][r] = LF_pr_reg
                            // + LF_est; ; dfLF_est   += dfLF(r);
                            dfCa_est += dfLF(r);
                            dfLF_pr_reg += dfLF(r);
                            dfLF(r) = 0;
                        }
                ...
                // Ctot_age_obs.elem_value(i,j) = Ctot_pr(i,j) + LF_est *
                // Cobs(i,j) * C2Dunits;
                dfLF_est += catch_obs(i, j) * C2Dunits *
                            mat->lat_correction(j) * dfCtot(i, j);
                ...
                dfSNsum(i, j) -=
                    dfLF_est * selectivity_f_age * uu(i, j) / pow(EB(i, j), 2);
                dfLF_est = 0.0;
