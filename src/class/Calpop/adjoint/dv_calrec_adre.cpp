...
void dv_calrec_adre();
void dftridag_y(
    dvector& a, dvector& bet, dvector& c, dvector& dfrhs, dvector& dfuvec,
    dvector& dfgam, int inf, int sup);
void dftridag_x(
    dvector& a, dvector& bet, dvector& c, dvector& rhs, dvector& uvec,
    dvector& gam, dvector& dfbet, dvector& dfrhs, dvector& dfuvec,
    dvector& dfgam, int inf, int sup);
...
void dv_calrec_adre() {
    ...
    for (int itr = iterationNumber; itr >= 1; itr--) {
        ...
        for (int j = jsup; j >= jinf; j--) {
            ...
            dftridag_x(
                a(j), xbet(j), c(j), rhs, uvec, gam, dfxbet(j), dfxrhs, dfuvec,
                dfxgam, imin, imax);
