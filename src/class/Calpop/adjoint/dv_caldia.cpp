...
void f_accessibility(
    dvector& l_access, dvector& lf_access, const dvector forage,
    const dvector O2, const dvector T, double twosigsq, double temp_mean,
    double oxy_teta, double oxy_cr, const int nl, const int nb_forage,
    const ivector day_layer, const ivector night_layer, const double DL);
void f_accessibility(
    dvector& l_access, dvector& lf_access, const dvector forage,
    const dvector O2, const dvector T, double temp_mean, double temp_max,
    double delta1, double delta2, double delta3, double oxy_teta, double oxy_cr,
    const int nl, const int nb_forage, const ivector day_layer,
    const ivector night_layer, const double DL);

void dfybet_comp(
    dmatrix& dfybet, dmatrix& dfd, dmatrix& dfe, dmatrix& dff, const dmatrix d,
    const dmatrix e, const dmatrix f, unsigned long int pos_map, const int maxn,
    const int dt);
void dfd1_comp(
    const char pos, double& dfd1, double& dfsigmam, double& dfsigma,
    double& dfuinf, const double uinf, const double twodd, const double d);
void dfd2_comp(
    const char pos, double& dfd2, double& dfsigmam, double& dfsigma,
    double& dfsigmap, double& dfu, const double u, const double twodd,
    const double d);
void dfd3_comp(
    const char pos, double& dfd3, double& dfsigma, double& dfsigmap,
    double& dfusup, const double usup, const double twodd, const double d);
void dfdh_comp(
    const char pos, double& dfHinf, double& dfH, double& dfHsup, double dfdh,
    const double d);
void dfdhx_comp(
    const char pos, double& dfHinf, double& dfH, double& dfHsup, double dfdh,
    const double habitat, const double habitat_sup, const double habitat_inf,
    const double d);
void dfdhy_comp(
    const char pos, double& dfHinf, double& dfH, double& dfHsup, double dfdh,
    const double habitat, const double habitat_sup, const double habitat_inf,
...
void dv_caldia() {
    ...
    pop->Recomp_DEF_coef(
        *map, *param, *mat, t_count, jday, habitat, d, e, f, adv_x, adv_y, sp,
        age, mss_species, c_diff_fish, sigma_species);
    ...
    for (int i = isup; i >= iinf; i--) {
        for (int j = map->jsup[i]; j >= map->jinf[i]; j--) {
            ...
            dfd3_comp(
                pos, dff(i, j), dfDiff_y(i, j), dfDiff_y(i, j + 1),
                dfAdv_y(i, j + 1), adv_y(i, j + 1), twodydy, dy);

            // e[i][j] = d2(pos, sigmam, sigma, sigmap, v, twodydy, dydy, dy,
            // dt);
            dfd2_comp(
                pos, dfe(i, j), dfDiff_y(i, j - 1), dfDiff_y(i, j),
                dfDiff_y(i, j + 1), dfAdv_y(i, j), adv_y(i, j), twodydy, dy);

            // d[i][j] = d1(pos, sigmam, sigma, vinf, twodydy, dydy, dy);
            dfd1_comp(
                pos, dfd(i, j), dfDiff_y(i, j - 1), dfDiff_y(i, j),
                dfAdv_y(i, j - 1), adv_y(i, j - 1), twodydy, dy);
        }
    }
    ...
    for (int j = jsup; j >= jinf; j--) {
        for (int i = map->isup[j]; i >= map->iinf[j]; i--) {
            ...
            dfd3_comp(
                pos, dfc(j, i), dfDiff_x(i, j), dfDiff_x(i + 1, j),
                dfAdv_x(i + 1, j), adv_x(i + 1, j), twodxdx, dx);

            // b[j][i] = d2(pos, sigmam, sigma, sigmap, u, twodxdx, dxdx, dx);
            dfd2_comp(
                pos, dfb(j, i), dfDiff_x(i - 1, j), dfDiff_x(i, j),
                dfDiff_x(i + 1, j), dfAdv_x(i, j), adv_x(i, j), twodxdx, dx);

            // a[j][i] = d1(pos, sigmam, sigma, uinf, twodxdx, dx);
            dfd1_comp(
                pos, dfa(j, i), dfDiff_x(i - 1, j), dfDiff_x(i, j),
                dfAdv_x(i - 1, j), adv_x(i - 1, j), twodxdx, dx);
        }
    }
    ...
    const double unit_x =
        pow(length, mss_size_slope) * (3600 * 24.0 * deltaT / 1852) * dx;
    const double unit_y =
        pow(length, mss_size_slope) * (3600 * 24.0 * deltaT / 1852) * dy;
    const double Dspeed = Vmax_diff - 0.25 * length / lmax;
    const double Dinf =
        pow(Dspeed * length * 3600 * 24.0 * deltaT / 1852, 2) / (4.0 * deltaT);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nl > 0) {
                ...
                if (nl >= nb_layer) {
                    //	                    		dHdx = (habitat[i+1][j] -
                    //habitat[i][j])/dx; 					dHdy = (habitat[i][j+1] -
                    //habitat[i][j])/dy;

                    if (pos_x == SANS)
                        dHdx =
                            (habitat[i + 1][j] - habitat[i - 1][j]) / (2 * dx);
                    ...
                    if (pos_y == SANS)
                        dHdy =
                            (habitat[i][j + 1] - habitat[i][j - 1]) / (2 * dy);
                ...
                double dfrho_y = 0.0;
                /*
                                                //advection_y(i,j) = mat.v[i][j]
                   + MSS * unit_y * dHdy; dfMSS(i,j) += unit_y * dHdy *
                   dfAdv_y(i,j); dfdhdy     += mss_species * unit_y *
                   dfAdv_y(i,j); dfAdv_y(i,j) = 0.0;

                                                //const double length =
                   param->length[sp][age]*0.01;
                                                //advection_x(i,j) = mat.u[i][j]
                   + MSS * unit_x * dHdx * mat.lat_correction[j]; dfMSS(i,j) +=
                   unit_x * dHdx  * lat_correction(j) * dfAdv_x(i,j); dfdhdx +=
                   mss_species * unit_x * lat_correction(j) * dfAdv_x(i,j);
                                                dfAdv_x(i,j) = 0.0;
                */
                ...
                if (v_y >= 0)
                    dfv_y = param->dffunc_limit_one(v_y, Vinf * dfv_y) / Vinf;
                if (v_y < 0)
                    dfv_y = param->dffunc_limit_one(-v_y, Vinf * dfv_y) / Vinf;

                // v_x = Vinf*param.func_limit_one(v_x/Vinf);
                if (v_x >= 0)
                    dfv_x = param->dffunc_limit_one(v_x, Vinf * dfv_x) / Vinf;
                if (v_x < 0)
                    dfv_x = param->dffunc_limit_one(-v_x, Vinf * dfv_x) / Vinf;
                ...
                if (nl >= nb_layer) {
                    ...
                    dfdh_comp(
                        pos_y, dfH(i, j - 1), dfH(i, j), dfH(i, j + 1), dfdhdy,
                        dy);
                    dfdh_comp(
                        pos_x, dfH(i - 1, j), dfH(i, j), dfH(i + 1, j), dfdhdx,
                        dx);
                ...
                /*
                                                //double D =
                   Dmax*(1-habitat(i,j)/(c_diff_fish+habitat(i,j))); dfH(i,j) -=
                   Dmax * c_diff_fish/pow(c_diff_fish+habitat(i,j),2) * dfD;
                                                //dfSigma(i,j) += 1000 *
                   (1-habitat(i,j)/(c_diff_fish+habitat(i,j))) * dfD;
                                                dfSigma(i,j) += Dinf *
                   (1-habitat(i,j)/(c_diff_fish+habitat(i,j))) * dfD;
                                                dfC_diff(i,j)+= Dmax *
                   habitat(i,j)/pow(c_diff_fish+habitat(i,j),2) * dfD; dfD =
                   0.0;
                */
                // double D = Dmax*(1-c_diff_fish*pow(habitat(i,j),3));
                dfH(i, j) -=
                    Dmax * 3.0 * c_diff_fish * pow(habitat(i, j), 2) * dfD_pr;
                dfSigma(i, j) +=
                    Dinf * (1 - c_diff_fish * pow(habitat(i, j), 3)) * dfD_pr;
                dfC_diff(i, j) -= Dmax * pow(habitat(i, j), 3) * dfD_pr;
                dfD_pr = 0.0;

                /*  				//double D =
                   Dmax*(1-D_dec_H1*pow(habitat(i,j),c_diff_fish)); dfH(i,j) -=
                   Dmax * c_diff_fish * D_dec_H1 *
                   pow(habitat(i,j),c_diff_fish-1) * dfD_pr; dfSigma(i,j) +=
                   Dinf * diff_habitat * dfD_pr; dfC_diff(i,j)-= Dmax * D_dec_H1
                   * pow(habitat(i,j),c_diff_fish) * log(habitat(i,j)) * dfD_pr;
                                                dfD_pr = 0.0;
                */

                /*				//double D =
                   Dmax*(1-c_diff_fish*pow(habitat(i,j),3)); dfH(i,j)     -=
                   Dmax * 3.0 * c_diff_fish*pow(habitat(i,j),2) * dfD;
                                                dfSigma(i,j) += Dinf *
                   (1-c_diff_fish*pow(habitat(i,j),3)) * dfD; dfC_diff(i,j)-=
                   Dmax * pow(habitat(i,j),3) * dfD; dfD = 0.0;
                */
            }
        }
    }
...
void dv_caldia_UV() {
    ...
    ivector day_layer(0, nb_forage - 1);
    day_layer = param->day_layer;
    ivector night_layer(0, nb_forage - 1);
    night_layer = param->night_layer;
    ...
    for (int i = iinf; i <= isup; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0) {
                ...
                if (Tfunc_Gaussian) {
                    f_accessibility(
                        l_access, lf_access, F, O2, T, twosigsq, temp_age,
                        oxy_teta, oxy_cr, nb_layer, nb_forage, day_layer,
                        night_layer, DL);
                } else {
                    f_accessibility(
                        l_access, lf_access, F, O2, T, temp_age, temp_max,
                        delta1, delta2, delta3, oxy_teta, oxy_cr, nb_layer,
                        nb_forage, day_layer, night_layer, DL);
                }
                for (int l = 0; l < nl; l++) {
    ...
    // cout << "2: " << t_count << " " << age << " "<<  l_access << " " <<
    // lf_access << " " << temp_age << " " << temp_max << " " << delta1 << " "
    // << delta2 << " " << delta3 << " " << oxy_teta << " " << oxy_cr << " " <<
    // nb_layer << endl;
    ...
    pop->Recomp_DEF_UV_coef(
        *map, *param, *mat, u, v, habitat, d, e, f, adv_x, adv_y, sp, age,
        mss_species, c_diff_fish, sigma_species, jday);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nl > 0) {
                ...
                /*
                                                //advection_y(i,j) =
                   c*mat.v[i][j] + MSS * unit_y * dHdy; dfV(i,j)   += c *
                   dfAdv_y(i,j); dfc 	   += v(i,j) * dfAdv_y(i,j); dfMSS(i,j)
                   += unit_y * dHdy * dfAdv_y(i,j); dfdhdy     += mss_species *
                   unit_y * dfAdv_y(i,j); dfAdv_y(i,j) = 0.0;

                                                //advection_x(i,j) =
                   c*mat.u[i][j] + MSS * unit_x * dHdx * mat.lat_correction[j];
                                                dfU(i,j)   += c * dfAdv_x(i,j);
                                                dfc 	   += u(i,j) *
                   dfAdv_x(i,j); dfMSS(i,j) += unit_x * dHdx  *
                   lat_correction(j) * dfAdv_x(i,j); dfdhdx     += mss_species *
                   unit_x * lat_correction(j) * dfAdv_x(i,j); dfAdv_x(i,j) =
                   0.0;
                */
                // precompute v_x and v_y
                double v_x = mss_species * unit_x * dHdx / Vinf;
                ...
                // advection_x(i,j) = c*mat.u[i][j] + v_x *
                ...
                // finally, contribution to derivatives dfU and dfV through c
                // and r const double c = 1.0-r*length/lmax;
                dfr -= dfc * length / lmax;
                ...
                double dfC = -(500.0 * deltaT / 30.0) * dffV /
                             pow(500.0 * deltaT / 30.0 + expr2, 2);
                // expr2 = sqrt(U*U+V*V);
                if (expr2 > 0) {
                ...
                */
                /*
                   ...
                   Dmax * 3.0 * c_diff_fish*pow(habitat(i,j),2) * dfD_pr;
                                                dfSigma(i,j) += Dinf *
                   (1-c_diff_fish*pow(habitat(i,j),3)) * dfD_pr; dfC_diff(i,j)-=
                   Dmax * pow(habitat(i,j),3) * dfD_pr; dfD_pr = 0.0;
                */
                ...
                dfH(i, j) -=
                    Dmax * 3.0 * c_diff_fish * pow(habitat(i, j), 2) * dfD;
                dfSigma(i, j) +=
                    Dinf * (1 - c_diff_fish * pow(habitat(i, j), 3)) * dfD;
                ...
                /*  				//double D =
                   Dmax*(1-D_dec_H1*pow(habitat(i,j),c_diff_fish)); dfH(i,j) -=
                   Dmax * c_diff_fish * D_dec_H1 *
                   pow(habitat(i,j),c_diff_fish-1) * dfD; dfSigma(i,j) += Dinf *
                   diff_habitat * dfD; dfC_diff(i,j)-= Dmax * D_dec_H1 *
                   pow(habitat(i,j),c_diff_fish) * log(habitat(i,j)) * dfD; dfD
                   = 0.0;
                */
            }
        }
    }
    // cout << norm(dfH) << " " << norm(dfU) << " " << norm(dfV) << " "<<
    // norm(dfd)<< " " << norm(dfe) << " "<< norm(dff)<< endl;
    dfa.save_dmatrix_derivatives(a_pos);
    dfb.save_dmatrix_derivatives(b_pos);
    dfc.save_dmatrix_derivatives(c_pos);
    dfd.save_dmatrix_derivatives(d_pos);
...
void dfybet_comp(
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        // ybet[i][jmin] = 1/(e[i][jmin]+dt);
        dfe(i, jmin) -=
            (1 / ((e(i, jmin) + dt) * (e(i, jmin) + dt))) * dfybet(i, jmin);
        dfybet(i, jmin) = 0.0;
    }
}
