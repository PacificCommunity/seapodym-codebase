...
void CCalpop::precaldia_comp(
    ...
    // BL=(0.1,0.5,1.0,2.0) m. correspondingly which results in speeds
    // (1,1.6,2,2.5) m/sec.
    const double CHI_x =
    ...
    const double Dspeed =
        Vmax_diff -
        0.25 * length / lmax;  // fixed, given in 'body length' units
    const double Dinf =
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0) {
                ...
                double dHdy = 0;
                if (habitat(i, j) == 0) {
                    cout << "Zero habitat at " << age << " " << i << " " << j
                         << " " << endl;
                    exit(1);
                }
                if (nl >= nb_layer) {  // only if all layers exist! Make sure
                                       // the mask has value 3 for all layers!
                    ...
                    if (map.global) {
                        if ((i == 1) && (map.carte[nti - 2][j])) {
                            if (pos_x == SANS)
                                dHdx =
                                    (habitat[i + 1][j] - habitat[nti - 2][j]) /
                                    (2 * dx);
                            else if (pos_x == D_FERME)
                                dHdx = (habitat[i][j] - habitat[nti - 2][j]) /
                                       (dx);
                        }
                        if ((i == nti - 2) && (map.carte[1][j])) {
                            if (pos_x == SANS)
                                dHdx = (habitat[1][j] - habitat[i - 1][j]) /
                                       (2 * dx);
                            else if (pos_x == G_FERME)
                ...
                // reduction of upper layer current speed due to vertical
                // behaviour of tuna within layers(used for test) (1-r) the
                //coefficient of reduction for ~0 currents and largest fish.
                const double r = rmax / (1.0 + (rc * 30.0 / dt) * C);
                const double c = 1.0 - r * length / lmax;

                // species velocity is proportional to the gradient of habitat
                // index:
                v_x = CHI_x * dHdx;
                v_y = CHI_y * dHdy;
                // limit maximal velocity by Vinf to avoid approximation errors
                // with finite differences in case of strong gradients
                ...
                // to save predicted speed for some age group (currently the
                // last one)
                if (age == agemax) speed[j] = sqrt(v_x * v_x + v_y * v_y);
            }
        }
    }
}
