...
void CCalpop::InitCalPop(CParam& param, const PMap& map) {
    ...
    Mss_species.allocate(map.imin, map.imax, map.jinf, map.jsup);
    Mss_species.initialize();
    Mss_size_slope.allocate(map.imin, map.imax, map.jinf, map.jsup);
    Mss_size_slope.initialize();
    C_diff_fish.allocate(map.imin, map.imax, map.jinf, map.jsup);
    C_diff_fish.initialize();
    Sigma_species.allocate(map.imin, map.imax, map.jinf, map.jsup);
    Sigma_species.initialize();
    ...
    // The maximal speed, which is supported by ADI, i.e. does not generate
    // negative densities due to approximation errors with finite differences in
    // case of strong gradients, high MSS (mostly for largest fish) and nearly
    // zero densities. Vinf will be used to constrain resulting velocities as a
    // function of local habitat gradients. Vinf is set to allow <=nmax grid
    // cells moves per one iteration. The value nmax is set empirically. Thus,
    // from optimizations with SKJ tagging data it is necessary to set nmax=2.
    // Examples of run configurations with
    // nmax=3:
    // for 1deg,1month,30N Vinf = 5400 nmi/mo; for 1/4deg,7days,28N Vinf = 5400
    // nmi/mo for 1/12deg,1day,12N Vinf = 5400 nmi/mo nmax=2: for
    // 1deg,1month,45N Vinf = 5400 nmi/mo; for 1/4deg,7days,42N Vinf = 5400
    // nmi/mo for 1/12deg,1day,18N Vinf = 5400 nmi/mo
    const int nmax = 1;
    Vinf = nmax * deltax * iterationNumber;

    // Exploited biomass
    int nb_ne_fishery = sum(
        param
            .mask_fishery_sp_no_effort[0]);  // ATTN: violation of multi-species
    if (!param.flag_no_fishing) {            //
        ...
        if (nb_ne_fishery) {
            ...
            for (int fne = 0; fne < nb_ne_fishery; fne++) {
                dvarsSNsum(fne).allocate(
                    map.imin, map.imax, map.jinf, map.jsup);
                dvarsSNsum(fne).initialize();
            }
        }
    } else {
        // do not allocate memory for selected by fisheries biomass in
        // NO-FISHING run
    }
