...
void f_accessibility(
    ...
    const dvector O2, const dvector T, double twosigsq, double temp_age,
...
void CCalpop::RecompDiagCoef_juv(
    const PMap& map, CMatrices& mat, const int t_count, const dmatrix mortality,
    ...
    for (int j = map.jmin; j <= map.jmax; j++) {
        for (int i = map.iinf[j]; i <= map.isup[j]; i++) {
            ...
            bbm[j][i] =
                d2(pos, sigmam, sigma, sigmap, ucur, twodxdx, dxdx, dx, 0);
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        for (int j = map.jinf[i]; j <= map.jsup[i]; j++) {
            ...
            ee[i][j] =
                d2(pos, sigmam, sigma, sigmap, vcur, twodydy, dydy, dy, 0);
...
void CCalpop::Recomp_DEF_coef(
    ...
    const double CHI_x =
        MSS * pow(length, MSS_size_slope) * (3600 * 24.0 * dt / 1852) * dx;
    const double CHI_y =
        MSS * pow(length, MSS_size_slope) * (3600 * 24.0 * dt / 1852) * dy;
    const double Dspeed = Vmax_diff - 0.25 * length / lmax;
    const double Dinf =
        pow(Dspeed * length * 3600 * 24.0 * dt / 1852, 2) / (4.0 * dt);
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0) {
                ...
                double U = mat.un[t_count][0][i][j];
                double V = mat.vn[t_count][0][i][j];
                ...
                advection_x(i, j) =
                    c * U +
                    v_x * mat.lat_correction[j];  // for computing derivatives
                                                  // in dv_caldia
...
void CCalpop::Recomp_DEF_UV_coef(
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0) {
                ...
                advection_x(i, j) =
                    c * U +
                    v_x * lat_correction[j];  // for computing derivatives in
                                              // dv_caldia
                advection_y(i, j) = c * V + v_y;
...
void CCalpop::RecompDiagCoef_UV_adult(
    const PMap& map, CParam& param, CMatrices& mat, const int t_count,
    const int jday, const dmatrix& mortality, const dmatrix& habitat,
    dmatrix& aa, dmatrix& bbm, dmatrix& cc, dmatrix& dd, dmatrix& ee,
    dmatrix& ff, const int sp, const int age, const double MSS,
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0) {
                ...
                // need to recompute average currents, attn, accessibility is
                // computed for ages param.age_compute_habitat[sp][age] only
                // (passed here as age)
                if (Tfunc_Gaussian) {
                    f_accessibility(
    ...
    // cout << "1: " << t_count << " " << age << " "<<  l_access << " " <<
    // lf_access << " " << temp_age << " " << temp_max << " " << delta1 << " "
    // << delta2 << " " << delta3 << " " << oxy_teta << " " << oxy_cr << " " <<
    // nb_layer << endl; cout << norm(advection_x) << endl; CALDIA SECTION
    double dxdx = dx * dx;
    double twodxdx = 2 * dxdx;
