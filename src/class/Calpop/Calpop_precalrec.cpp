...
/*
void CCalpop::precalrec(VarParamCoupled& param, PMap& map, CMatrices& mat,
dvar_matrix& mortality, bool flag_fishing, const int sp, const int age)
{
        dvarsBM = dvarsB;
                        ...
                        if (map.carte(i,j)){
                                ...
                                if ( flag_fishing ){
                                        ...
                                        for (int f=0; f<nb_fishery; f++){
                                                if (
param.mask_fishery_sp[sp][f]){ dvariable catchability =
param.dvarsQ_sp_fishery[sp][k]; double selectivity  =
param.selectivity_sp_fishery_age[sp][k][age]; fishing_mortality +=
mat.effort[f][i][j]*catchability*selectivity*mat.lat_correction(j); k++;
                                                }
                                        }
