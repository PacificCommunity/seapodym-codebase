...
// void CMatrices::createMatTransport(const CParam& param, const PMap& map, int
// nbi, int nbj)
{
...
void CMatrices::createMatOcean(
    const PMap& map, int t0, int nbt, int nbi, int nbj, int nb_layer, int dt) {
    // 1D
    lastlat.allocate(map.jmin, map.jmax);  // Latitude of cell j
    lat_correction.allocate(
        map.jmin, map.jmax);  // Correction for cell area at latitude j
    // maxGD_lat.allocate(map.jmin, map.jmax);				// Correction for cell area at
    // latitude j
    // dDL.allocate(map.jmin, map.jmax);				// Correction for cell area at
    // latitude j
    ...
    daylength.allocate(
        1, nbdays_max, 0, nbj - 1);  // Length of day based on latitude and date
    // grad_daylength.allocate(1, nbdays_max, 0, nbj-1);		// gradient of
    // daylength;

    u.allocate(
        map.imin, map.imax, map.jinf,
        map.jsup);  // u(i,j) = advection est-ouest suivant les i
    v.allocate(
        map.imin, map.imax, map.jinf,
        map.jsup);  // v(i,j) = advection nord-sud suivant les j
    speed.allocate(
        map.imin, map.imax, map.jinf,
        map.jsup);  // magnitude of species velocity (without passive compnt);
    ...
    vld.allocate(t0, nbt);  // variable defining vertical layer depths (can be
                            // either MLD or ZEU)
    ph1.allocate(t0, nbt);
...
void CMatrices::createMatForage(
...
void CMatrices::createMatHabitat(
    const PMap& map, const int nb_forage, const int nb_species, int t0, int nbt,
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        sigma_season(sp).allocate(
            1, 366, a0_adult, nb_ages - 1);  // to be checked!!!
        ...
        for (int n = 0; n < nb_forage; n++) {
            ...
            for (int t = t0; t <= nbt; t++)
                F_access_sum_age(sp, n, t).allocate(
    ...
    // store the habitat indices for selected ages: age_compute_habitat (see
    // Param class)
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int t = t0; t <= nbt; t++) {
            const int a0_adult = sp_adult_age0(sp);
            const int nb_ages =
                age_compute_habitat(sp, sp_nb_age_class(sp) - 1);
            adult_habitat(sp, t).allocate(a0_adult, nb_ages);
            // cout << "allocated adult_habitat for indices "<< a0_adult << "
            // "<<  nb_ages << endl;
            for (int a = a0_adult; a < nb_ages; a++) {
                adult_habitat(sp, t, a).allocate(
...
void CMatrices::createMatHabitat_input(
...
void CMatrices::createMatSpecies(
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        density_before(sp).allocate(
            t0, nbt);  // fish density before solution of ADRE
        for (int t = t0; t <= nbt; t++) {
            const int a0_adult = sp_adult_age0(sp);
            const int nb_ages = sp_nb_age_class(sp);
            ...
            for (int a = 0; a < nb_ages; a++) {
                if (a >= a0_adult)
                    density_before(sp, t, a).allocate(
                        map.imin1, map.imax1, map.jinf1, map.jsup1);
            }
        }
    }
    ...
    /*
            ...
            for (int sp = 0; sp < nb_species; sp++){
                    ...
                    for (int age = 0; age < agemax; age++){
                            pop_species(sp,age).allocate(map.imin1, map.imax1,
       map.jinf1, map.jsup1); pop_species(sp, age).initialize();
                    }
            }
            ...
            for (int sp = 0; sp < nb_species; sp++){
                    ...
                    for (int age = 0; age < agemax; age++){
                            juv_species(sp,age).allocate(map.imin1, map.imax1,
       map.jinf1, map.jsup1); juv_species(sp, age).initialize();
                    }
            }
    */
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int age = 0; age < nb_ages; age++) {
            init_density_species(sp, age).allocate(
                map.imin1, map.imax1, map.jinf1, map.jsup1);
...
void CMatrices::createMatEffort(
    const PMap& map, int nbi, int nbj, int nb_fleet) {
...
void CMatrices::createMatTotCatch(
...
void CMatrices::createMatCatch(
    const PMap& map, int nbi, int nbj, int nb_species, const IVECTOR& nb_fleet,
    const ivector a0_adult, const IVECTOR& nb_cohorts,
    const IVECTOR& nb_region_sp) {
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int f = 0; f < fleetmax; f++) {
            Ctot_proportion_fishery(sp, f).allocate(
                map.imin, map.imax, map.jinf, map.jsup);
    ...
    // C_N_sp_age_fishery : somme des captures (en nombre) estimees par espece,
    // par age et par flotille a chaque pas de temps C_N_sp_age_fishery_qtr:
    // somme des captures (en nombre) estimees par age et par flotille groupees
    // par trimestre
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int age = a0; age < agemax; age++) {
            ...
            C_tot_no_effort_sp_age[sp][age].allocate(
                map.imin, map.imax, map.jinf, map.jsup);
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int age = a0; age < agemax; age++) {
            ...
            for (int region = 0; region < regionmax; region++) {
                const int fleetmax = nb_fleet(sp);
                C_N_sp_age_fishery_qtr(sp, age, region)
                    .allocate(0, fleetmax - 1);
                C_N_sp_age_fishery_qtr(sp, age, region).initialize();
            }
        }
    }

    // Sum_C_N_sp_age_fishery_area: sommes totales des captures (en nombre)
    // estimees par age, espece, flotille et region
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int age = 0; age < agemax; age++) {
            ...
            for (int n = 0; n < 5; n++) {
                ...
                for (int region = 0; region <= regionmax; region++) {
                    const int fleetmax = nb_fleet(sp);
                    Sum_C_N_sp_age_fishery_area(sp, age, n, region)
                        .allocate(0, fleetmax);
                    Sum_C_N_sp_age_fishery_area(sp, age, n, region)
                        .initialize();
                }
...
void CMatrices::MeanVarMovement(
    ...
    if (sum_density == 0) {
        ...
        mean_diffusion(sp, age) =
            sigma_species *
            pow(Vdiff * length_age * 0.01 * 3600 * dT * 24.0 / 1852, 2) /
            (4 * dT);
    } else {
...
void CMatrices::MeanVarMortality(
    const PMap& map, const dmatrix& M, const double Mp_max, const double Ms_max,
    const double Mp_exp, const double Ms_slope, const double mean_age_in_month,
    const int sp, const int age) {
    double sum_density = sum(density_after(sp, age));
    //	double sum_density = 0;
    if (sum_density == 0) {
        mean_mortality(sp, age) =
            (Ms_max + Mp_max) * exp(-Mp_exp * mean_age_in_month) +
...
void CMatrices::MeanVarTemperature(
    const PMap& map, const int sp, const int sp_nb_cohort_lv,
    const int a0_adult, const int t_count) {  // larvae and juveniles only, see
                                              // OnRunCoupled for other cohorts
...
double CMatrices::comp_waverage(
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                ...
                // else
                //                                 res +=
                //                                 var(i,j)*density_before(sp,1,age,i,j);
            }
        }
    }
    res /= sum_density;
...
double CMatrices::comp_waverage2(
