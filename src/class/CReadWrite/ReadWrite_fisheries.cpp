...
// Function reads MONTHLY fishing data and distributes it to the model time step
// upon reading Note, we don't duplicate the data at every model time step,
// which is <= 30days, but only scale the effort and catch with the factor
// deltaT/30. Then the use of this data in accounting for fishing mortality or
// the prediction of catch is straighforward
void CReadWrite::rtxt_fishery_data(
    ...
    if (littxt) {
        cout << endl
             << "Reading fishing data from the file: " << endl
             << filename.c_str() << endl;
        littxt >> nb_fishery_file;
        ivector nb_recs_fishery(0, nb_fishery_file - 1);
        nb_recs_fishery.initialize();

        for (int f = 0; f < nb_fishery_file; f++) littxt >> nb_recs_fishery(f);
        ...
        while (rec < all_rec) {
            ...
            littxt >> fishery >> year >> month >> day >> gear >> lat >> lon >>
                freso >> eff >> harv;
            if (lon < 0) lon += 360.0;
                ...
                if (param.mask_fishery_sp[0][f]) {
                    if (fishery_name == param.list_fishery_name[f]) {
                        ...
                        if (date >= start_fishing && date <= end_fishing) {
                            ...
                            // scale eff and harv from monthly (30 days) to
                            // model temporal resolution cout << "Scale fishing
                            // effort by " << (double)param.deltaT / 30 << endl;
                            ...
                            if (i == 0 || j == 0) {
                                cout << "Attn: fishing data index 0 for "
                                        "(lon,lat) = ("
                                     << lon << "," << lat << "), (i,j) = (" << i
                                     << "," << j << ")" << endl;
                            }
                            ...
                            if ((i > hr_x && i + hr_x < nbi - 1) &&
                                ...
                                if (map.carte(i, j) && eff) {
                                    ...
                                    nrec_fishery(f)++;
                                    frec_tmp[nrec_oceanmask].set_record(
                                        lon, lat, i, j, eff, harv[0]);
        ...
        if (!DataExist && sum(param.mask_fishery_sp(0)) != 0) {
            ...
            cout
                << "WARNING: no fishery data found: fishery mask forced to zero"
                << endl;
        }
...
void CReadWrite::degrade_fishery_reso(
    ...
    int nflon =
        (int)((param.longitudeMax - param.longitudeMin + deltaX) / creso + .5);
    int nflat =
        (int)((param.latitudeMax - param.latitudeMin + deltaY) / creso + .5);
    flon.allocate(0, nflon - 1);
    flon.initialize();
    flat.allocate(0, nflat - 1);
    flat.initialize();
    ...
    // Interested to see only in optimisation mode, which does not support
    // currently high resolutions
    if (deltaX >= 1)
        cout << "Centers of degraded C-cells: \n"
             << flon[0] << ", " << flon[1] << ",..., " << flon[nflon - 1]
             << endl
             << flat[0] << ", " << flat[1] << ",..., " << flat[nflat - 1]
             << endl;
    ...
    for (int f = 0; f < nb_fishery; f++) {
        ...
        if (!param.mask_fishery_sp_no_effort[0][f]) {
            if (param.mask_fishery_sp[0][f]) {
                if (fishery_reso[f] <
                    creso) {  // we want only to degrade resolution!
                    // note: if length(data)=0, i.e. fishery_reso[f]==0 code
                    // below will be executed but 'p' loop
                    fishery_reso[f] = creso;
                    int m_old = 0;
                    for (int tcount = 1; tcount <= nbt; tcount++) {
                        // we need year and month only
                        Date::update_time_variables(
                            tcount, param.deltaT, param.date_mode, jday_spinup,
                            jday, day, month, year, xx);
                        int y = year - yearini;
                        int m = month;
                        if (m != m_old) {
                            ...
                            for (int p = nstart; p < nend; p++) {
                                ...
                                int fi = (int)((lon - (cwest + 0.5 * creso)) /
                                               creso);
                                int fj =
                                    (int)((cnorth - 0.5 * creso - lat) / creso);
                                int i = param.lontoi(flon[fi]);
                                int j = param.lattoj(flat[fj]);

                                // NOTICE: some records may be lost if the
                                // C-cell center is on land
                                if (map.carte(i, j) == 0) {
                                    i = -1;
                                    count++;
                                }
...
void CReadWrite::delete_fisheries_rec() {
    if (fisheries_data_read) delete[] frec;
}

// the fishing effort redistributed to the resolution of the model is used to
// compute fishing mortality and in MPA simulations
void CReadWrite::set_effort_rm(
    CParam& param, PMap& map, const int nbt, const int jday_spinup) {
    const int nb_fishery = param.get_nbfishery();
    const int nb_species = param.get_nbspecies();
    const int yearini = param.ndatini / 10000;
    ...
    long int maxn = nrec_oceanmask;  // corrected on 20210719, double check that
                                     // it works in all cases!
    if (!param.fdata_rm) maxn = nrec_oceanmask_original * af_m * af_n;
    if (maxn < 0) {
        cerr << "Wrong size of memory to allocate for fishing effort at model "
                "resolution, nrec_oceanmask is too big:"
             << nrec_oceanmask_original << "; Will exit now!" << endl;
        exit(1);
    }

    // temporal containers as we don't know here the size of redistributed
    // effort fishing_effort *efr_tmp;
    ...
    for (int f = 0; f < nb_fishery; f++)
        if (param.mask_fishery_sp[0][f]) {
            ...
            for (int tcount = 1; tcount <= nbt; tcount++) {
                // we need year and month only
                Date::update_time_variables(
                    tcount, param.deltaT, param.date_mode, jday_spinup, jday,
                    day, month, year, xx);
                int y = year - yearini;
                int m = month;
                if (m != m_old) {
                    ...
                    for (int p = nstart; p < nend; p++) {
                        ...
                        if (deltaX < fishery_reso(f) ||
                            deltaY < fishery_reso(f)) {
                            int ki = 0;
                            int kj = 0;
                            // this function returns area of fishing within one
                            // cell
                            param.afcoef(Elon, Elat, af, ki, kj, f);

                            // move the effort to the ocean if some cells within
                            // its area are on land
                            double afw = 0.0;
                            for (int ii = 0; ii < af_m; ii++)
                                for (int jj = 0; jj < af_n; jj++)
                                    if (af(ii, jj) &&
                                        map.carte(ii + ki, jj + kj))
                                        afw += af(ii, jj);
                            afw = 1.0 / afw;
                            for (int ii = 0; ii < af_m; ii++)
                                for (int jj = 0; jj < af_n; jj++) {
                                    if (af(ii, jj) &&
                                        ...
                                        if (C[0] > 0)
                                            mask_catch(ii + ki, jj + kj) +=
                                                afw * af(ii, jj);  //*C[0];
                                        efr_tmp[nrec_oceanmask].set_effort(
                                            ii + ki, jj + kj, eff);

                                        if (numrec_rm(f, y, m) ==
                                            0)  // position to start reading
                                            position_rm(f, y, m) =
                                                nrec_oceanmask;
                                        nrec_oceanmask++;
                                        numrec_rm(f, y, m)++;
                        ...
                        if (E - sumE > 1e-2) {
                            cout << "E was not conserved! " << Elon << " "
                                 << Elat << "E before redistribution = " << E
                                 << "!= " << sumE << "after!" << endl;
                        }
    ...
    for (int rec = 0; rec < nrec_oceanmask; rec++) efr[rec] = efr_tmp[rec];
    // deleting temporal container
    delete[] efr_tmp;
}

// the catch redistribution (optional) is necessary for MPA simulations ||
// EEZ/MFCL regional extraction only i.e. if (deltaX<fishery_reso(f) |
// deltaY<fishery_reso(f))
void CReadWrite::set_frec_rm(
    CParam& param, const PMap& map, const int nbt, const int jday_spinup) {
    cout << "Notification: EEZ or MPA option activated..." << endl;
    cout << "Begin redistribution of the fishing data, resolutions (by "
            "fishery) are:"
         << endl
         << param.fishery_reso << endl;
    ...
    Elon.allocate(0, nbi - 1, 0, nbj - 1);
    Elon.initialize();
    Elat.allocate(0, nbi - 1, 0, nbj - 1);
    Elat.initialize();
                ...
                if (m != m_old) {
                        ...
                        for (int j = 0; j < nbj - 1; j++) {
                            if (effort(i, j)) {
                                ...
                                if (deltaX < fishery_reso(f) ||
                                    ...
                                    int ki = 0;
                                    int kj = 0;
                                    // this function returns area of fishing
                                    // within one cell
                                    param.afcoef(
                                        Elon(i, j), Elat(i, j), af, ki, kj, f);

                                    // move the effort to the ocean if some
                                    // cells within its area are on land
                                    double afw = 0.0;
                                    for (int ii = 0; ii < af_m; ii++)
                                        for (int jj = 0; jj < af_n; jj++)
                                            if (af(ii, jj) &&
                                    ...
                                    reso_model(f) = 1;
                                } else {  // if original resolution < model
                                          // resolution
                                    // dvector harv(0,nb_species-1);
                                    // for (int sp=0; sp<nb_species; sp++)
                                    //	harv[sp] = Cobs(sp,i,j);
                    ...
                    if (sum(Cobs[0]) - sumC > 1e-2) {
                        cout << "WARNING: for the fishery " << f << " "
                             ...
                             << "total C before redistribution = "
                             << sum(Cobs[0]) << "!= " << sumC << "after!"
                             << endl;
                        exit(1);
                    }
                }
    ...
    cout << "Finished redistribution of the fishing data to the resolutions:"
         << endl
         << param.fishery_reso << endl;
...
void CReadWrite::set_frec_rm_no_effort_fisheries(
    CParam& param, const PMap& map, const int nbt, const int jday_spinup) {
    cout << "Notification: The redistribution of fisheries 'without effort' to "
                    ...
                    for (int i = 0; i < nbi - 1; i++) {
                        for (int j = 0; j < nbj - 1; j++) {
                            if (effort(i, j)) {
                                ...
                                if ((deltaX < fishery_reso(f) ||
                                     deltaY < fishery_reso(f)) &&
                                    param.mask_fishery_sp_no_effort
                                        [0][f]) {  // Attn: '0' for species!
                                    int ki = 0;
                                    int kj = 0;
                                    // this function returns area of fishing
                                    // within one cell
...
void CReadWrite::get_fishery_data(
...
void CReadWrite::mpa_areas_comp(PMap& map, CParam& param) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                if (map.maskMPA(i, j) < 0)
                    area_mpa += param.cell_surface_area(j);
                if (map.maskMPA(i, j) > 0)
                    area_els += param.cell_surface_area(j);
    ...
    cout << endl
         << "Areas: mpa = " << area_mpa << ", total = " << area_mpa + area_els
         << ", percentage of mpa = " << 100 * area_mpa / (area_els + area_mpa)
         << endl;
}

void CReadWrite::get_fishery_data_mpa(
    PMap& map, CParam& param, D3_ARRAY& effort, D4_ARRAY& catch_obs,
    D3_ARRAY& efflon, D3_ARRAY& efflat, int y, const int m) {
    // WARNING: mpa scenarios work for 'sp=0' only
    // mpa mask should be flagged with negative values for MPA and positive for
    // nonMPA zones
    y -= (int)param.save_first_yr;
    ...
    for (int f = 0; f < nb_fishery; f++) {
        ...
        if (param.mask_fishery_sp[0][f]) {
            ...
            // need to compute sum(CPUE) after reading all effort and catch
            // data: after redistribution their might be more than one record
            // for ij
            for (int i = map.imin; i <= map.imax; i++) {
                const int jmin = map.jinf[i];
                const int jmax = map.jsup[i];
            ...
            double sumtot =
                sum(sum_mpa) + sum_nonmpa;  // total effort at the time step
            // nothing to do if
            // 1. sum = 0
            ...
            if (!param.actual_eff) {
                ...
                if (sqrt(pow(sum(sum_mpa) - CHECKSUM, 2)) > 1e-3)
                    cout << sum_mpa << " " << sum(sum_mpa) << " != " << CHECKSUM
                         << endl;
            }
                            ...
                            for (int n = 0; n < nb_mpa; n++) {
                                ...
                                switch (S) {
                                    ...
                                    case 2:  // close MPA for fishing
                                        // see above, E is set =0 in 'negative'
                                        // cells
                                        break;
                                    case 3:  // close MPA and redistribute
                                             // effort equally outside of MPAs
                                        if (map.maskMPA(i, j) > 0)
                                            effort(f, i, j) +=
                                                sum_mpa[n] / count;
                                        break;
                                    case 4:  // close MPA and redistribute
                                             // effort ~ to:
                                        if (map.maskMPA(i, j) > 0) {
                                            ...
                                            if (sum_cpue_nonmpa >
                                                ...
                                                effort(f, i, j) *=
                                                    (1 +
                                                     sum_mpa[n] / sum_nonmpa);
                                            }
                                        }
                                        break;
                                    case 5:  // effort proportional reduction
                                             // equivalent to MPA closure
                                        effort(f, i, j) *=
                                            1 - sum_mpa[n] / sumtot;
                                        break;
                                }  // end of switch
                                if (map.maskMPA(i, j) == param.mpa_ID[n])
                                    sumE1[n] += effort(f, i, j);
                            }  // end of 'n' loop
            ...
            for (int n = 0; n < nb_mpa; n++) {
                ...
                switch (S) {
                    case 1:
                        ...
                        if (sqrt(pow(
                                sum_nonmpa + (1 + X) * sum_mpa[n] - aftersum,
                                2)) > 1e-3)
                            cerr << "WARNING: An error in the scenario " << S
                                 << " for the fishery " << f << ", "
                                 << sum_nonmpa + (1 + X) * sum_mpa[n]
                                 << " != " << aftersum << endl;
                        break;
                    case 2:
                    ...
                    case 3:
                        if (sqrt(pow(aftersum - beforsum, 2)) > 1e-3)
                            cerr << "WARNING: total effort of the fishery " << f
                                 << " is not preserved in the scenario " << S
                                 << ", " << beforsum << " != " << aftersum
                                 << endl;
                        break;
                    case 4:
                    ...
                    case 5:
                        if (sqrt(pow(aftersum - sum_nonmpa, 2)) > 1e-3)
                            cerr << "WARNING: total effort of the fishery " << f
                                 << " is not reduced by Empa in the scenario "
                                 << S << ", " << sum_nonmpa
                                 << " != " << aftersum << endl;
                        break;
                }
    ...
    if (wtxt) {
        wtxt << y + (int)param.save_first_yr << '\t' << m << '\t' << Ctot
             << '\t' << Ctot_mpa_fishery << '\t' << Ctot_mpa << endl;
    }
    wtxt.close();
}

void CReadWrite::inc_obs_catch_mpa(
...
void CReadWrite::get_average_effort(
    CParam& param, D3_ARRAY& effort, D3_ARRAY& efflon, D3_ARRAY& efflat,
    const int nby, const int m) {
    int y1 = max(param.save_first_yr, param.save_last_yr - nby) -
             param.save_first_yr;
...
void CReadWrite::get_average_effort_rm(
    CParam& param, dmatrix& effort, const int f, const int nby, const int m) {
    int y1 = max(param.save_first_yr, param.save_last_yr - nby + 1) -
             param.save_first_yr;
    int y2 = (int)param.save_last_yr - (int)param.save_first_yr;
...
// this effort is given on original data resolution, is used to predict catch
void CReadWrite::get_effort_lonlat(
...
// redistiributed to the resolution of the model fishing effort is used to
// compute fishing mortality
void CReadWrite::get_effort_rm(
    CParam& param, dmatrix& effort, const int f, int y, const int m) {
    effort.initialize();
    ...
    if (param.nb_yr_forecast)
        if (y > param.save_last_yr) {  // temporal, no month
            get_average_effort_rm(
                param, effort, f, 10, m);  // temporal, 10 must be global
            return;
        }
    y -= (int)param.save_first_yr;
    int nstart = position_rm(f, y, m);
...
void CReadWrite::get_average_selectivity(
    ...
    Ctot.initialize();
    int firstyear = (int)param.save_first_yr;
    int year = firstyear;
    int firstmonth = param.get_month(param.save_first_yr);
        ...
        for (int f = 0; f < nb_fishery; f++) {
            if (fisheries(f)) {
                ...
                for (int p = nstart; p < nend; p++) {
                    int i = frec[p].get_i();
                    int j = frec[p].get_j();
                    if (map.maskEEZ[i][j] < 0) Ctot(f) += frec[p].get_catch();
                    // dvector harv(0,nb_species-1);
                    // harv = frec[p].get_catch();
                    // for (int sp=0; sp<nb_species; sp++)
                    //         Ctot(f) += harv[sp];
    ...
    for (int a = 0; a < nb_ages; a++) {
        for (int f = 0; f < nb_fishery; f++)
            if (fisheries(f)) swa(a) += s(f, a) * Ctot(f) / Ctotf;
    }
    swa /= max(swa);
    cout << endl
         << "Total catches for fisheries used to compute exploited biomass = "
         << Ctot << endl;
...
void CReadWrite::read_lf_WCPO(
    CParam& param, string filename, const float startdate, const float enddate,
    const int sp) {  // data used by Multifan-CL, file changed only in header
    ...
    if (littxt) {
        ...
        for (int f = 0; f < nb_fleets; f++) littxt >> fcodes[f];
        for (int f = 0; f < nb_fleets; f++) littxt >> region[f];
        // read the line
        // flexible to different file contents, namely to the presence of weight
        // data
        string s;
        getline(littxt, s, '\n');
        getline(littxt, s, '\n');
        ...
        for (int n = 0; n < nb_records; n++) {
            ...
            if ((date >= startdate) && (date <= enddate)) {
                ...
                int qtr = month / 3;
                /// double L_pr =
                /// param.juv_length(sp,param.sp_nb_age_class_jv[sp]-1);
                ...
                for (int a = a0; a < nb_ages - 1; a++) {
                    ...
                    right =
                        0.5 * (param.length(sp, a) + param.length(sp, a + 1));
                ...
                for (int l = 0; l < nb_intervals; l++)
                    if ((Length[l] > right) &&
                        (Length[l] < param.length[sp][nb_ages - 1] + 10))
                        frq(f, region(f_num - 1) - 1, y, qtr, nb_ages - 1) +=
                            lf[l];
            }
        }
        littxt.close();
        ...
        cout << endl
             << "WARNING : Cannot read file " << filename.c_str() << endl;
    }
...
void CReadWrite::read_lf_EPO(
    CParam& param, string filename, const float startdate, const float enddate,
    const int sp) {  // data provided by IATTC, file changed only in header
...
void CReadWrite::read_lf_fine(
    ...
    if (littxt) {
        cout << endl
             << "Reading LF data from the file: " << endl
             << filename.c_str() << endl;
        int nb_regions, nb_fleets, nb_records, nb_intervals, l1, dl;
        ...
        // read the line
        // flexible to different file contents, namely to the presence of weight
        // data read number of size groups, first size and step
        littxt >> nb_intervals >> l1 >> dl;
        ...
        for (int l = 0; l < nb_intervals; l++) Length(l) = l1 + dl * (l + 0.5);
        for (int l = 0; l <= nb_intervals; l++) bin_bound(l) = l1 + dl * l;
        ...
        for (int n = 0; n < nb_records; n++) {
            ...
            if ((date >= startdate) && (date <= enddate) && qtr != 0) {
                ...
                for (int l = 0; l < nb_intervals; l++) {
                    if (bin_bound[l] >= param.length_bins(sp, a0) &&
                        bin_bound[l + 1] <= param.length_bins(sp, nb_ages)) {
                        if (lf[l] > 0) {
                            for (int a = a0; a < nb_ages; a++) {
                                ...
                                if (bin_bound[l] >= param.length_bins(sp, a) &&
                                    bin_bound[l] <=
                                        param.length_bins(sp, a + 1))
                                    left_in = true;
                                if (bin_bound[l + 1] >=
                                        param.length_bins(sp, a) &&
                                    bin_bound[l + 1] <=
                                        param.length_bins(sp, a + 1))
                                    right_in = true;
                                if (left_in && right_in) {
                                ...
                                if (!left_in && !right_in && !both_in &&
                                    !part_in)
                                    continue;

                                double deltal = 0;
                                if (left_in)
                                    deltal = param.length_bins(sp, a + 1) -
                                             bin_bound[l];
                                if (right_in)
                                    deltal = bin_bound[l + 1] -
                                             param.length_bins(sp, a);
                                if (both_in) deltal = dl;
                                if (part_in)
                                    deltal = param.length_bins(sp, a + 1) -
                                             param.length_bins(sp, a);
                                frq(f, reg - 1, y, qtr - 1, a) +=
                                    lf[l] * deltal / dl;
                            }
                        }
                ...
                // control check
                if (sqrt(pow(sum(lf) - sum(frq(f, reg - 1, y, qtr - 1)), 2)) >
                    0.001) {
                    cout << f << " " << reg << " " << y << " " << qtr << " "
                         << sum(lf) << " " << sum(frq(f, reg - 1, y, qtr - 1))
                         << endl;
                    cout << lf << " " << endl
                         << frq(f, reg - 1, y, qtr - 1) << endl
                         << endl;
                    exit(1);
                }

                //!!!TEST here: constrain the sample size by 1000 if S>1000 (see
                //!robust LF likelihood)
    ...
    cout << "Number of fine-resolution size distributions used from LF data: "
         << nbsd << endl;
    cout << endl;
}

void CReadWrite::read_frq_data(
    ...
    frq.allocate(
        0, nb_fishery - 1, 0, nb_region - 1, 0, nby - 1, 0, nbq - 1, a0,
        nb_ages - 1);
    frq.initialize();
    ...
    if (param.frq_like[sp]) {
        if (param.flag_twin) {
            read_pred_frq_data(
                param, param.file_frq_data[0], startdate, enddate, sp);
            return;
        }
        ...
        if (!param.use_lf_regstruc) {
            ...
            if (param.nb_frq_files == 2)
                read_lf_WCPO(
                    param, param.file_frq_data[1], startdate, enddate, sp);
            // read_lf_EPO(param,param.file_frq_data[1],startdate,enddate,sp);
            // //for skj only!
        } else {
            for (int f = 0; f < param.nb_frq_files; f++)
                read_lf_fine(
                    param, param.file_frq_data[f], startdate, enddate, sp);
        }
    }
    ...
    d4_array sum_frq(
        0, nb_fishery - 1, 0, nb_region - 1, 0, nbq, a0, nb_ages - 1);
    ...
    // we shall aggregate only those LF data which will be taken into account in
    // the likelihood, i.e. on the mask...
    for (int i = map.imin; i <= map.imax; i++) {
                                ...
                                for (int y = 0; y < nby; y++) {
                                    if (norm(frq(f, r, y, q))) {
                                        for (int a = a0; a < nb_ages; a++) {
                                            sum_frq(f, r, q, a) +=
                                                frq(f, r, y, q, a);
                                            sum_frq_fishery(f, q, a) +=
                                                frq(f, r, y, q, a);
                                            sum_frq_region(r, q, a) +=
                                                frq(f, r, y, q, a);
                                        }
                                    }
                                }
    ...
    // write the file with read observations aggregated over Seapodym's age
    // classes during simulation period
    ...
    if (ecritLF) {
        ...
        for (int reg = 0; reg < param.nb_region_sp_B[sp]; reg++)
            for (int f = 0; f < nb_fishery; f++)
                if (param.mask_fishery_sp[sp][f])
                    ecritLF << param.list_fishery_name[f] << "_"
                            << param.sp_name[sp] << "_region_"
                            << param.area_sp_B[sp][reg] << '\t';

        for (int f = 0; f < nb_fishery; f++)
            if (param.mask_fishery_sp[sp][f])
                ecritLF << "sum_" << param.list_fishery_name[f] << "_"
                        << param.sp_name[sp] << '\t';

        for (int reg = 0; reg < param.nb_region_sp_B[sp]; reg++)
            ecritLF << "sum_" << param.sp_name[sp] << "_region_"
                    << param.area_sp_B[sp][reg] << '\t';
        ...
        cout << endl
             << "WARNING : Cannot write file " << filename.c_str() << endl;
    }
}
...
void CReadWrite::write_frq_data(
    ...
    if (!FILEMODE) {
        ...
        if (ecritLF) {
            ecritLF << "year" << '\t' << "quarter" << '\t' << "fisheries"
                    << '\t' << "region" << '\t';
            for (int a = 0; a < nb_ages; a++) ecritLF << a << '\t';
            ecritLF << endl;
        } else
            cout << endl
                 << "WARNING : Cannot write file " << filename.c_str() << endl;
...
void CReadWrite::read_pred_frq_data(
    ...
    if (littxt) {
        ...
        int year, qtr, f, reg;
        int nb_records =
            (int)((enddate - startdate) * 4) * nb_fishery * nb_region;
        for (int n = 0; n < nb_records; n++) {
...
void CReadWrite::get_LF_qtr_data(
