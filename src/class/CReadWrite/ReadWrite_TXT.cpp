...
void CReadWrite::InitFRspeciesFileTxt(CParam& param)
{
        ...
        for (int sp=0; sp<nb_species;sp++)
        {
                ...
                if (ecritFR)
                {
                        ...
                        ecritFR << "young: from age "<<
param.age_autonomous[sp]<<" to age "<<param.age_mature[sp]<<'\t'<<'\t'<<'\t';
                        ecritFR<<endl;
                        ecritFR << "Adult: from age "<<param.age_mature[sp]<<"
to age " <<param.sp_nb_cohorts[sp] <<'\t'<<'\t'<<'\t'; ecritFR<<endl<<endl;
                        ...
                        for (int a=0; a<nb_reg; a++)
                          for (int n=0; n< nb_forage; n++)
                                  ecritFR << param.sp_name[sp] << " young: area
" << param.area_sp_B[sp][a] << " FR " << param.frg_name[n] << '\t';

                        for (int a=0; a<nb_reg; a++)
                          for (int n=0; n< nb_forage; n++)
                                  ecritFR << param.sp_name[sp] << " adult: area
" << param.area_sp_B[sp][a] << " FR " << param.frg_name[n] << '\t';
                        ...
                        cout<<endl<<"WARNING : Failed to initialize file
"<<FileSpFR[sp]<<endl;
        }
...
void CReadWrite::SaveFRspeciesFileTxt(CParam& param, CMatrices& mat, PMap& map,
int mois2, int yr2)
{
...
//		FR_area_average.initialize();
                ...
                if (ecritFR)
                {
                        ecritFR << yr2 <<'\t' << mois2 <<'\t';
//                        for (int n=0;n<nb_forage;n++){
// save.AverageByArea(map,FR_species,FR_area_average[n],cell_area,nb_reg); for
(int reg=0; reg< param.nb_region_sp_B[sp]; reg++) for (int n=0;n<nb_forage;n++)
...
//
                        ...
                        cout<<endl<<"WARNING : Cannot write file
"<<FileSpFR[sp]<<endl;
        }
}
*/
...
void CReadWrite::init_writing(CParam& param) {
    ...
    if (!param.use_mask_catch) mask_catch = 1e3;
}
...
void CReadWrite::InitSepodymFileTxt(CParam& param) {
    // cerr << "Force Exit[" << __FILE__ << ':' << __LINE__ << "]: function
    ...
    if (ecritSumDym) {
        ...
        ecritSumDym << "date" << '\t' << "tstep" << '\t' << "P in 10N-45N"
                    << '\t' << "P in 10S-10N" << '\t' << "P in 35S-10S" << '\t'
                    << "P total" << '\t';
        for (int n = 0; n < nb_forage; n++) {
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ecritSumDym << "B Larvae " << param.sp_name[sp] << '\t' << "B Juv "
                        << param.sp_name[sp] << '\t' << "B Rec "
                        << param.sp_name[sp] << '\t' << "B Young "
                        << param.sp_name[sp] << '\t' << "B Adult "
                        << param.sp_name[sp] << '\t' << "B Total"
                        << param.sp_name[sp] << '\t';
        }
                ...
                for (int sp = 0; sp < nb_species; sp++) {
                    if (param.mask_fishery_sp[sp][f]) {
                        ecritSumDym << "effort " << param.list_fishery_name[f]
                                    << '\t';
                        ecritSumDym << "obs C_" << param.sp_name[sp] << "_"
                                    << param.list_fishery_name[f] << '\t';
                        ecritSumDym << "pred C_" << param.sp_name[sp] << "_"
                                    << param.list_fishery_name[f] << '\t';
                        ecritSumDym << "obs CPUE_" << param.sp_name[sp] << "_"
                                    << param.list_fishery_name[f] << '\t';
                        ecritSumDym << "pred CPUE_" << param.sp_name[sp] << "_"
                                    << param.list_fishery_name[f] << '\t';
                    }
                }
            }
    ...
    // NOTE: cohorts larvae-recruits will be written in numbers, young-adult in
    // tonnes
    if (ecritSumQA) {
        for (int sp = 0; sp < nb_species; sp++)
        ...
        for (int r = 0; r < param.nb_region; r++)
            ecritSumQA << param.area[r]->lgmin << " " << param.area[r]->lgmax
                       << " " << param.area[r]->ltmin << " "
                       << param.area[r]->ltmax << endl;
        ...
        ecritSumQA << endl;
        for (int sp = 0; sp < nb_species; sp++)
            ecritSumQA << "young: from age " << param.age_autonomous[sp]
                       << " to age" << param.age_mature[sp]
                       << " (see maturity at age)" << '\t' << '\t' << '\t';
        ecritSumQA << endl;
        for (int sp = 0; sp < nb_species; sp++)
        ...
        ecritSumQA << endl;
        for (int sp = 0; sp < nb_species; sp++)
            ecritSumQA << "Adult: from age " << param.age_mature[sp]
                       << " to age " << param.sp_a0_adult[sp] << '\t' << '\t'
                       << '\t';
        ecritSumQA << endl;
        for (int sp = 0; sp < nb_species; sp++)
            ecritSumQA << "Total biomass: from age " << param.age_autonomous[sp]
                       << " to " << param.sp_nb_cohorts[sp] << '\t' << '\t'
                       << '\t';
        ...
        if (param.nb_region) {
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                if (nb_reg > 0) {
                    for (int a = 0; a < nb_reg; a++)
                        ecritSumQA << param.sp_name[sp] << " N Larvae region "
                                   << param.area_sp_B[sp][a] << '\t';
                    ecritSumQA << "Total N Larvae" << '\t';
                    for (int a = 0; a < nb_reg; a++)
                        ecritSumQA << param.sp_name[sp] << " N Juv. region "
                                   << param.area_sp_B[sp][a] << '\t';
                    ecritSumQA << "Total N Juv." << '\t';
                    for (int a = 0; a < nb_reg; a++)
                        ecritSumQA << param.sp_name[sp] << " N Rec. region "
                                   << param.area_sp_B[sp][a] << '\t';
                    ecritSumQA << "Total N. Rec." << '\t';
                    for (int a = 0; a < nb_reg; a++)
                        ecritSumQA << param.sp_name[sp] << " B Young region "
                                   << param.area_sp_B[sp][a] << '\t';
                    ecritSumQA << "Total B. Young" << '\t';
                    for (int a = 0; a < nb_reg; a++)
                        ecritSumQA << param.sp_name[sp] << " B Adult region "
                                   << param.area_sp_B[sp][a] << '\t';
                    ecritSumQA << "Total B Adult" << '\t';
                    for (int a = 0; a < nb_reg; a++)
                        ecritSumQA << param.sp_name[sp] << " B tot. region "
                                   << param.area_sp_B[sp][a] << '\t';
                    ecritSumQA << "Total B" << '\t';
                }
            }
    ...
    // one file by EEZ:
    // date, N.Larvae, N.Juveniles, N.Recruites, B.Young, B.Adult, B.tot, E[f],
    // Cobs[f], Cpred[f]
    if (param.nb_EEZ != 0) {
        for (int a = 0; a < param.nb_EEZ; a++) {
            string fileSumEEZ = param.strdir_output + "SumEEZ" + "_" +
                                param.EEZ_name[a] + ".txt";
            ...
            if (ecritSumEEZ) {
                ecritSumEEZ << "year" << '\t' << "month" << '\t' << "day"
                            << '\t';
                for (int sp = 0; sp < nb_species; sp++) {
                    // NOTE: cohorts larvae-recruits will be written in numbers,
                    // young-adult in tonnes

                    ecritSumEEZ << param.sp_name[sp] << " N Larvae "
                                << param.EEZ_name[a] << '\t';
                    ecritSumEEZ << param.sp_name[sp] << " N Juv. "
                                << param.EEZ_name[a] << '\t';
                    ecritSumEEZ << param.sp_name[sp] << " N Rec. "
                                << param.EEZ_name[a] << '\t';
                    ecritSumEEZ << param.sp_name[sp] << " B Young "
                                << param.EEZ_name[a] << '\t';
                    ecritSumEEZ << param.sp_name[sp] << " B Adult "
                                << param.EEZ_name[a] << '\t';
                    ecritSumEEZ << param.sp_name[sp] << " B tot. "
                                << param.EEZ_name[a] << '\t';
                    if (nb_fishery && !param.flag_no_fishing) {
                        for (int f = 0; f < nb_fishery; f++) {
                            if (param.mask_fishery_sp[sp][f]) {
                                ...
                                ecritSumEEZ << "effort "
                                            << param.list_fishery_name[f]
                                            << '\t';
                                ecritSumEEZ << "obs C_" << param.sp_name[sp]
                                            << "_" << param.list_fishery_name[f]
                                            << '\t';
                                ecritSumEEZ << "pred C_" << param.sp_name[sp]
                                            << "_" << param.list_fishery_name[f]
                                            << '\t';
                                ecritSumEEZ << "obs CPUE_" << param.sp_name[sp]
                                            << "_" << param.list_fishery_name[f]
                                            << '\t';
                                ecritSumEEZ << "std obs CPUE_"
                                            << param.sp_name[sp] << "_"
                                            << param.list_fishery_name[f]
                                            << '\t';
                                ecritSumEEZ << "pred CPUE_" << param.sp_name[sp]
                                            << "_" << param.list_fishery_name[f]
                                            << '\t';
                            }
                        }
                ...
                cout << endl
                     << "WARNING : Cannot write file " << fileSumEEZ << endl;
        }
    }
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        string fileMeanVar =
            param.strdir_output + param.sp_name[sp] + "_MeanVar.txt";
        ...
        if (ecritMeanVar) {
            ...
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                for (int a = 0; a < nb_ages; a++)
                    ecritMeanVar << param.sp_name[sp] << " mortality-at-age "
                                 << a << '\t';
                for (int a = 0; a < nb_ages; a++)
                    ecritMeanVar << param.sp_name[sp] << " speed-at-age " << a
                                 << '\t';
                for (int a = 0; a < nb_ages; a++)
                    ecritMeanVar << param.sp_name[sp] << " diffusion-at-age "
                                 << a << '\t';
                for (int a = 0; a < nb_ages; a++)
                    ecritMeanVar << param.sp_name[sp] << " temperature-at-age "
                                 << a << '\t';
            }
            ecritMeanVar << endl;
        } else
            cout << endl
                 << "WARNING : Cannot write file " << fileMeanVar << endl;
    }
    ...
    if (!param.flag_no_fishing) {
        if (nb_fishery) {
            // Generate the name of the files
            for (int sp = 0; sp < nb_species; sp++) {
                dymFileSpCorr.push_back(
                    param.strdir_output + param.sp_name[sp] +
                    "_Spatial_Corr.txt");
            }
        }
            ...
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                if (ecritcor) {
                    ...
                    for (int f = 0; f < nb_fishery; f++) {
                        if (param.mask_fishery_sp[sp][f])
                            ecritcor << "n" << '\t' << "r "
                                     << param.list_fishery_name[f] << "_"
                                     << param.sp_name[sp] << '\t' << "prob"
                                     << '\t' << "cpue_r "
                                     << param.list_fishery_name[f] << "_"
                                     << param.sp_name[sp] << '\t' << "prob"
                                     << '\t';
                    ...
                    cout << endl
                         << "WARNING : Cannot write file " << dymFileSpCorr[sp]
                         << endl;
                }
            }
        }
        ...
        if (nb_fishery > 0) {
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                if (ecritQtr_N) {
                    ...
                    /*
                                                    ecritQtr_N <<"Quarter "<<
                       '\t'<< "Year "<< endl; ecritQtr_N << "length" <<'\t' ;
                                                    for (int reg=0; reg<
                       param.nb_region_sp_B[sp]; reg++){ for (int f=0;
                       f<nb_fishery; f++){ if (param.mask_fishery_sp[sp][f])
                    ...
                    */
                    ...
                    cout << endl
                         << "WARNING : Cannot write file " << dymFileSpLF[sp]
                         << endl;
                }
            }
        }
    ...
    if (param.nb_region || param.nb_EEZ) {
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int a = 0; a < param.sp_nb_cohorts[sp]; a++) {
                ...
                ostr << a;
                string fileJreg = param.strdir_output + param.sp_name[sp] +
                                  "_FluxesRegion_age" + ostr.str() + ".txt";
                ofstream ecritJreg;
                ecritJreg.open(fileJreg.c_str(), ios::out);
                if (ecritJreg) {
                    ecritJreg << "Regional fluxes predicted for "
                              << param.sp_name[sp] << endl;
                    ecritJreg << "Mean age(months) " << mean_age_cohort << endl;
                    ecritJreg << "Mean length(cm) " << param.length(sp, a)
                              << endl;
                    ecritJreg << "Mean weight(kg) " << param.weight(sp, a)
                              << endl;
                    ...
                    cout << endl
                         << "WARNING : Cannot write file " << fileJreg << endl;
                if (a < param.sp_nb_cohorts[sp] - 1)
                    mean_age_cohort = mean_age_cohort +
                                      .5 * param.sp_unit_cohort[sp][a] +
                                      .5 * param.sp_unit_cohort[sp][a + 1];
            }
        }
    }
}

void CReadWrite::SaveFluxesCohortsFileTxt(
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            for (int a = 0; a < param.sp_nb_cohorts[sp]; a++) {
                ...
                if (ecritJreg) {
                        ...
                        if (nbreg > 0) {
                            for (int ri = 0; ri < nb_reg; ri++) {
                                ...
                                for (int rj = 0; rj < nb_reg; rj++) {
                                    ...
                                    ecritJreg
                                        << mat.fluxes_region(a, reg_i, reg_j)
                                        << '\t';
                                }
                                ecritJreg << endl;
                            }
                    ...
                    cout << endl
                         << "WARNING : Cannot append to the file " << fileJreg
                         << endl;
            }
        }
    }
...
///////////////////////////////////////////////////////////////////////////////
void CReadWrite::SaveSepodymFileTxt(
    CParam& param, CMatrices& mat, PMap& map, dvector sumP, DVECTOR& sumF,
    DVECTOR& sumFprime, DVECTOR& sumF_area_pred, DVECTOR& sumF_required_by_sp,
    ...
    if (ecritSumDym) {
        ...
        // writing the data
        ecritSumDym << t_total << '\t' << sumP[0] << '\t' << sumP[1] << '\t'
                    << sumP[2] << '\t' << sumP[3] << '\t';
        for (int n = 0; n < nb_forage; n++) {
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ecritSumDym << mat.sum_B_larvae[sp] << '\t' << mat.sum_B_juv[sp]
                        << '\t' << mat.sum_B_recruit[sp] << '\t'
                        << mat.sum_B_young[sp] << '\t' << mat.sum_B_adult[sp]
            ...
            for (int f = 0; f < nb_fishery; f++) {
                for (int sp = 0; sp < nb_species; sp++) {
                    if (param.mask_fishery_sp[sp][f]) {
                        ...
                        ecritSumDym << total << '\t';
                        // cout <<"total effort by fishery "<<f<< " " << total
                        // << endl;
                        ...
                        if (N > 0) cpue = total / N;
                        // cout << "\t PREDICTED CPUE: " << total << " MEAN: "
                        // << total/(N+1) << " " << cpue << endl;
                        ecritSumDym << cpue << '\t';
                        // ecritSumDym << total << '\t';
                        // end test
    ...
    if (ecritSumQA) {
        ...
        /*
           ...
           = 1.852*param.deltaX*1.852*param.deltaY; for (int
           j=map.jmin;j<=map.jmax;j++) cell_area[j] = cell_area /
           mat.lat_correction[j];
        */
        // int nbt_yr = (int) (365.25/param.deltaT);
        const int nbt = param.get_nbt();  //(int) (nbt_yr*(param.save_last_yr -
                                          //param.save_first_yr + 1.0/nbt_yr) );
        ...
        if (param.nb_region) {
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                if (nb_reg > 0) {
                    ...
                    save.SumByArea(
                        map, mask_catch, mat.larvae[sp], sum_area, cell_area,
                        nb_reg, nbt);
                    for (int z = 0; z < nb_reg + 1; z++)
                        ecritSumQA << sum_area[z] << '\t';

                    save.SumByArea(
                        map, mask_catch, mat.juvenile[sp], sum_area, cell_area,
                        nb_reg, nbt);
                    for (int z = 0; z < nb_reg + 1; z++)
                        ecritSumQA << sum_area[z] << '\t';

                    save.SumByArea(
                        map, mask_catch, mat.recruit[sp], sum_area, cell_area,
                        nb_reg, nbt);
                    for (int z = 0; z < nb_reg + 1; z++)
                        ecritSumQA << sum_area[z] << '\t';

                    save.SumByArea(
                        map, mask_catch, mat.young[sp], sum_area, cell_area,
                        nb_reg, nbt);
                    for (int z = 0; z < nb_reg + 1; z++)
                        ecritSumQA << sum_area[z] << '\t';

                    save.SumByArea(
                        map, mask_catch, mat.adult[sp], sum_area, cell_area,
                        nb_reg, nbt);
                    for (int z = 0; z < nb_reg + 1; z++)
                        ecritSumQA << sum_area[z] << '\t';

                    save.SumByArea(
                        map, mask_catch, mat.total_pop[sp], sum_area, cell_area,
                        nb_reg, nbt);
                    for (int z = 0; z < nb_reg + 1; z++)
                        ecritSumQA << sum_area[z] << '\t';
                }
        ...
        for (int a = 0; a < param.nb_EEZ; a++) {
            ...
            if (ecritSumEEZ) {
                ...
                for (int sp = 0; sp < nb_species; sp++) {
                    // NOTE: cohorts larvae-recruits will be written in numbers,
                    // young-adult in tonnes
                    sumEEZ = save.SumByEEZ(
                        map, EEZ_ID, mat.larvae[sp], cell_area, nlon, nlat);
                    ecritSumEEZ << sumEEZ << '\t';
                    sumEEZ = save.SumByEEZ(
                        map, EEZ_ID, mat.juvenile[sp], cell_area, nlon, nlat);
                    ecritSumEEZ << sumEEZ << '\t';
                    sumEEZ = save.SumByEEZ(
                        map, EEZ_ID, mat.recruit[sp], cell_area, nlon, nlat);
                    ecritSumEEZ << sumEEZ << '\t';
                    sumEEZ = save.SumByEEZ(
                        map, EEZ_ID, mat.young[sp], cell_area, nlon, nlat);
                    ecritSumEEZ << sumEEZ << '\t';
                    sumEEZ = save.SumByEEZ(
                        map, EEZ_ID, mat.adult[sp], cell_area, nlon, nlat);
                    ecritSumEEZ << sumEEZ << '\t';
                    sumEEZ = save.SumByEEZ(
                        map, EEZ_ID, mat.total_pop[sp], cell_area, nlon, nlat);
                    ecritSumEEZ << sumEEZ << '\t';
                    ...
                    if (nb_fishery && !param.flag_no_fishing) {
                        ...
                        for (int f = 0; f < nb_fishery; f++) {
                            if (param.mask_fishery_sp[sp][f]) {
                                Nb_obs = save.NobsByEEZ(
                                    map, EEZ_ID, mat.effort[f], nlon, nlat);
                                ecritSumEEZ << Nb_obs << '\t';
                                sumEEZ = save.SumByEEZ(
                                    map, EEZ_ID, mat.effort[f], no_area, nlon,
                                    nlat);
                                ecritSumEEZ << sumEEZ << '\t';
                                sumEEZ = save.SumByEEZ(
                                    map, EEZ_ID, mat.catch_obs(sp, k), no_area,
                                    nlon, nlat);
                                ecritSumEEZ << sumEEZ << '\t';
                                sumEEZ = save.SumByEEZ(
                                    map, EEZ_ID, mat.catch_est(sp, k), no_area,
                                    nlon, nlat);
                                ecritSumEEZ << sumEEZ << '\t';
                                sumEEZ = save.SumByEEZ(
                                    map, EEZ_ID, mat.catch_obs(sp, k),
                                    mat.effort(f), nlon, nlat);
                                ecritSumEEZ << sumEEZ << '\t';

                                double mean_CPUE = 0;
                                if (Nb_obs > 0) mean_CPUE = sumEEZ / Nb_obs;
                                sumEEZ = save.StdCPUEByEEZ(
                                    map, EEZ_ID, mat.catch_obs(sp, k),
                                    mat.effort(f), mean_CPUE, Nb_obs, nlon,
                                    nlat);
                                ecritSumEEZ << sumEEZ << '\t';

                                sumEEZ = save.SumByEEZ(
                                    map, EEZ_ID, mat.catch_est(sp, k),
                                    mat.effort(f), nlon, nlat);
                                ecritSumEEZ << sumEEZ << '\t';
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            if (ecritcor) {
                ...
                for (int f = 0; f < nb_fishery; f++)
                    if (param.mask_fishery_sp[sp][f]) {
                        fonction.corcatch(
                            mat.catch_est[sp][k], mat.catch_obs[sp][k],
                            map.imin, map.imax, map.jinf, map.jsup, nn,
                            cor_catch, z_catch, prob_catch, map.carte, 0);
                        fonction.corcpue(
                            mat.catch_est[sp][k], mat.catch_obs[sp][k],
                            mat.effort[f], map.imin, map.imax, map.jinf,
                            map.jsup, nn, cor_cpue, z_cpue, prob_cpue,
                            map.carte, 0);
                        ecritcor << nn << '\t' << cor_catch << '\t'
                                 << prob_catch << '\t' << cor_cpue << '\t'
                                 << prob_cpue << '\t';
                        k++;
                    }
    ...
    if (!param.flag_no_fishing) {
        ...
        // exit(1);
        //  si nouveau trimestre ENREGISTRE a condition qu'il y ait eu un
        //  passage (ntq) au minimum
        // if ( (nb_fishery>0) && (qtr2  != qtr1) && (ntq>0) )
        // cout << param.nb_region_sp_B[0] << endl; exit(1);
        if (sum(param.mask_fishery_sp[0]) != 0) {
            year0_fishing = (int)(param.ndatini / 10000);
        }
        int year = yr2 - year0_fishing;  //(int)param.save_first_yr;
        if ((nb_fishery > 0) &&
            (mois2 == 3 || mois2 == 6 || mois2 == 9 || mois2 == 12)) {
            for (int sp = 0; sp < nb_species; sp++) {
                const int a0 = param.sp_a0_adult[sp];
                const int nb_ages = param.sp_nb_cohorts[sp];
                ...
                if (ecritQtr_N) {
                    ...
                    for (int f = 0; f < nb_fishery; f++) {
                        if (param.mask_fishery_sp[sp][f]) {
                            for (int reg = 0; reg < param.nb_region_sp_B[sp];
                                 reg++) {
                                double sumpred = 0.0;
                                double sumobs = 0.0;
                                for (int a = a0; a < nb_ages; a++) {
                                    sumpred +=
                                        mat.C_N_sp_age_fishery_qtr[sp][a][reg]
                                                                  [k];
                                    if (!param.nb_yr_forecast)
                                        sumobs +=
                                            frq(f, reg, year, mois2 / 3 - 1, a);
                                }
                                ...
                                if (sumpred > 0) {
                                    ...
                                    for (int a = a0; a < nb_ages; a++)
                                        ecritQtr_N
                                            << mat.C_N_sp_age_fishery_qtr[sp][a]
                                                                         [reg]
                                                                         [k]
                                            << '\t';
                                    ecritQtr_N << endl;
                    ...
                    /*
                                                    //ecritQtr_N <<"Quarter "<<
                       qtr1 <<'\t'<< "Year "<< yr2 <<endl; for (int
                       a=0;a<param.sp_nb_age_class_ad[sp];a++)
                                                    {
                                                            ecritQtr_N
                       <<param.length[sp][a]<<'\t'; for (int reg=0; reg<
                       param.nb_region_sp_B[sp]; reg++)
                                                            {
                                                                    for (int
                       k=0;k<param.nb_fishery_by_sp[sp]; k++)
                                                                    {
                                                                            ecritQtr_N
    ...
    if (!param.flag_no_fishing) {
        ...
        // sum for each of four quarter over all the series + sum of the four
        // quarters
                ...
                for (int a = a0; a < nb_ages; a++) {
                    ...
                    for (int r = 0; r < nb_region_sp_B; r++) {
                        ...
                        for (int k = 0; k < nb_fishery_by_sp; k++) {
                            // somme pour la climatologie trimestrielle par
                            // region et pecherie
                            mat.Sum_C_N_sp_age_fishery_area[sp][a][qtr1 - 1]
                                                           [reg][k] +=
                                mat.C_N_sp_age_fishery_qtr[sp][a][reg][k];
                            // somme pour la climatologie trimestrielle par
                            // pecherie
                            ...
                            mat.Sum_C_N_sp_age_fishery_area[sp][a][5 - 1][reg]
                                                           [k] +=
                                mat.C_N_sp_age_fishery_qtr[sp][a][reg][k];
                            // somme totale par pecherie
                            mat.Sum_C_N_sp_age_fishery_area[sp][a][5 - 1]
                                                           [nb_region_sp_B]
                                                           [k] +=
                                mat.C_N_sp_age_fishery_qtr[sp][a][reg][k];
    ...
    if (!param.flag_no_fishing) {
        ...
        if ((nb_fishery > 0) &&
            ...
            // Generate the name of the files
            for (int sp = 0; sp < nb_species; sp++) {
                dymFileSumSpLF.push_back(
                    param.strdir_output + param.sp_name[sp] + "_LF_Q_sum.txt");
            }
        }
            ...
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                if (ecritQtr_S) {
                    ...
                    for (int reg = 0; reg < param.nb_region_sp_B[sp]; reg++) {
                        for (int f = 0; f < nb_fishery; f++) {
                            if (param.mask_fishery_sp[sp][f])
                                ecritQtr_S << param.list_fishery_name[f] << "_"
                    ...
                    for (int f = 0; f < nb_fishery; f++) {
                        if (param.mask_fishery_sp[sp][f])
                            ecritQtr_S << "sum_" << param.list_fishery_name[f]
                                       << "_" << param.sp_name[sp] << '\t';
                    }

                    for (int reg = 0; reg < param.nb_region_sp_B[sp]; reg++) {
                        ecritQtr_S << "sum_" << param.sp_name[sp] << "_region_"
                                   << param.area_sp_B[sp][reg] << '\t';
                    }
                    ecritQtr_S << endl;
                    ecritQtr_S.close();
                    ...
                    cout << endl
                         << "WARNING : Cannot write file " << dymFileSumSpLF[sp]
                         << endl;
                }
            }
        }
        //////////////////////////////
        // ECRITURE des sommes des captures par age, region pecherie pour les 4
        // trimestres et le total des 4
        double totalregion;
                ...
                if (ecritQtr_S) {
                    for (int q = 0; q < 5; q++) {
                        ...
                        for (int a = a0; a < nb_ages; a++) {
                            ...
                            const int nb_fishery_by_sp =
                                param.nb_fishery_by_sp[sp];
                            const int nb_region_sp_B = param.nb_region_sp_B[sp];
                            for (int r = 0; r < nb_region_sp_B; r++) {
                                int reg = param.area_sp_B[sp][r] - 1;
                                for (int k = 0; k < nb_fishery_by_sp; k++) {
                                    // somme pour la climatologie trimestrielle
                                    // par region et pecherie
                                    ecritQtr_S
                                        << mat.Sum_C_N_sp_age_fishery_area
                                               [sp][a][q][reg][k]
                                        << '\t';
                                }
                            }
                            for (int k = 0; k < nb_fishery_by_sp; k++) {
                                // somme pour la climatologie trimestrielle par
                                // pecherie
                                ecritQtr_S << mat.Sum_C_N_sp_age_fishery_area
                                                  [sp][a][q][nb_region_sp_B][k]
                                           << '\t';
                                totalpecherie +=
                                    mat.Sum_C_N_sp_age_fishery_area
                                        [sp][a][q][nb_region_sp_B][k];
