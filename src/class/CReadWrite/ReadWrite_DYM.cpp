...
void CReadWrite::rbin_headpar(
    string file_in, int &nlong, int &nlat, int &nlevel) {
    // cerr << "Force Exit[" << __FILE__ << ':' << __LINE__ << "]: function
    // hasn't been fixed yet\n";
    ifstream litbin(file_in.c_str(), ios::binary | ios::in);
    if (!litbin) {
        cerr << "Error[" << __FILE__ << ':' << __LINE__
             << "]: Unable to read file \"" << file_in << "\"\n";
        exit(1);
    }

    const int nbytes = 4;

    // for new dym format, skip the first 4 parameters (Idformat, Idfunc,
    // Minval, Maxval)
    ...
    string file_io, double minvalstep, double maxvalstep)  // Gael Nov04
{
    ...
    if (!rwbin) {
        cerr << "Error[" << __FILE__ << ':' << __LINE__
             << "]: Unable to read-write file \"" << file_io << "\"\n";
        exit(1);
    }
...
//////////////////////////////////////////////////////////////////////
void CReadWrite::rbin_mat2d(
    string file_in, PMap &map, DMATRIX &mat2d, int nlat, int nlong,
    int nbytetoskip) {
    ifstream litbin(file_in.c_str(), ios::binary | ios::in);
    ...
    for (int i = 0; i < nlat; i++) {
        for (int j = 0; j < nlong; j++) {
            ...
            if (map.carte(i, j)) mat2d[i][j] = buf;
        }
    }
...
/*
void CReadWrite::rbin_input2d(string file_in, const imatrix& carte, DMATRIX&
mat2d, int nbi, int nbj, int nbytetoskip)
{
        ifstream litbin(file_in.c_str(), ios::binary | ios::in);

        if (!litbin)
        {
                cerr << "Error[" << __FILE__ << ':' << __LINE__ << "]: Unable to
read file \"" << file_in << "\"\n"; exit(1);
        }
...
////////////// ecriture matrice
void CReadWrite::wbin_mat2d(
    string file_out, const DMATRIX &mat2d, int nlat, int nlong, bool FILEMODE) {
    ofstream ecritbin;
    if (FILEMODE)
...
/////////////////////////////////////////////////////////////
void CReadWrite::wbin_transpomat2d(
    string file_out, const DMATRIX &mat2d, int nlong, int nlat, bool FILEMODE) {
    // cerr << "Force Exit[" << __FILE__ << ':' << __LINE__ << "]: function
    // hasn't been fixed yet\n";
    ////////////// ecriture matrice
    ofstream ecritbin;
...
////////////////////////////////////
void CReadWrite::InitSepodymFileDym(
    ...
    /*	int nb_var_F= 1;
            ...
            for (int n=0; n<nb_forage;n++){
                    for (int var=0; var<nb_var_F;var++){
                            ...
                            // Call the function to write the header of each
       file wbin_header(dymFileFPred[var+(nb_var_F*n)], param.idformat,
       param.idfunc, minval, maxval, param.nlong, param.nlat, nb_mo, zlevel[0],
       zlevel[nb_mo -1], mat.xlon, mat.ylat, zlevel, msksp);
                    }
            }
    */
    ...
    if (nb_species) {
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int var = 0; var < nb_var; var++) {
                dymFileSpPred.push_back(
                    param.strdir_output + param.sp_name[sp] + "_" +
                    var_name[var] + ".dym");
                wbin_header(
                    dymFileSpPred[var], param.idformat, param.idfunc, minval,
                    maxval, param.nlong, param.nlat, nb_mo, zlevel[0],
                    zlevel[nb_mo - 1], mat.xlon, mat.ylat, zlevel, msksp);
            }
        }
    }
    ...
    if (nb_fishery && !param.flag_no_fishing) {
        // Generate the name of the files
        for (int sp = 0; sp < nb_species; sp++) {
            dymFileSpC.push_back(
                param.strdir_output + param.sp_name[sp] + "_Cobs.dym");
            dymFileSpC.push_back(
                param.strdir_output + param.sp_name[sp] + "_Cpred.dym");
            nfiles += 2;
            if (param.write_all_fisheries_dym) {
                int k = 0;
                for (int f = 0; f < nb_fishery; f++) {
                    if (param.mask_fishery_sp[sp][f]) {
                        // Inna 09.09: need to write prediction and observation
                        // file by fishery to be used for validation
                        dymFileSpC.push_back(
                            param.strdir_output + param.sp_name[sp] + "_Cobs" +
                            param.list_fishery_name[f] + ".dym");
                        dymFileSpC.push_back(
                            param.strdir_output + param.sp_name[sp] + "_Cpred" +
                            param.list_fishery_name[f] + ".dym");
                        nfiles += 2;
                        // dymFileSpC.push_back(param.strdir_output +
                        // param.sp_name[sp]+"_CPUEobs" +
                        // param.list_fishery_name[f] +".dym");
                        // dymFileSpC.push_back(param.strdir_output +
                        // param.sp_name[sp]+"_CPUEpred" +
                        // param.list_fishery_name[f] +".dym"); nfiles += 4;
                        k++;
                    }
                }
            }
    ...
    for (int nf = 0; nf < nfiles; nf++) {
        wbin_header(
            dymFileSpC[nf], param.idformat, param.idfunc, minval, maxval,
...
void CReadWrite::SaveSepodymFileDym(CParam &param, PMap &map, CMatrices &mat) {
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.Hs, nlon, nlat);
        ixv++;
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.Ha, nlon, nlat);
        ixv++;
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.advection_x, nlon, nlat);
        ixv++;
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.advection_y, nlon, nlat);
        ixv++;
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.speed, nlon, nlat);
        ixv++;
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.diffusion_y, nlon, nlat);
        ixv++;
        // SaveDymFile(map, mat, dymFileSpPred[ixv], mask_catch, nlon, nlat);
        // ixv++;
        //	SaveDymFile(map, mat, dymFileSpPred[ixv], EFR, nlon, nlat);
        //ixv++;

        // larvae
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.larvae[sp], nlon, nlat);
        ixv++;

        // juvenile
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.juvenile[sp], nlon, nlat);
        ixv++;

        // young
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.young[sp], nlon, nlat);
        ixv++;

        // recruit
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.recruit[sp], nlon, nlat);
        ixv++;
        //		SaveDymFile(map, mat, dymFileSpPred[ixv],  mat.Hj, nlon,
        //nlat); ixv++;

        // adults
        SaveDymFile(map, mat, dymFileSpPred[ixv], mat.adult[sp], nlon, nlat);
        ixv++;

        // total population
        SaveDymFile(
            map, mat, dymFileSpPred[ixv], mat.total_pop[sp], nlon, nlat);
        ixv++;
        ...
        if (nb_fishery && !param.flag_no_fishing) {
            int ixv = 0;
            SaveDymFile(
                map, mat, dymFileSpC[ixv], mat.total_obs_catch[sp], nlon, nlat);
            ixv++;
            SaveDymFile(
                map, mat, dymFileSpC[ixv], mat.total_pred_catch[sp], nlon,
                nlat);
            ixv++;
            if (param.write_all_fisheries_dym) {
                int k = 0;
                ...
                for (int f = 0; f < nb_fishery; f++) {
                    if (param.mask_fishery_sp[sp][f]) {
                        // Catch by fishery
                        SaveDymFile(
                            map, mat, dymFileSpC[ixv], mat.catch_obs[sp][k],
                            nlon, nlat);
                        ixv++;
                        SaveDymFile(
                            map, mat, dymFileSpC[ixv], mat.catch_est[sp][k],
                            nlon, nlat);
                        ixv++;
                        // SaveDymFile(map, mat, dymFileSpC[ixv], mat.effort[f],
                        // nlon, nlat); ixv++;
                        /*
                                                                        //CPUE
                        by fishery cpue_obs.initialize(); cpue_obs = -1;
                                                                        cpue_est.initialize();
                                                                        for (int
                        i=map.imin; i <= map.imax; i++){ for (int j=map.jinf[i]
                        ; j<=map.jsup[i] ; j++){ if (map.carte[i][j] &&
                        mat.effort[f][i][j]){ double pred =
                        mat.catch_est(sp,k,i,j); double obs =
                        mat.catch_obs(sp,k,i,j); double like = 0.0; if (pred>0){
                                        like = pred - obs*log(pred) +
                        gammln(obs+1.0);
                                }
                        //	dvector like_lf =
                        LFlike(mat.LF_qtr_obs(sp),mat.C_N_sp_age_fishery[0],param.sp_a0_adult[sp],
                        //
                        param.sp_cohorts(sp),param.nb_region_sp_B[0],k,param.frq_like_param(0));
                                                                ...
                                                                SaveDymFile(map,
                        mat, dymFileSpC[ixv],  cpue_obs,  nlon, nlat); ixv++;
                                                                SaveDymFile(map,
                        mat, dymFileSpC[ixv],  cpue_est,  nlon, nlat); ixv++;
                        */
                        k++;
                    }
                }
...
void CReadWrite::SaveDymFile(
    PMap &map, CMatrices &mat, string file, const dmatrix &data, const int nlon,
    const int nlat) {
    bool FileMode = true;
    ...
    // if (file=="run4804x30dx2deg/output/simout/Young_Habitat.dym") cout <<
    // minval << " " << maxval << endl;
