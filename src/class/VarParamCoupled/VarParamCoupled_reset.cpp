...
dvariable VarParamCoupled::reset(dvar_vector x) {
    ...
    if (doc.get("/Mp_mean_max/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsMp_mean_max[i] =
                boundp(x[idx], Mp_mean_max_min, Mp_mean_max_max, penalty);
            // large_pen(dvarsMp_mean_max[i],Mp_mean_max_min,
            // Mp_mean_max_max,penalty,0.01); Hint: last constant - critical
            // distance to the boundary for linear scaling, i.e. in order to get
            // explicite distance multiply this constant by abs(xmax-xmin)
    ...
    if (doc.get("/Mp_mean_exp/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsMp_mean_exp[i] =
                boundp(x[idx], Mp_mean_exp_min, Mp_mean_exp_max, penalty);
            // large_pen(dvarsMp_mean_exp[i],Mp_mean_exp_min,
            // Mp_mean_exp_max,penalty,0.01);
    ...
    if (doc.get("/Ms_mean_max/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsMs_mean_max[i] =
                boundp(x[idx], Ms_mean_max_min, Ms_mean_max_max, penalty);
            // large_pen(dvarsMs_mean_max[i],Ms_mean_max_min,
            // Ms_mean_max_max,penalty,0.01);
    ...
    if (doc.get("/Ms_mean_slope/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsMs_mean_slope[i] =
                boundp(x[idx], Ms_mean_slope_min, Ms_mean_slope_max, penalty);
    ...
    if (doc.get("/M_mean_range/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsM_mean_range[i] =
                boundp(x[idx], M_mean_range_min, M_mean_range_max, penalty);
    ...
    if (doc.get("/a_sst_spawning/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsA_sst_spawning[i] =
                boundp(x[idx], a_sst_spawning_min, a_sst_spawning_max, penalty);
    ...
    if (doc.get("/b_sst_spawning/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsB_sst_spawning[i] =
                boundp(x[idx], b_sst_spawning_min, b_sst_spawning_max, penalty);
    ...
    if (doc.get("/a_sst_larvae/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsA_sst_larvae[i] =
                boundp(x[idx], a_sst_larvae_min, a_sst_larvae_max, penalty);
    ...
    if (doc.get("/b_sst_larvae/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsB_sst_larvae[i] =
                boundp(x[idx], b_sst_larvae_min, b_sst_larvae_max, penalty);
    ...
    if (doc.get("/alpha_hsp_prey/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsAlpha_hsp_prey[i] =
                boundp(x[idx], alpha_hsp_prey_min, alpha_hsp_prey_max, penalty);
    ...
    if (doc.get("/alpha_hsp_predator/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsAlpha_hsp_predator[i] = boundp(
                x[idx], alpha_hsp_predator_min, alpha_hsp_predator_max,
                penalty);
    ...
    if (doc.get("/beta_hsp_predator/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsBeta_hsp_predator[i] = boundp(
                x[idx], beta_hsp_predator_min, beta_hsp_predator_max, penalty);
    ...
    if (doc.get("/a_sst_habitat/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsA_sst_habitat[i] =
                boundp(x[idx], a_sst_habitat_min, a_sst_habitat_max, penalty);
    ...
    if (doc.get("/b_sst_habitat/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsB_sst_habitat[i] =
                boundp(x[idx], b_sst_habitat_min, b_sst_habitat_max, penalty);
    ...
    if (doc.get("/T_age_size_slope/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsT_age_size_slope[i] = boundp(
                x[idx], T_age_size_slope_min, T_age_size_slope_max, penalty);
    ...
    for (int n = 0; n < 3; n++) {
        ...
        if (doc.get(s, "use") == "true") {
            for (int i = 0; i < nb_species; i++) {
                dvarsThermal_func_delta[n][i] = boundp(
                    x[idx], thermal_func_delta_min[n],
    ...
    if (doc.get("/a_oxy_habitat/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsA_oxy_habitat[i] =
                boundp(x[idx], a_oxy_habitat_min, a_oxy_habitat_max, penalty);
    ...
    if (doc.get("/b_oxy_habitat/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsB_oxy_habitat[i] =
                boundp(x[idx], b_oxy_habitat_min, b_oxy_habitat_max, penalty);
    ...
    for (int n = 0; n < nb_forage; n++) {
        if (doc.get(
                string("/eF_habitat/") + frg_name[n] + "/variable", "use") ==
            "true") {
            for (int i = 0; i < nb_species; i++) {
                dvarsEF_habitat[n][i] = boundp(
                    x[idx], eF_habitat_min[n], eF_habitat_max[n], penalty);
    ...
    if (doc.get("/hp_cannibalism/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsHp_cannibalism[i] =
                boundp(x[idx], hp_cannibalism_min, hp_cannibalism_max, penalty);
    ...
    if (doc.get("/sigma_species/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsSigma_species[i] =
                boundp(x[idx], sigma_species_min, sigma_species_max, penalty);
    ...
    if (doc.get("/MSS_species/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsMSS_species[i] =
                boundp(x[idx], MSS_species_min, MSS_species_max, penalty);
    ...
    if (doc.get("/MSS_size_slope/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsMSS_size_slope[i] =
                boundp(x[idx], MSS_size_slope_min, MSS_size_slope_max, penalty);
    ...
    if (doc.get("/c_diff_fish/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsC_diff_fish[i] =
                boundp(x[idx], c_diff_fish_min, c_diff_fish_max, penalty);
    ...
    if (doc.get("/nb_recruitment/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsNb_recruitment[i] =
                boundp(x[idx], nb_recruitment_min, nb_recruitment_max, penalty);
    ...
    if (doc.get("/a_adults_spawning/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsA_adults_spawning[i] = boundp(
                x[idx], a_adults_spawning_min, a_adults_spawning_max, penalty);
    ...
    if (doc.get("/spawning_season_peak/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsSpawning_season_peak[i] = boundp(
                x[idx], spawning_season_peak_min, spawning_season_peak_max,
                penalty);
    ...
    if (doc.get("/spawning_season_start/variable", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            dvarsSpawning_season_start[i] = boundp(
                x[idx], spawning_season_start_min, spawning_season_start_max,
                penalty);
                ...
                if (mask_fishery_sp[i][f]) {
                    if (doc.get(
                            "/q_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "use") == "true") {
                        dvarsQ_sp_fishery[i][ifx] = boundp(
                            x[idx], q_sp_fishery_min[i][ifx],
                            q_sp_fishery_max[i][ifx], penalty);
                ...
                if (mask_fishery_sp[i][f]) {
                    if (doc.get(
                            "/s_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "use") == "true") {
                        dvarsSslope_sp_fishery[i][ifx] = boundp(
                            x[idx], s_slope_sp_fishery_min[i][ifx],
                            s_slope_sp_fishery_max[i][ifx], penalty);
                        // large_pen(dvarsSslope_sp_fishery[i][ifx],s_slope_sp_fishery_min[i][ifx],s_slope_sp_fishery_max[i][ifx],penalty,0.01);

                        s_slope_sp_fishery[i][ifx] =
                            value(dvarsSslope_sp_fishery[i][ifx]);
                    ...
                    if (s_func_type[f] > 1)
                        if (doc.get(
                                "/s_sp_fishery/" + list_fishery_name[f] +
                                    "/length_threshold",
                                "use") == "true") {
                            dvarsSlength_sp_fishery[i][ifx] = boundp(
                                x[idx], dvarpars_min[idx], dvarpars_max[idx],
                                penalty);
                            s_length_sp_fishery[i][ifx] =
                                value(dvarsSlength_sp_fishery[i][ifx]);
                    ...
                    if (s_func_type[f] >= 3)
                        if (doc.get(
                                "/s_sp_fishery/" + list_fishery_name[f] +
                                    "/right_asymptote",
                                "use") == "true") {
                            dvarsSasympt_sp_fishery[i][ifx] = boundp(
                                x[idx], s_asympt_sp_fishery_min[i][ifx],
                                s_asympt_sp_fishery_max[i][ifx], penalty);
                            s_asympt_sp_fishery[i][ifx] =
                                value(dvarsSasympt_sp_fishery[i][ifx]);
    ...
    if (doc.get("/likelihood_parameters/variables", "use") == "true") {
        for (int i = 0; i < nb_species; i++) {
            ...
            for (int f = 0; f < nb_fishery; f++) {
                if (mask_fishery_sp[i][f]) {
                    // if (like_types[i][ifx]==2 || like_types[i][ifx]==4 ||
                    // like_types[i][ifx]==5){
                    if (/*like_types[i][ifx]==2 || */ like_types[i][ifx] == 4 ||
                        like_types[i][ifx] == 5) {
                        dvarsLike_param[i][ifx] = boundp(
                            x[idx], dvarpars_min[idx], dvarpars_max[idx],
                            penalty);
    ...
    for (int i = 0; i < nb_species; i++) {
        ...
        for (int f = 0; f < nb_fishery; f++) {
            if (mask_fishery_sp[i][f]) {
                if (like_types[i][ifx] == 5 &&
                    doc.get("/likelihood_parameters/variables", "use") ==
                        "true") {
                    dvarsProb_zero[i][ifx] = boundp(
                        x[idx], dvarpars_min[idx], dvarpars_max[idx], penalty);
    ...
    // cout << statpars << endl;
    // exit(1);
    if (!scalc()) cout << value(penalty) << "; ";
    return penalty;
}

// partially autodiff function
void VarParamCoupled::large_pen(
    dvariable x, double xmin, double xmax, dvariable& penalty,
    const double eps) {
    /*
...
double VarParamCoupled::Bard_pen(
    double x, double xmin, double xmax, double pen) {
    if (x < 1e-7) x = 1e-7;
...
/*
//adjoint
void VarParamCoupled::large_pen(dvariable x, double xmin, double xmax,
dvariable& penalty, const double eps)
{
        double x_c = value(x);
...
void dv_Bard_pen()
{
        ...
        if (xs>1-s || xs<s) {
                ...
                //double penalty =
-pen*(log(xs+1e-40)+log(1.0-xs+1.e-40)+log(4.0)); double dfxs =
-P*(1/(xs+eps)-1/(1-xs+eps))*dfB;
...
void VarParamCoupled::save_statistics(
    const string dirout, const adstring_array x_names, double likelihood,
    dvector g, double elapsed_time, int status, int iter, int nvars) {
    ofstream ofs;
    double gmax;
    ...
    if (status >= 1) {
        ...
        if (tuna_spinup != 0)
            text = "included, period of population building: " +
                   str((int)(save_first_yr - 4)) + "-" +
                   str(int(save_first_yr)) + "; ";
        ofs << "Simulation: domain " << latitudeMin << ", " << latitudeMax
            << ", " << longitudeMin << ", " << longitudeMax << "; spinup "
            << text
            << "skipped after spinup or initial conditions: " << nbsteptoskip
            << "; time period with likelihood optimization: "
            << save_first_yr + nbsteptoskip / 12 << "-" << save_last_yr << endl;
        ofs << "Name";
        for (int idx = 0; idx < _nstatpars; idx++)
            ofs << "\t" << statpar_names[idx];
        ofs << endl;
        ofs << "Value";
        for (int idx = 0; idx < _nstatpars; idx++) ofs << "\t" << statpars[idx];
        ofs << endl;
        ...
        ofs << "Iteration\t"
            ...
            << "||G||";
        int ncoltoskip = 6;
        for (int idx = 1; idx <= nvars; idx++) ofs << "\t" << x_names[idx];
        ofs << endl;
        for (int i = 0; i < ncoltoskip; i++) ofs << "\t0";
        for (int idx = 1; idx <= nvars; idx++) ofs << "\t" << dvarpars_min[idx];
        ofs << endl;
        for (int i = 0; i < ncoltoskip; i++) ofs << "\t0";
        for (int idx = 1; idx <= nvars; idx++) ofs << "\t" << dvarpars_max[idx];
        ofs << endl;
    }
    gmax = max(fabs(g));
    ofs << iter << "\t" << status << "\t" << elapsed_time << "\t" << likelihood
        << "\t" << gmax << "\t" << norm(g);
    for (int idx = 1; idx <= nvars; idx++) ofs << "\t" << dvarpars[idx];
