...
bool VarParamCoupled::read(const string& parfile) {
    ...
    if (!doc.get("/save_first_date", "year").empty()) {
        ...
        ndatini = get_year(save_first_yr) * 10000 +
                  get_month(save_first_yr) * 100 + 15;
        ndatfin =
            get_year(save_last_yr) * 10000 + get_month(save_last_yr) * 100 + 15;
    }
    ...
    date_mode = 3;  // default value for monthly stepping, uses 360-days
                    // calendar
    if (deltaT < 30) date_mode = 1;
    // rundates will be used for reading the data (fishing etc) for the
    ...
    if (!doc.get("/type_oxy", "value").empty())
        ...
        cout << "WARNING: the type of oxygen data is not set up, default is "
                "monthly time series!"
             << endl;
    ...
    // Climatological files to be ignored in the parfile: no spinup or forecasts
    // in this version.
    if (tuna_spinup || nb_yr_forecast) {
        ...
        for (int k = 0; k < nb_layer; k++) {
            std::ostringstream ostr;
            ostr << "layer" << k;
            strfile_tmc.push_back(
                str_dir + doc.get("/strfile_t_clm", ostr.str()));
            strfile_umc.push_back(
                str_dir + doc.get("/strfile_u_clm", ostr.str()));
            strfile_vmc.push_back(
                str_dir + doc.get("/strfile_v_clm", ostr.str()));
            if (!type_oxy)
                strfile_oxymc.push_back(
                    str_dir + doc.get("/strfile_oxy_clm", ostr.str()));
        }
    }
    ...
    if (!doc.get("/write_all_cohorts_dym", "value").empty())
        write_all_cohorts_dym =
            doc.getInteger("/write_all_cohorts_dym", "value");

    write_all_fisheries_dym = 0;
    if (!doc.get("/write_all_fisheries_dym", "value").empty())
        write_all_fisheries_dym =
            doc.getInteger("/write_all_fisheries_dym", "value");

    // If output directory doesn't exist create it
    string test = strdir_output + "/test";
    ofstream ecritbin(test.c_str(), ios::binary | ios::out);
    if (!ecritbin) {
        cerr << "WARNING : Cannot find output directory '" << strdir_output
             << "', will create the new folder './output'" << endl;
    ...
    /*
            ...
            if (!doc.get("/mpa_simulation").empty()){
                    mpa_simulation =
       doc.getInteger("/mpa_simulation/isON","value"); if (mpa_simulation){
                            mpa_scenario =
       doc.getInteger("/mpa_simulation/scenario", "value"); str_file_maskMPA =
       str_dir + doc.get("/mpa_simulation/mpa_mask", "value");
                    }
            }
    */
    mpa_simulation = 0;
    ...
    habitat_run_type =
        0;  // 0 -spawning; 1- feeding (in this case will read age in months)
    nb_habitat_run_age = 1;  // one for spawning habitat, can be several for
                             // feeding habitat Note, for optimization it is
                             // indespensable to provide at least three!
    ...
    // double C Conversion parameter from unit of PP (nitrogen or carbon) to wet
    // weight of forage
    //(Iverson 1990): ratio C/N for fish =3.6 ; ratio Dry weight/ C =2.4 ; ratio
    //Wet weight/Dry weight = 3.3
    // 1 mmol N -> g N (1 mole N =14g) 1 mmol N = 14/1000* 3.6 * 2.4 * 3.3 = 0.4
    // g WW 1 mmol C -> g C (1 mole C =12g) 1 mmol C = 12/1000* 2.4 * 3.3 =
    // 0.095 g WW source_frg[nb_forage]  - percentage of new prod transferred to
    // each forage day_layer[nb_forage]   - layer where each forage is during
    // the day night_layer[nb_forage] - layer where each forage is during the
    // night tstep_forage	   - time step for the forage (days) sigma_fcte
    // - diffusion coefficient for forage (nm2.d-1)
    ////////////////////
    ...
    if (nb_species) {
        ...
        sp_nb_cohort_jv.allocate(
            0, nb_species - 1);  // renamed sp_nb_age_class_jv;
        sp_nb_cohort_ad.allocate(
            0, nb_species - 1);                   // renamed sp_nb_age_class_ad;
        sp_a0_adult.allocate(0, nb_species - 1);  // NEW;
        sp_unit_cohort.allocate(0, nb_species - 1);  // renamed
                                                     // sp_unit_age_class
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        if (!doc.get("/seasonal_migrations", sp_name[sp]).empty())
            seasonal_migrations[sp] =
                doc.getInteger("/seasonal_migrations", sp_name[sp]);
        if (seasonal_migrations[sp])
            if (gcalc() && latitudeMax > 65)
                cout << "ATTN: Make sure the latitudes above 65N/S are masked "
                     << "to avoid NANs in the gradient of Season_peak" << endl;

        // old parameter files:
        if (doc.get("/spawning_season_peak").empty()) {
            cerr << "Current version require variable parameters "
                    "<spawning_season_peak> and <spawning_season_start>! "
                    "Modify your parameter file accordingly. The program will "
                    "exit now... "
                 << endl;
            exit(1);
        }
        spawning_season_peak[sp] =
            doc.getDouble("/spawning_season_peak", sp_name[sp]);
        spawning_season_start[sp] =
            doc.getDouble("/spawning_season_start", sp_name[sp]);
        ...
        if (!doc.get("/uncouple_sst_larvae", sp_name[sp]).empty()) {
            uncouple_sst_larvae[sp] =
                doc.getInteger("/uncouple_sst_larvae", sp_name[sp]);
            if (uncouple_sst_larvae[sp]) {
                // optimal temperature in Gaussian temperature function for
                // larvae survival
        ...
        // previously alpha_spawning in food to predator ratio in the spawning
        // index became two parameters - one in the food (prey) function and
        // another in predator one
        alpha_hsp_prey[sp] = doc.getDouble("/alpha_hsp_prey", sp_name[sp]);
        alpha_hsp_predator[sp] =
            doc.getDouble("/alpha_hsp_predator", sp_name[sp]);
        beta_hsp_predator[sp] =
            doc.getDouble("/beta_hsp_predator", sp_name[sp]);

        // maximal number of larvae (by cell) at large spawning biomass of
        // adults
        nb_recruitment[sp] = doc.getDouble("/nb_recruitment", sp_name[sp]);

        // slope coefficient in Beverton-Holt function
        a_adults_spawning[sp] =
            doc.getDouble("/a_adults_spawning", sp_name[sp]);
    }
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        if (!doc.get("/gaussian_thermal_function", sp_name[sp]).empty()) {
            gaussian_thermal_function[sp] =
                doc.getInteger("/gaussian_thermal_function", sp_name[sp]);
        }
        ...
        // slope of the function of optimal temperature by age as a function of
        // fish size
        ...
        if (!gaussian_thermal_function[sp]) {
            ...
            // three parameters of alternative (non-Gaussian) thermal function
            // in feeding habitat index
            for (int n = 0; n < 3; n++) {
                std::ostringstream ostr;
                ostr << "delta" << n + 1;
                thermal_func_delta[n][sp] = doc.getDouble(
                    string("/thermal_func/") + ostr.str(), sp_name[sp]);
            }
        }
        if (gaussian_thermal_function[sp]) {
            for (int n = 0; n < 3; n++) {
        ...
        for (int n = 0; n < nb_forage; n++) {
            eF_habitat[n][sp] = doc.getDouble(
                string("/eF_habitat/") + frg_name[n], sp_name[sp]);
        }
    }
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        if (!doc.get("/vertical_movement", sp_name[sp]).empty())
            vert_movement[sp] =
                doc.getInteger("/vertical_movement", sp_name[sp]);

        // multiplier to maximal diffusion coefficient which is linked to fish
        // size
        sigma_species[sp] = doc.getDouble("/sigma_species", sp_name[sp]);

        // curvature coefficient in the function of habitat to compute local
        // diffusion rate
    ...
    // Adding rmax to the parfile as fixed parameter. Make it par[sp] once it's
    // proven necessary
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        for (int n = 0; n < nb_life_stages; n++) {
            nb_cohort_life_stage[n] =
                doc.getInteger("/nb_cohort_life_stage/" + sp_name[sp], n);
            if (life_stage[n].find("larvae") == 0)
                sp_nb_cohort_lv[sp] = nb_cohort_life_stage[n];
            if (life_stage[n].find("juvenile") == 0)
                sp_nb_cohort_jv[sp] = nb_cohort_life_stage[n];
            if (life_stage[n].find("young") == 0)
                sp_nb_cohort_ad[sp] = nb_cohort_life_stage[n];
            if (life_stage[n].find("adult") == 0)
                sp_nb_cohort_ad[sp] += nb_cohort_life_stage[n];
        }
        ...
        for (int a = 0; a < sp_nb_cohorts[sp]; a++) {
            sp_unit_cohort[sp][a] =
                doc.getInteger("/sp_unit_cohort/" + sp_name[sp], a);
            if (sp_unit_cohort[sp][a] < deltaT) {
                cerr << "Sorry, have to stop here, the model requires that "
                        "sp_unit_cohort (size of the cohort) >= deltaT!"
                     << endl;
                exit(1);
            }
        ...
        // these length bins bounds will be used to aggregate LF data within
        // Seapodym cohorts.
        if (!doc.get("/length/bounds").empty()) {
            for (int a = sp_a0_adult[sp]; a <= sp_nb_cohort_ad[sp]; a++)
                length_bins[sp][a] =
                    doc.getDouble(string("/length/") + "bounds", a);
            ...
            length_bins[sp][sp_nb_cohorts[sp] - 1] =
                1.5 * L_pr - .5 * length(sp, sp_nb_cohorts[sp] - 3);
            length_bins[sp][sp_nb_cohorts[sp]] =
                length[sp][sp_nb_cohorts[sp] - 1] + 2.0;
        }
        ...
        if (doc.get("/age_compute_habitat/" + sp_name[sp]).empty()) {
            ...
            for (int a = 0; a < sp_nb_cohorts[sp]; a++)
                age_compute_habitat[sp][a] =
                    doc.getInteger("/age_compute_habitat/" + sp_name[sp], a);
        }
        ...
        // Maturity:
        // age at first maturity (usually age class with proportion of mature
        // individuals = 0.1-0.2)
        ...
        if (doc.get("/maturity_age").empty()) {
            cout << endl
                 << "IMPORTANT: Maturity-at-age described by step function "
                    "with 0 below and 1 above 50% maturity"
                 << endl
                 << endl;
            for (int a = 0; a < sp_nb_cohorts[sp]; a++) {
        ...
        if (!doc.get("/maturity_age").empty()) {
            cout
                << endl
                << "IMPORTANT: Maturity-at-age described by continuous function"
                << endl;
            for (int a = 0; a < sp_nb_cohorts[sp]; a++)
                maturity_age[sp][a] =
                    doc.getDouble("/maturity_age/" + sp_name[sp], a);

            for (int a = 0; a < sp_nb_cohorts[sp]; a++) {
                if (maturity_age[sp][a] >= 0.15) {
                    cout << "IMPORTANT: first age class with spawning "
                            "potential (15% of individuals are mature) is "
                         << a << endl;
                    age_mature[sp] = a;
                    break;
                }
            ...
            for (int a = 0; a < sp_nb_cohorts[sp]; a++) {
                if (maturity_age[sp][a] >= 0.5) {
                    cout << "FOR INFORMATION ONLY: first age class with 50% "
                            "maturity is "
                         << a << endl;
                    break;
                }
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        ...
        if (!doc.get("/food_requirement_in_mortality", sp_name[sp]).empty()) {
            food_requirement_in_mortality[sp] =
                doc.getInteger("/food_requirement_in_mortality", sp_name[sp]);
            // percentage of forage available for species assuming presence of
            // other species in the habitat
            residual_competition[sp] =
                doc.getDouble("/residual_competition", sp_name[sp]);
        }
    }
    ...
    if (nb_fishery) {
        ...
        cpue_mult.allocate(
            0, nb_fishery -
                   1);  // cpue multiplier in the likelihood (Inna 18/11/2010)
        cpue_mult.initialize();

        // fishery name (code with 1 letter and 1 number; e.g., L1 for long-line
        // or S2 for purse-seine)
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            if (!doc.get("/file_catch_data").empty())
                nb_catch_files = doc.getInteger(
                    string("/file_catch_data/nb_files"), sp_name[sp]);
            for (int nf = 0; nf < nb_catch_files; nf++) {
                std::ostringstream ostr;
                ostr << "file" << nf + 1;
                file_catch_data.push_back(
                    str_dir_fisheries +
                    doc.get("/file_catch_data/" + sp_name[sp], ostr.str()));
            }
        }
        for (int sp = 0; sp < nb_species; sp++) {
            nb_frq_files = 0;
            if (!doc.get("/file_frq_data").empty()) {
                nb_frq_files = doc.getInteger(
                    string("/file_frq_data/nb_files"), sp_name[sp]);
                if (nb_frq_files)
                    for (int nf = 0; nf < nb_frq_files; nf++) {
                        ...
                        file_frq_data.push_back(
                            str_dir_fisheries +
                            doc.get(
                                "/file_frq_data/" + sp_name[sp], ostr.str()));
                    }
            } else {
        ...
        // Number of fisheries by species (nb of fisheries catching a given
        // species)
        ...
        if (!tags_only) {
            ...
            for (int sp = 0; sp < nb_species; sp++) {
                for (int f = 0; f < nb_fishery; f++) {
                    mask_fishery_sp[sp][f] = doc.getInteger(
                        string("/mask_fishery_sp/") + sp_name[sp], f);
                    if (mask_fishery_sp[sp][f]) nb_fishery_by_sp[sp]++;
                }
            }
        }
        // the strdir_output should always be declared for the reference (Fref)
        // run the directory output_F0 will be automatically created inside if
        // F0 setup is ON
        if (sum(mask_fishery_sp) == 0) {
            ...
            ofstream ecritbin(test.c_str(), ios::binary | ios::out);
            if (!ecritbin) {
                cerr << "WARNING : Cannot find 'output_F0' directory for no "
                        "fishing scenario in '"
                     << strdir_output << "', will create it!'" << endl;
                mkdir(strdir_output.c_str(), 0777);
            } else
                remove(test.c_str());
        }
        ...
        if (!doc.get("/mask_fishery_likelihood").empty()) {
            for (int sp = 0; sp < nb_species; sp++) {
                if (!doc.get(string("/mask_fishery_likelihood/") + sp_name[sp])
                         .empty()) {
                    for (int f = 0; f < nb_fishery; f++)
                        mask_fishery_sp_like[sp][f] = doc.getInteger(
                            string("/mask_fishery_likelihood/") + sp_name[sp],
                            f);
                }
            }
        }
        mask_fishery_sp_no_effort.allocate(
            0, nb_species - 1, 0, nb_fishery - 1);
        mask_fishery_sp_no_effort
            .initialize();  // by default all fisheries have effort
        fisheries_no_effort_exist.allocate(0, nb_species - 1);
        if (!doc.get("/mask_fishery_no_effort").empty()) {
            for (int sp = 0; sp < nb_species; sp++) {
                if (!doc.get(string("/mask_fishery_no_effort/") + sp_name[sp])
                         .empty()) {
                    for (int f = 0; f < nb_fishery; f++)
                        mask_fishery_sp_no_effort[sp][f] = doc.getInteger(
                            string("/mask_fishery_no_effort/") + sp_name[sp],
                            f);
                }
                // fisheries_no_effort_exist will be used as a flag to avoid
                // catch removal from density of tags
                if (!flag_no_fishing)
                    cout << "FISHERIES without effort (1) in the simulation: "
                         << mask_fishery_sp_no_effort(sp) << endl;
                fisheries_no_effort_exist[sp] =
                    sum(mask_fishery_sp_no_effort[sp]);

                // check: if no effort fisheries have different catch units in
                // the parfile the catch removal method requires all fisheries
                // catch to have the same units
                if (fisheries_no_effort_exist(sp)) {
                    ...
                    if (sum_units > 0 &&
                        sum_units != fisheries_no_effort_exist(sp)) {
                        cout << "WRONG CONFIGURATION: Fisheries without effort "
                                "in simulation - same catch units required for "
                                "all 'no effort' fisheries! The program will "
                                "exit, sorry."
                             << endl;
                        exit(1);
                    }
        ...
        /*
                        mask_mpa_fishery.allocate(0, nb_species - 1, 0,
           nb_fishery - 1); mask_mpa_fishery = 1; if (mpa_simulation){ for (int
           sp=0;sp<nb_species;sp++) { for (int f=0;f<nb_fishery;f++)
                                                mask_mpa_fishery[sp][f] =
           doc.getInteger(string("/mpa_simulation/mpa_fisheries/") +
           sp_name[sp], f);
                                }
                        }
        */
        // to convert the units of LL from hooks to thous.hooks to avoid small
        // value of catchability parameter to be set by the user, by default
        // = 1.0
        ...
        if (!doc.get("/eff_units_converter").empty())
            for (int f = 0; f < nb_fishery; f++)
                eff_units_converter(f) =
                    doc.getDouble("/eff_units_converter", f);
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            int k = 0;
            for (int f = 0; f < nb_fishery; f++) {
                int type = doc.getInteger(
                    string("/likelihood_types/") + sp_name[sp], f);
        ...
        if (mpa_simulation) {
            ...
            for (int i = 0; i < nb_mpa; i++) {
                ...
                mpa_scenario[i] =
                    doc.getInteger("/mpa_simulation/mpa_scenario", i);
                if (mpa_scenario[i] == 1)
                    mpa_S1_X[i] = doc.getInteger("/mpa_simulation/mpa_S1_X", i);
            }
            for (int f = 0; f < nb_fishery; f++)
                mpa_fishery[f] =
                    doc.getInteger(string("/mpa_simulation/mpa_fisheries"), f);
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            if (flag_no_fishing) {
                frq_like[sp] = 0;
                cout << "WARNING: LF likelihood is forced to OFF in NO FISHING "
                        "run"
                     << endl;
            }
        }

        // 3. TAGs likelihood: further options here (see above the code for main
        // flags)
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            if (tag_like[sp]) {
                if (!doc.get("/tag_gauss_kernel_on", "value").empty()) {
                    tag_gauss_kernel_on =
                        doc.getInteger("/tag_gauss_kernel_on", "value");
                }
            }
        }
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            if (!doc.get("/stock_likelihood", sp_name[sp]).empty()) {
                stock_like[sp] =
                    doc.getInteger("/stock_likelihood", sp_name[sp]);
            }

            if (stock_like[sp]) {
                mean_stock_obs[sp] =
                    doc.getDouble("/mean_stock_obs/" + sp_name[sp], "value");
                stock_lonmin[sp] =
                    doc.getDouble("/mean_stock_obs/" + sp_name[sp], "lgmin");
                stock_lonmax[sp] =
                    doc.getDouble("/mean_stock_obs/" + sp_name[sp], "lgmax");
                stock_latmin[sp] =
                    doc.getDouble("/mean_stock_obs/" + sp_name[sp], "ltmin");
                stock_latmax[sp] =
                    doc.getDouble("/mean_stock_obs/" + sp_name[sp], "ltmax");
            }
        }
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int f = 0; f < nb_fishery; f++)
                if (mask_fishery_sp[sp][f]) {
                    if (like_types(sp, k) == 2 || like_types(sp, k) == 4 ||
                        like_types(sp, k) == 5)
                        like_param[sp][k] = doc.getDouble(
                            string("/likelihood_parameters/") +
                                list_fishery_name[f],
                            sp_name[sp]);
                    k++;
                }
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            dx_tags = 1;
            dy_tags = 1;
            lonmin_tags = longitudeMin;
            lonmax_tags = longitudeMax;
            latmin_tags = latitudeMin;
            latmax_tags = latitudeMax;
            ...
            if (tag_like(sp)) {
                if (!doc.get("/tags_grid").empty()) {
                    ...
                    if (dx_tags < deltaX / 60) {
                        dx_tags = deltaX / 60;
                        cout << "WARNING: changed the tagging data "
                                "longitudinal resolution to deltaX = "
                             << deltaX / 60 << " degree" << endl;
                    }
                    if (dy_tags < deltaY / 60) {
                        dy_tags = deltaY / 60;
                        cout << "WARNING: changed the tagging data latitudinal "
                                "resolution to deltaY = "
                             << deltaY / 60 << " degree" << endl;
                    }
                    lonmin_tags =
                        doc.getDouble(string("/tags_grid/longitude"), "east");
                    lonmax_tags =
                        doc.getDouble(string("/tags_grid/longitude"), "west");
                    latmin_tags =
                        doc.getDouble(string("/tags_grid/latitude"), "south");
                    latmax_tags =
                        doc.getDouble(string("/tags_grid/latitude"), "north");
                }
                nb_tag_files = doc.getInteger(
                    string("/file_tag_data/nb_files"), sp_name[sp]);
                ...
                if (nb_tag_files) {
                    for (int nf = 0; nf < nb_tag_files; nf++) {
                        ...
                        file_tag_data.push_back(
                            str_dir_tags +
                            doc.get(
                                "/file_tag_data/" + sp_name[sp], ostr.str()));
                    }
                } else {
            ...
            for (int f = 0; f < nb_fishery; f++) {
                if (mask_fishery_sp[sp][f]) {
                    ...
                    if (!doc.get("/prob_zero").empty())
                        prob_zero[sp][k] = doc.getDouble(
                            string("/prob_zero/") + list_fishery_name[f],
                            sp_name[sp]);
                    k++;
                }
            }
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int f = 0; f < nb_fishery; f++) {
                if (mask_fishery_sp[sp][f]) {
                    q_sp_fishery[sp][k] = doc.getDouble(
                        string("/q_sp_fishery/") + list_fishery_name[f],
                        sp_name[sp]);
                    if (mask_fishery_sp_no_effort[sp][f] &&
                        doc.get(
                            "/q_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "use") == "true")
                        doc.set(
                            "/q_sp_fishery/" + list_fishery_name[f] +
        ...
        for (int f = 0; f < nb_fishery; f++) {
            if (!doc.get(
                        "/q_sp_fishery/" + list_fishery_name[f] + "/variable",
                        "dyn")
                     .empty())
                q_dyn_fishery[f] = doc.getDouble(
                    "/q_sp_fishery/" + list_fishery_name[f] + "/variable",
                    "dyn");
        }
        ...
        for (int f = 0; f < nb_fishery; f++)
            s_func_type[f] = doc.getInteger(
                string("/s_sp_fishery/") + list_fishery_name[f] +
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int f = 0; f < nb_fishery; f++) {
                if (mask_fishery_sp[sp][f]) {
                    s_slope_sp_fishery[sp][k] = doc.getDouble(
                        string("/s_sp_fishery/") + list_fishery_name[f],
                        sp_name[sp]);
                    if (s_func_type[f] > 1)
                        s_length_sp_fishery[sp][k] = doc.getDouble(
                            string("/s_sp_fishery/") + list_fishery_name[f] +
                                string("/length_threshold"),
                            sp_name[sp]);
                    if (s_func_type[f] >= 3)
                        s_asympt_sp_fishery[sp][k] = doc.getDouble(
                            string("/s_sp_fishery/") + list_fishery_name[f] +
                                string("/right_asymptote"),
                            sp_name[sp]);
                    k++;
                }
            }
    ...
    for (int sp = 0; sp < nb_species; sp++) {
        if (frq_like[sp] && !flag_no_fishing) {
            use_lf_regstruc = 1;
            cout << "WARNING: LF likelihood is active -> using LF regions for "
                    "biomass aggregation!"
                 << endl;
            if (nb_species > 1)
                cout << "ATTENTION: Verify that LF data for species " << sp
                     << " have the same regional structure!" << endl;
            define_regions();
        } else {
            if (!frq_like[sp])
                cout << "WARNING: LF likelihood is not active -> "
                        "use_lf_regstruc is OFF -> use regional structure for "
                        "biomass aggregation"
                     << endl;
            use_lf_regstruc = 0;
        }
    ...
    if (!use_lf_regstruc) {
        ...
        if (nb_region) {
            cout << "INFO: Parfile regional structure in use; number of "
                    "regions: "
                 << nb_region << endl;
            ...
            for (int a = 0; a < nb_region; a++) {
                ...
                if (area[a]->lgmin < longitudeMin)
                    area[a]->lgmin = longitudeMin;
                area[a]->lgmax = doc.getDouble(ostr.str(), "lgmax");
                if (area[a]->lgmax > longitudeMax)
                    area[a]->lgmax = longitudeMax;
            ...
            for (int sp = 0; sp < nb_species; sp++) {
                nb_region_sp_B[sp] =
                    doc.getInteger("/nb_region_sp_B", sp_name[sp]);
                if (nb_region_sp_B[sp] > nb_region) {
                    nb_region_sp_B[sp] = nb_region;
                    cout << "WARNING: nb_region_sp_B cannot be > nb_region, "
                            "value "
                         << nb_region_sp_B[sp] << " will be taken" << endl;
                }
            }
            ...
            for (int sp = 0; sp < nb_species; sp++) {
                ...
                for (int a = 0; a < amax; a++) {
                    area_sp_B[sp][a] =
                        doc.getInteger(string("/area_sp_B/") + sp_name[sp], a);
                }
            }
        } else {
            nb_region_sp_B.allocate(0, nb_species - 1);
            ...
            cout << endl
                 << "WARNING!!!!!! nb_region=0 -> NO biomass aggregations";
            if (!flag_no_fishing)
    ...
    par_read_bounds(
        Mp_mean_max, Mp_mean_max_min, Mp_mean_max_max, "/Mp_mean_max", nni);
    par_read_bounds(
        Mp_mean_exp, Mp_mean_exp_min, Mp_mean_exp_max, "/Mp_mean_exp", nni);
    par_read_bounds(
        Ms_mean_max, Ms_mean_max_min, Ms_mean_max_max, "/Ms_mean_max", nni);
    par_read_bounds(
        Ms_mean_slope, Ms_mean_slope_min, Ms_mean_slope_max, "/Ms_mean_slope",
        nni);
    par_read_bounds(
        M_mean_range, M_mean_range_min, M_mean_range_max, "/M_mean_range", nni);
    par_read_bounds(
        a_sst_spawning, a_sst_spawning_min, a_sst_spawning_max,
        "/a_sst_spawning", nni);
    par_read_bounds(
        b_sst_spawning, b_sst_spawning_min, b_sst_spawning_max,
        "/b_sst_spawning", nni);
    par_read_bounds(
        a_sst_larvae, a_sst_larvae_min, a_sst_larvae_max, "/a_sst_larvae", nni);
    par_read_bounds(
        b_sst_larvae, b_sst_larvae_min, b_sst_larvae_max, "/b_sst_larvae", nni);
    par_read_bounds(
        alpha_hsp_prey, alpha_hsp_prey_min, alpha_hsp_prey_max,
        "/alpha_hsp_prey", nni);
    par_read_bounds(
        alpha_hsp_predator, alpha_hsp_predator_min, alpha_hsp_predator_max,
        "/alpha_hsp_predator", nni);
    par_read_bounds(
        beta_hsp_predator, beta_hsp_predator_min, beta_hsp_predator_max,
        "/beta_hsp_predator", nni);
    par_read_bounds(
        a_sst_habitat, a_sst_habitat_min, a_sst_habitat_max, "/a_sst_habitat",
        nni);
    par_read_bounds(
        b_sst_habitat, b_sst_habitat_min, b_sst_habitat_max, "/b_sst_habitat",
        nni);
    par_read_bounds(
        T_age_size_slope, T_age_size_slope_min, T_age_size_slope_max,
        "/T_age_size_slope", nni);
    ...
    for (int n = 0; n < 3; n++) {
        ...
        ostr << "delta" << n + 1;
        par2_read_bounds(
            thermal_func_delta[n], thermal_func_delta_min[n],
            thermal_func_delta_max[n], "/thermal_func", ostr.str(), nni);
    }
    par_read_bounds(
        a_oxy_habitat, a_oxy_habitat_min, a_oxy_habitat_max, "/a_oxy_habitat",
        nni);
    par_read_bounds(
        b_oxy_habitat, b_oxy_habitat_min, b_oxy_habitat_max, "/b_oxy_habitat",
        nni);
    eF_habitat_min.allocate(0, nb_forage - 1);
    eF_habitat_max.allocate(0, nb_forage - 1);
    for (int n = 0; n < nb_forage; n++) {
        par2_read_bounds(
            eF_habitat[n], eF_habitat_min[n], eF_habitat_max[n], "/eF_habitat",
            frg_name[n], nni);
    }
    par_read_bounds(
        hp_cannibalism, hp_cannibalism_min, hp_cannibalism_max,
        "/hp_cannibalism", nni);
    par_read_bounds(
        sigma_species, sigma_species_min, sigma_species_max, "/sigma_species",
        nni);
    par_read_bounds(
        MSS_species, MSS_species_min, MSS_species_max, "/MSS_species", nni);
    par_read_bounds(
        MSS_size_slope, MSS_size_slope_min, MSS_size_slope_max,
        "/MSS_size_slope", nni);
    par_read_bounds(
        c_diff_fish, c_diff_fish_min, c_diff_fish_max, "/c_diff_fish", nni);
    par_read_bounds(
        nb_recruitment, nb_recruitment_min, nb_recruitment_max,
        "/nb_recruitment", nni);
    par_read_bounds(
        a_adults_spawning, a_adults_spawning_min, a_adults_spawning_max,
        "/a_adults_spawning", nni);
    par_read_bounds(
        spawning_season_peak, spawning_season_peak_min,
        spawning_season_peak_max, "/spawning_season_peak", nni);
    par_read_bounds(
        spawning_season_start, spawning_season_start_min,
        spawning_season_start_max, "/spawning_season_start", nni);
            ...
            for (int f = 0; f < nb_fishery; f++) {
                if (mask_fishery_sp[sp][f]) {
                    if (doc.get(
                            ...
                            "use") == "true") {
                        _nvarcalc++;
                        q_sp_fishery_min[sp][k] = doc.getDouble(
                            "/q_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "min");
                        q_sp_fishery_max[sp][k] = doc.getDouble(
                            "/q_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "max");
                    } else {
                        statpar_names_temp[nni] =
                            "q sp fishery(" + str(sp) + "," + str(k) + ")";
    ...
    if (doc.get("/s_sp_fishery/variables", "use") == "true") {
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int f = 0; f < nb_fishery; f++) {
                if (mask_fishery_sp[sp][f]) {
                    if (doc.get(
                            "/s_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "use") == "true") {
                        _nvarcalc++;
                        s_slope_sp_fishery_min[sp][k] = doc.getDouble(
                            "/s_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "min");
                        s_slope_sp_fishery_max[sp][k] = doc.getDouble(
                            "/s_sp_fishery/" + list_fishery_name[f] +
                                "/variable",
                            "max");
                    } else {
                        statpar_names_temp[nni] =
                            "s sp fishery(" + str(sp) + "," + str(k) + ")";
                    ...
                    if (s_func_type[f] > 1) {
                        if (doc.get(
                            ...
                            statpar_names_temp[nni] = "length threshold(" +
                                                      str(sp) + "," + str(k) +
                                                      ")";
                    ...
                    if (s_func_type[f] >= 3) {
                        if (doc.get(
                                ...
                                "use") == "true") {
                            _nvarcalc++;
                            s_asympt_sp_fishery_min[sp][k] = doc.getDouble(
                                "/s_sp_fishery/" + list_fishery_name[f] +
                                    "/right_asymptote",
                                "min");
                            s_asympt_sp_fishery_max[sp][k] = doc.getDouble(
                                "/s_sp_fishery/" + list_fishery_name[f] +
                                    "/right_asymptote",
                                "max");
                        } else {
                            statpar_names_temp[nni] = "right asymptote(" +
                                                      str(sp) + "," + str(k) +
                                                      ")";
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int f = 0; f < nb_fishery; f++)
                if (mask_fishery_sp[sp][f]) {
                    ...
                    if (s_func_type[f] > 1) {
                        statpar_names_temp[nni] = "s length threshold(" +
                                                  str(sp) + "," + str(k) + ")";
                        statpars_temp[nni] = s_length_sp_fishery[sp][k];
                        nni++;
                    }
                    if (s_func_type[f] >= 3) {
                        statpar_names_temp[nni] =
                            "s right asymptote(" + str(sp) + "," + str(k) + ")";
                        statpars_temp[nni] = s_asympt_sp_fishery[sp][k];
                        nni++;
                    }
                    k++;
    ...
    if (doc.get("/likelihood_parameters/variables", "use") == "true") {
            ...
            for (int f = 0; f < nb_fishery; f++)
                if (mask_fishery_sp[sp][f]) {
                    if (like_types[sp][k] == 4 || like_types[sp][k] == 5)
                        ...
                        statpar_names_temp[nni] = "likelihood parameter(" +
                                                  str(sp) + "," + str(k) + ")";
        ...
        for (int sp = 0; sp < nb_species; sp++) {
            ...
            for (int f = 0; f < nb_fishery; f++)
                if (mask_fishery_sp[sp][f]) {
                    statpar_names_temp[nni] =
                        "likelihood parameter(" + str(sp) + "," + str(k) + ")";
                    statpars_temp[nni] = like_param[sp][k];
                    nni++;
                    k++;
                }
    ...
    for (int sp = 0; sp < nb_species; sp++)
        for (int k = 0; k < nb_fishery_by_sp[sp]; k++)
            if (like_types[sp][k] == 5 &&
                doc.get("/likelihood_parameters/variables", "use") == "true")
                _nvarcalc++;

    if (_nvarcalc == 0) {
        cerr << "ERROR: at least one control variable required!!! Will exit "
                "now..."
             << endl;
        exit(1);
    }
    ...
    if (!doc.get("/hyperspace_projection").empty()) {
        ...
        varproj_nsteps.allocate(0, nb_varproj);
        varproj_nsteps.initialize();
        for (int n = 0; n < nb_varproj; n++) {
            std::ostringstream ostr;
            ostr << "var" << n + 1;
            varproj.push_back(
                doc.get("/hyperspace_projection/" + ostr.str(), "name"));
            varproj_nsteps[n] = doc.getInteger(
                "/hyperspace_projection/" + ostr.str(), "nsteps");
        }
    }
    return true;
}
...
void VarParamCoupled::re_read_varparam() {
    ...
    par_read(
        alpha_hsp_prey_min, alpha_hsp_prey_max, "/alpha_hsp_prey", 0, 10000);
    par_read(
        alpha_hsp_predator_min, alpha_hsp_predator_max, "/alpha_hsp_predator",
        0, 10000);
    par_read(
        beta_hsp_predator_min, beta_hsp_predator_max, "/beta_hsp_predator", 0,
        10000);
    ...
    par_read(
        T_age_size_slope_min, T_age_size_slope_max, "/T_age_size_slope", 0, 10);
    for (int n = 0; n < 3; n++) {
        std::ostringstream ostr;
        ostr << "delta" << n + 1;
        string s = "thermal_func" + ostr.str();
        par_read(
            thermal_func_delta_min[n], thermal_func_delta_max[n], s, 0, 0.5);
    }
    ...
    par_read(
        a_adults_spawning_min, a_adults_spawning_max, "/a_adults_spawning", 0,
        1000);
    par_read(
        spawning_season_peak_min, spawning_season_peak_max,
        "/spawning_season_peak", 0, 366);
    par_read(
        spawning_season_start_min, spawning_season_start_max,
        "/spawning_season_start", 0.9, 1.5);
...
void CParam::define_regions() {  // create the regional structure from LF file
    ...
    for (int f = 0; f < nb_frq_files; f++) {
        ...
        if (littxt) {
            ...
            cout << endl
                 << "WARNING : Cannot read LF data file with regions"
                 << filename.c_str() << endl;
        }
    }
    area = new region*[nb_region];
    ...
    // this part is temporal, just to keep old definitions to avoid a lot of
    // changes further in the code nb regions par espece ou aggreger les donnees
    // de biomasses
    nb_region_sp_B.allocate(0, nb_species - 1);
    for (int sp = 0; sp < nb_species; sp++) nb_region_sp_B[sp] = nb_region;
    // liste des regions par espece ou aggreger les biomasses
    area_sp_B.allocate(0, nb_species - 1);
    for (int sp = 0; sp < nb_species; sp++) {
        const int amax = nb_region_sp_B[sp];
        ...
        for (int a = 0; a < amax; a++) {
            ...
            area_sp_B[sp][a] =
                area[a]->area_id;  // wont work in multispecies mode!!!
        }
    }
}

void VarParamCoupled::par_read_bounds(
    dvector& var, double& var_min, double& var_max, string s, int& nni) {
    string n = s;
...
void VarParamCoupled::par_read(
    double& varmin, double& varmax, string s, const double fixmin,
    const double fixmax) {
    string n = s;
    std::string whitespace = " ";
    n.erase(0, 1);
...
void VarParamCoupled::par2_read_bounds(
    dvector& var, double& var_min, double& var_max, string s1, string s2,
    int& nni) {
    string n = s1 + "_" + s2;
    std::string whitespace = " ";
    n.erase(0, 1);
