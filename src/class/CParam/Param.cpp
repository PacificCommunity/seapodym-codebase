...
//////////////////////////////////////////////////////////////////////
CParam::CParam() {}
CParam::~CParam() { delete_param(true); }
void CParam::delete_param(bool flag) {
    if ((flag) && (nb_species != 0) && (sp_nb_cohort_ad[0] != 0) &&
        (nb_fishery)) {
    ...
    if ((flag) && (nb_region)) {
        for (int i = 0; i < nb_region; i++) delete[] area[i];
        delete[] area;
    }
...
// la longueur d'un degre de longitude selon la latitude est proportionnelle au
...
double CParam::correction_lat(double lat) {
    ...
    // For a given latitude, return the ratio to the distance of 1 deg at
    // equator
...
double CParam::lastlat(int j) {
    double lastlat =
        (double)(latitudeMax - (j * deltaY / 60.0) + deltaY / 60.0);
    return lastlat;
}

// IMPORTANT: in order to get correct indices, (latitudeMax,longitudeMin) in
// parfile should give the NORTH-WEST corner of the grid cell and not the
// center, where all variables are calculated
double CParam::jtolat(
    int j) {  // returns latitudinal coordinate for Seapodym j index
    // latmax is the center: double lat = (double) (latitudeMax - (deltaY/60.0)
    // * (j - 1)); latmax is the corner
    double lat = (double)(latitudeMax - (deltaY / 60.0) * (j - 0.5));
    return lat;
}

double CParam::itolon(
    int i) {  // returns longitudinal coordinate for Seapodym i index
    // lonmin is the center: double lon = (double) (longitudeMin + (deltaX/60.0)
    // * (i - 1)); lonmin is the corner:
    double lon = (double)(longitudeMin + (deltaX / 60.0) * (i - 0.5));
    return lon;
}

int CParam::lattoj(
    double lat) {  // returns Seapodym index for the given latitude
...
int CParam::lontoi(
    double lon) {  // returns Seapodym index for the given longitude
...
void CParam::afcoef(
    ...
    const int
        f) {  // returns matrix of coefficients for the biomass aggregation over
              // fishing reso*reso area used in predicted_catch computation:
              //  |a(ki,kj) ...       |
              //  | ...                   |
              //  | ...  a(ki+i,kj+j) ... |
              //  | ...               ... |
    ...
    const int iw = lontoi(xw);
    const int ie = lontoi(xe);
    const int jn = lattoj(yn);
    const int js = lattoj(ys);
...
double CParam::selectivity_comp(
    const int sp, const int age, const int f,
    const int k /*, const int step_count*/) {
    ...
    switch (s_func_type[f]) {
        case 1:  // logistic
            selectivity =
                length(sp, age) / (s_slope_sp_fishery(sp, k) + length(sp, age));
            break;

        case 2:  // sigmoid
            selectivity = 1 / (1 + exp(s_slope_sp_fishery(sp, k) *
                                       (selected_length - length(sp, age))));
            break;
        ...
        case 3:  // asymmetric Gaussian
            ...
            if (length(sp, age) <= selected_length)
                selectivity =
                    exp(-pow(length(sp, age) - selected_length, 2) / sigma_sq);
            else
                selectivity =
                    (1 - s_asympt_sp_fishery(sp, k)) *
                        exp(-pow(length(sp, age) - selected_length, 2) /
                            sigma_sq) +
                    s_asympt_sp_fishery(sp, k);
        ...
        case 4:  // two-modal Gaussian
            ...
            sigma_sq =
                pow(s_slope_sp_fishery(sp, k), 2);  // the same sigma - temporal
            const double g1 =
                0.15 * exp(-pow(length(sp, age) - s_length1, 2) / sigma_sq);
            const double g2 =
                exp(-pow(length(sp, age) - s_length2, 2) / sigma_sq);
            selectivity = (g1 + g2) /
                          (1 + exp(-pow(s_length2 - s_length1, 2) / sigma_sq) +
                           exp(-pow(s_length1 - s_length2, 2) / sigma_sq));
            break;
    }
    return selectivity;
}

void CParam::rbin_input2d(
...
void CParam::rbin_mat2d(
    string file_in, const imatrix& carte, DMATRIX& mat2d, int nlat, int nlong,
    int nbytetoskip) {
    ifstream litbin(file_in.c_str(), ios::binary | ios::in);
