...
void VarSimtunaFunc::Faccessibility_comp(
    VarParamCoupled& param, VarMatrices& mat, const PMap& map, double temp_max,
    double oxy_teta, double oxy_cr, const int sp, const int age, const int jday,
    const int t) {
    ...
    dvector O2(0, nb_layer - 1);
    O2.initialize();
    dvector T(0, nb_layer - 1);
    T.initialize();
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0 && nl <= nb_layer) {
                ...
                for (int n = 0; n < nb_forage; n++) {
                    if (day_layer[n] < nl)
                        mat.dvarF_access(n, age).elem_value(i, j) =
                            (l_access[day_layer[n]] * DL +
                             l_access[night_layer[n]] * (1.0 - DL));
                }
            }
        }
    }
...
void VarSimtunaFunc::Average_currents_comp(
    VarParamCoupled& param, VarMatrices& mat, const PMap& map, const int age,
    const int
        t) {  // Computes tuna speed according to accessibilities to all layers
...
double f_accessibility_layer(
    ...
    // note, the constant is needed to avoid division by zero in weighted
    // average calculation of lf_access it does not have an impact on the
    // absolute value of f_access
...
void f_accessibility(
    ...
    const dvector O2, const dvector T, double twosigsq, double temp_mean,
    double oxy_teta, double oxy_cr, const int nl, const int nb_forage,
    const ivector day_layer, const ivector night_layer,
    const double DL) {  // computes accessibility functions to the vertical
                        // layer and weighted by density of forage
    ...
    for (int l = 0; l < nl; l++) {
        l_access(l) = f_accessibility_layer(
            O2(l), T(l), twosigsq, temp_mean, oxy_teta, oxy_cr);

        // weighted by the Forage biomass
        for (int n = 0; n < nb_forage; n++) {
            if (day_layer[n] == l) lf_access(l) += l_access(l) * forage[n] * DL;
            if (night_layer[n] == l)
                lf_access(l) += l_access(l) * forage[n] * (1 - DL);
        }

        sumL += lf_access(l);
    }

    for (int l = 0; l < nl; l++) lf_access(l) = lf_access(l) / sumL;
}

double thermal_func_type2(
    const double T, const double temp_min, const double temp_max,
    const double delta1, const double delta2,
    const double delta3) {  // Note, the condition of delta1 and delta2
                            // parameters: delta<0.95/max(temp_diff)
    ...
    double f_T = pow(
        (1.0 + exp(a1 * (T - temp_min))) * (1.0 + exp(a2 * (temp_max - T))), p);

    // scaling between 0 and 1
    double xmax = (log(a2 / a1) + a1 * temp_min + a2 * temp_max) / (a1 + a2);
    f_T /=
        pow((1.0 + exp(a1 * (xmax - temp_min))) *
                (1.0 + exp(a2 * (temp_max - xmax))),
            p);
...
void f_accessibility(
    dvector& l_access, dvector& lf_access, const dvector forage,
    const dvector O2, const dvector T, double temp_age, double temp_max,
    double delta1, double delta2, double delta3, double oxy_teta, double oxy_cr,
    const int nl, const int nb_forage, const ivector day_layer,
    const ivector night_layer,
    const double DL) {  // computes accessibility functions to the vertical
                        // layer and weighted by density of forage
    ...
    for (int l = 0; l < nl; l++) {
        l_access(l) = f_accessibility_layer(
            O2(l), T(l), temp_age, temp_max, delta1, delta2, delta3, oxy_teta,
            oxy_cr);
