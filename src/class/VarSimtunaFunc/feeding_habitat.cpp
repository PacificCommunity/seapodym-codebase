...
double sigma_hss_comp(
    double temp_age, double temp_max, const int age, const int jday);

void VarSimtunaFunc::Feeding_Habitat(
    VarParamCoupled& param, VarMatrices& mat, const PMap& map, dvar_matrix& Ha,
    ...
    if (migration_flag) {
        ...
        double sigma_sp_var =
            mat.sigma_season(sp, jday, age);  // sigma_hss_comp(param.temp_age[sp][age],
                                              // value(teta_max), age,jday);
...
void VarSimtunaFunc::Hf_comp(
    VarParamCoupled& param, VarMatrices& mat, const PMap& map, dvar_matrix& Ha,
    const int sp, const int age, const int jday, const int t) {
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0 && nl <= nb_layer) {
                ...
                if (!param.gcalc()) {
                    // last immature group if seasonal migrations, otherwise
                    // first mature if (age < param.age_mature[sp])
                    if (age == 11) mat.Hs[i][j] = value(Ha(i, j));

                    // always the last age computed here, will be overwritten by
                    // adult habitat if seasonal migrations
...
// Inna 05/13: the use of function is temporarily on hold, still using fixed
// values of sigma (see seasonal_switch)
double sigma_hss_comp(
    double temp_age, double temp_max, const int age, const int jday) {
    return 0;
}

void VarSimtunaFunc::Ha_comp(
    VarParamCoupled& param, VarMatrices& mat, const PMap& map, const dmatrix Hs,
    dvar_matrix& Ha, const int sp, const int jday) {
    const int nb_layer = param.nb_layer;
    for (int i = map.imin; i <= map.imax; i++) {
        const int jmin = map.jinf[i];
        const int jmax = map.jsup[i];
        for (int j = jmin; j <= jmax; j++) {
            ...
            if (nl > 0 && nl <= nb_layer) {
                ...
                Ha.elem_value(i, j) =
                    topo * (Hs(i, j) * sfunc + Hf * (1.0 - sfunc));
...
double VarSimtunaFunc::Tmean_comp(
                ...
                for (int l = 0; l < nbl; l++) {
                    if (Tfunc_Gaussian)
                        l_access(l) = f_accessibility_layer(
                            O2(l), T(l), twosigsq, temp_age, oxy_teta, oxy_cr);
                    else
                        l_access(l) = f_accessibility_layer(
                            O2(l), T(l), temp_age, temp_max, delta1, delta2,
