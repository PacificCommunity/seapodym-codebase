...
const double c_zoo =
    100.0;  // constant in the first tests ECCO, need to make it global or
            // parameter as soon as early life stage data will be available

void VarSimtunaFunc::Juvenile_Habitat(
    VarParamCoupled& param, CMatrices& mat, const PMap& map, dvar_matrix& Hj,
    int sp, const int t_count) {
    Hj.initialize();
    ...
    if (!param.uncouple_sst_larvae[sp]) {
        ...
        // SKJ condition temporarilly to enable the use of REF-2018 (CJFAS)
        // solution with this version
        if (param.sp_name[0].find("skj") != 0) b = param.dvarsB_sst_larvae[sp];
...
void VarSimtunaFunc::Juvenile_Habitat_cannibalism(
    VarParamCoupled& param, CMatrices& mat, const PMap& map, dvar_matrix& Hj,
    dvar_matrix& total_pop, int sp, const int t_count) {
    Hj.initialize();
    ...
    Hj_cannibalism_comp(
        param, mat, map, Hj, value(total_pop), value(a), value(b), value(c),
        t_count);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                dfa(i, j) += f_food * (pow(SST(i, j) - b, 2) / (pow(a, 3))) *
                             exp(-pow(SST(i, j) - b, 2) / (2 * pow(a, 2))) *
                             dfH(i, j);
                dfb(i, j) += f_food * ((SST(i, j) - b) / (pow(a, 2))) *
                             exp(-pow(SST(i, j) - b, 2) / (2 * pow(a, 2))) *
                             dfH(i, j);
                dfH(i, j) = 0.0;
            }
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                // Hj(i,j) = f_food * exp(-pow((SST(i,j)- b),2) / (2*a*a)) *
                // (1-total_pop(i,j)/(c+total_pop(i,j)));
                dfa(i, j) += (pow(SST(i, j) - b, 2) / (pow(a, 3))) *
                             exp(-pow(SST(i, j) - b, 2) / (2 * pow(a, 2))) *
                             f_food * f_cannibalism * dfH(i, j);
                dfb(i, j) += ((SST(i, j) - b) / (pow(a, 2))) *
                             exp(-pow(SST(i, j) - b, 2) / (2 * pow(a, 2))) *
                             f_food * f_cannibalism * dfH(i, j);
                dfN(i, j) -= (exp(-pow((SST(i, j) - b), 2) / (2 * a * a))) *
                             f_food * c / pow(c + N, 2) * dfH(i, j);
                dfc(i, j) += (exp(-pow((SST(i, j) - b), 2) / (2 * a * a))) *
                             f_food * N / pow(c + N, 2) * dfH(i, j);
                dfH(i, j) = 0.0;
            }
        }
    }
