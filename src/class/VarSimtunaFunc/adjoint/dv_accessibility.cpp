...
double f_accessibility_layer(
    const double O2, const double T, double twosigsq, double temp_mean,
    double oxy_teta, double oxy_cr);
void dff_accessibility_layer(
    double dff_access, double& dfmean, double& dfsigma, double& dfteta,
    double& dfoxy_cr, const double T, const double O, const double sigma,
    const double mean, const double teta, const double oxy_cr,
    const double sigmasq, const double sigmacb);
double thermal_func_type2(
    const double T, const double temp_min, const double temp_max,
    const double delta1, const double delta2, const double delta3);
void dfthermal_func_type2(
    const double T, const double temp_min, const double temp_max,
    const double delta1, const double delta2, const double delta3, double& dfT,
    double& dftemp_min, double& dftemp_max, double& dfdelta1, double& dfdelta2,
    double& dfdelta3);
double f_accessibility_layer(
    const double O2, const double T, double temp_age, double temp_max,
    double delta1, double delta2, double delta3, double oxy_teta,
    double oxy_cr);
void dff_accessibility_layer(
    double dff_access, double& dftemp_age, double& dftemp_max, double& dfdelta1,
    double& dfdelta2, double& dfdelta3, double& dfteta, double& dfoxy_cr,
    const double T, const double O, const double temp_age,
    const double temp_max, const double delta1, const double delta2,
    const double delta3, const double teta, const double oxy_cr);
            ...
            if (!tags_only || tags_age_solve(age)) {
                ...
                if (param.gaussian_thermal_function[sp]) {
                    gradient_structure::GRAD_STACK1->set_gradient_stack(
                        dv_accessibility_comp);
                } else
                    gradient_structure::GRAD_STACK1->set_gradient_stack(
                        dv_accessibility_ftype2_comp);
            }
        }
    }
}
...
void VarSimtunaFunc::Average_currents(
    ...
    gradient_structure::GRAD_STACK1->set_gradient_stack(
        dv_average_currents_comp);
}

/*
//previous (with p/2 and without re-scaling) version of dfthermal_func_type2
double dfthermal_func_type2(const double T, const double temp_min, const double
temp_max, const double delta1, const double delta2, const double delta3, double&
dfT, double& dftemp_min, double& dftemp_max, double& dfdelta1, double& dfdelta2,
double& dfdelta3)
{
        double temp_diff = temp_max - temp_min;
        if (temp_diff<0) {cout << "temp_max cannot be smaller than temp_min!
exit here..." << endl; exit(1);} double s_low =
0.05+delta1*temp_diff;//delta1=0.025 - test pars double s_hi  =
0.05+delta2*temp_diff; //delta2=0.015 double p     =-(0.5+delta3*temp_diff);
//delta3=0.2 double dfp   = 0.0; double dfs_low = 0.0; double dfs_hi  = 0.0;
...
void dfthermal_func_type2(
    ...
    double temp_diff = temp_max - temp_min;
    if (temp_diff < 0) {
        cout << "temp_max cannot be smaller than temp_min! exit here..."
             << endl;
        exit(1);
    }
    ...
    // a1=log(0.05+delta1*temp_diff); a2=log(0.05+delta2*temp_diff);
    // p=-(1.0+delta3*temp_diff);
...
void dff_accessibility_layer(
    double dff_access, double& dftemp_age, double& dfsigma, double& dfteta,
    double& dfoxy_cr, const double T, const double O, const double sigma,
    const double temp_age, const double teta, const double oxy_cr,
    const double sigmasq, const double sigmacb) {
...
void dff_accessibility_layer(
    ...
    double f_oxy = 1.0 / (1.0 + exprO1);
    double f_temp =
        thermal_func_type2(T, temp_age, temp_max, delta1, delta2, delta3);
    ...
    dfthermal_func_type2(
        T, temp_age, temp_max, delta1, delta2, delta3, dfT, dftemp_age,
        dftemp_max, dfdelta1, dfdelta2, dfdelta3);
...
void dv_accessibility_comp(void) {
    ...
    const dvar_matrix_position Temp_age_slope_pos =
    ...
    dmatrix dfTemp_age_slope =
        restore_dvar_matrix_derivatives(Temp_age_slope_pos);

    CParam* param = (CParam*)pos_param;

    const int nbf = param->get_nbforage();
    const int nbl = param->nb_layer;
    ivector day_layer(0, nbf - 1);
    day_layer = param->day_layer;
    ivector night_layer(0, nbf - 1);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nlayer > 0 && nlayer <= nbl) {
                ...
                for (int l = 0; l < nbl; l++) {
                    l_access(l) = f_accessibility_layer(
                        oxygen(l, i, j), tempn(l, i, j), twosigsq, temp_age,
                        oxy_teta, oxy_cr);
                    // weighted by the Forage biomass
                    for (int n = 0; n < nbf; n++) {
                        if (day_layer[n] == l)
                            lf_access(l) += l_access(l) * forage(n, i, j) * DL;
                        if (night_layer[n] == l)
                            lf_access(l) +=
                                l_access(l) * forage(n, i, j) * (1.0 - DL);
                    }
                    sumL += lf_access(l);
                }
                ...
                for (int l = nbl - 1; l >= 0; l--) {
                    ...
                    dfsumL -=
                        lf_access(l) * dfZ_access(l, i, j) / (expr1 * expr1);
                ...
                for (int l = nbl - 1; l >= 0; l--) {
                    ...
                    for (int n = nbf - 1; n >= 0; n--) {
                        if (night_layer[n] == l) {
                            // lf_access[l]+= l_access(l) *
                            // mat.forage[n][i][j]*(1-DL);
                            dfl_access(l) +=
                                forage(n, i, j) * (1.0 - DL) * dflf_access(l);
                        }
                        if (day_layer[n] == l) {
                            // lf_access[l]+= l_access(l) *
                            // mat.forage[n][i][j]*DL;
                            dfl_access(l) +=
                                forage(n, i, j) * DL * dflf_access(l);
                        }
                    }

                    // l_access(l)  =
                    // f_accessibility_layer(oxygen(l,i,j),tempn(l,i,j),twosigsq,teta,oxy_teta,oxy_cr);
                    // dff_accessibility_layer(dfl_access(l),dftemp_age,dfSigma_ha(i,j),dfOxy_teta(i,j),dfOxy_cr(i,j),
                    dff_accessibility_layer(
                        dfl_access(l), dftemp_age, dfsigma_ha, dfOxy_teta(i, j),
                        dfOxy_cr(i, j), tempn(l, i, j), oxygen(l, i, j),
                        sigma_ha, temp_age, oxy_teta, oxy_cr, sigsq, sigcb);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nlayer > 0 && nlayer <= nbl) {
                ...
                for (int l = 0; l < nbl; l++) {
                    l_access(l) = f_accessibility_layer(
                        oxygen(l, i, j), tempn(l, i, j), temp_age, temp_max,
                        delta1, delta2, delta3, oxy_teta, oxy_cr);
                    // weighted by the Forage biomass
                    for (int n = 0; n < nbf; n++) {
                        if (day_layer[n] == l)
                            lf_access(l) += l_access(l) * forage(n, i, j) * DL;
                ...
                for (int l = nbl - 1; l >= 0; l--) {
                    ...
                    for (int n = nbf - 1; n >= 0; n--) {
                        if (night_layer[n] == l) {
                            ...
                            // mat.forage[n][i][j]*(1-DL);
                            dfl_access(l) +=
                                forage(n, i, j) * (1 - DL) * dflf_access(l);
                        }
                        if (day_layer[n] == l) {
                            // lf_access[l]+= l_access(l) *
                            // mat.forage[n][i][j]*DL;
                    ...
                    // l_access(l)  =
                    // f_accessibility_layer(oxygen(l,i,j),tempn(l,i,j),temp_age,temp_max,delta1,delta2,delta3,oxy_teta,oxy_cr);
                    dff_accessibility_layer(
                        dfl_access(l), dftemp_age, dfTemp_max(i, j),
                        dfDelta1(i, j), dfDelta2(i, j), dfDelta3(i, j),
                        dfOxy_teta(i, j), dfOxy_cr(i, j), tempn(l, i, j),
                        oxygen(l, i, j), temp_age, temp_max, delta1, delta2,
                        delta3, oxy_teta, oxy_cr);
...
void dv_average_currents_comp() {
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nlayer > 0 && nlayer <= nbl) {
                for (int l = nbl - 1; l >= 0; l--) {
                    // param.dvarsU.elem_value(i,j) += mat.un(t,l,i,j) *
                    // Z_access;
                    dfZ_access(l, i, j) += un(l, i, j) * dfU(i, j);
                    // param.dvarsV.elem_value(i,j) += mat.vn(t,l,i,j) *
                    // Z_access;
