...
/// mortality rates at age functions. These functions include fixed natural
/// mortality rate and variable component, depending on habitat indices defined
/// for the life stage Forward functions are in mortality_sp.cpp
...
void VarSimtunaFunc::Mortality_Sp(
    ...
    if (age < param.sp_a0_adult[sp]) H.save_dvar_matrix_value();
...
void dv_M_sp_comp(void) {
    ...
    double mean_age_in_month =
        mean_age * param->sp_unit_cohort[sp][0] * param->deltaT / 30.0;
    ...
    if (age >= a0_adult) {
        ...
        for (int aa=age; aa>1; aa--){
                if
        (param->age_compute_habitat(sp,aa-1)!=param->age_compute_habitat(sp,aa)){
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            if (map->carte(i, j)) {
                ...
                dfH(i, j) -= M * (1.0 / Hval) * log(1.0 + range + Rage) *
                             H_var * dfM(i, j);
                dfRng(i, j) += M * exprH *
                               pow(1.0 + range + Rage, -H(i, j) / Hval) *
                               dfM(i, j);
