...
double f_accessibility_layer(
    const double O, const double T, double twosigsq, double temp_mean,
...
double f_accessibility_layer(
    ...
    double oxy_cr);
double hs_comp(
    double SST, double preys, double predators, const double a, const double b,
    const double c, const double d, const double e, const double ssv);
...
void dfspawning_habitat_seasonality(
    double& dfa, double& dfb, double& dfc, double& dfH, const double a,
    const double b, const double c, const double pf_ratio, const double SST,
    const double itopo);
void dfsst_habitat(
    double& dfa, double& dfb, double& dfH, const double a, const double b,
    const double SST, const double itopo);
void dfsigma_hss_comp(
    dvector& length, double temp_min, double temp_max, const int sp_nb_cohorts,
    const int age, const int jday);
...
void dv_Hf_comp(void) {
            ...
            if (nlayer > 0) {
                ...
                for (int l = 0; l < nbl; l++) {
                    if (Tfunc_Gaussian) {
                        l_access(l) = f_accessibility_layer(
                            oxygen(l, i, j), T(l), twosigsq, temp_age, oxy_teta,
                            oxy_cr);
                    } else {
                        l_access(l) = f_accessibility_layer(
                            oxygen(l, i, j), T(l), temp_age, temp_max, delta1,
                ...
                for (int n = 0; n < nbf; n++) {
                    ...
                    if (day_layer[n] < nlayer)
                        f_access = l_access[day_layer[n]] * DL +
                                   l_access[night_layer[n]] * (1 - DL);
    ...
    // cout << age << " " << norm(dfF_access(0)) << " " << norm(dfF_access(1))
    // << " " << norm(dfF_access(2)) << " " << norm(dfF_access(3)) << " " <<
    // norm(dfF_access(4)) << " " << norm(dfF_access(5)) << " " << norm(dfEF(0))
    // << " " << norm(dfEF(0)) <<" " << norm(dfEF(1)) <<" " << norm(dfEF(2)) <<"
    // " << norm(dfEF(3)) <<" " << norm(dfEF(4)) <<" " << norm(dfEF(5)) <<" " <<
    // norm(dfHa) <<endl;
...
// Inna 01/13: derivative code for the new function - sigma varying as a
// function of season peak Inna 05/13: the use of function is temporarily on
// hold, still using fixed values of sigma (see seasonal_switch)
void dfsigma_hss_comp(
    double& dfTemp_max, double& dfTemp_min, double& dfsigma, double temp_min,
    double temp_max, const double R, const int jday) {
    ...
    //	double DL_start_NH  = daylength_comp(lat_max_sp, jday_maxDL-60,
    //pi)/24.0;
...
void dv_Ha_comp(void) {
    ...
    double sigma_season = mat->sigma_season(
        sp, jday, age);  // sigma_hss_comp(temp_age,temp_max, age,jday);
    ...
    for (int i = imax; i >= imin; i--) {
        ...
        for (int j = jmax; j >= jmin; j--) {
            ...
            if (nlayer > 0 && nlayer <= nbl) {
                ...
                for (int n = 0; n < nbf; n++) F(n) = forage(n, i, j);
                double predators =
                    pred_surface_comp(F, DL, nbf, day_layer, night_layer);

                double O2_l2 = oxygen(1, i, j);
                double f_oxy = 1.0 / (1.0 + pow(0.01, O2_l2 - 0.1));

                double Hs = hs_comp(
                                sst(i, j), preys, predators, a_hs, b_hs, c_hs,
                                d_hs, e_hs, sigma_season) *
                            f_oxy;
                ...
                for (int l = 0; l < nbl; l++) {
                    if (Tfunc_Gaussian) {
                        // l_access(l)  =
                        // f_accessibility_layer(oxygen(l,i,j),tempn(l,i,j),twosigsq,
                        l_access(l) = f_accessibility_layer(
                            oxygen(l, i, j), T(l), twosigsq, temp_age, oxy_teta,
                            oxy_cr);
                    } else {
                        // l_access(l)  =
                        // f_accessibility_layer(oxygen(l,i,j),tempn(l,i,j),temp_age,
                        l_access(l) = f_accessibility_layer(
                            oxygen(l, i, j), T(l), temp_age, temp_max, delta1,
                            delta2, delta3, oxy_teta, oxy_cr);
                    }
