...
/// rate and variable component, depending on habitat indices defined for the
/// life stage

void VarSimtunaFunc::M_sp_comp(
    const PMap& map, dvar_matrix& M, const dmatrix& H, double Mp_max,
    double Ms_max, double Mp_exp, double Ms_slope, double range,
    double mean_age_in_dtau, const int dtau) {
    ...
    // will add nonlinear increment Rage, since for many cases (essic bet, skj,
    // ncep/era40 alb) range=0 is estimated, which results in zero variation of
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                M.elem_value(i, j) *= pow(
                    1.0 + Rage,
                    1 - H(i, j) /
                            Hval);  // modified old function (Rage was added)
            }
        }
    }
}

void VarSimtunaFunc::M_PH_juv_comp(
