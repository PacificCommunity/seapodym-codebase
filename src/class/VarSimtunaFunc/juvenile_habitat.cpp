...
void VarSimtunaFunc::Hj_cannibalism_comp(
    VarParamCoupled& param, CMatrices& mat, const PMap& map, dvar_matrix& Hj,
    const dmatrix& total_pop, double a, double b, double half_predators,
    const int t) {
    for (int i = map.imin; i <= map.imax; i++) {
        const int jmin = map.jinf[i];
        const int jmax = map.jsup[i];
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                ...
                double f_cannibalism =
                    1 - predators / (half_predators + predators);
