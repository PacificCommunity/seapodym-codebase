...
void VarSimtunaFunc::Seasonal_switch_year_precomp(
    ...
    for (int i = 1; i <= ndays; i++) {
        ...
        for (int j = map.jmin; j <= map.jmax; j++) {
            ...
            mat.season_switch(sp, i, j) =
                tetafunc(dn_ratio - season_start, tau);
        }
        for (int a = param.sp_a0_adult[sp]; a < param.sp_nb_cohorts[sp]; a++) {
            double lat_north = 58.5;
            double lat_south = -58.5;  // TEMPORAL: need to find a solution for
                                       // only one hemisphere
            double peak_est = 117.0;  // 125;//put here the estimated peak to
                                      // syncronize sigma with seasonal dynamics
            double DL_north =
                daylength_comp(lat_north, i - 1 + jday_maxDL - peak_est, pi) /
                24.0;
            double DL_south =
                daylength_comp(lat_south, i - 1 + jday_maxDL - peak_est, pi) /
                24.0;
            double dn_ratio_north = DL_north / (1.0 - DL_north);
            double dn_ratio_south = DL_south / (1.0 - DL_south);
            double dn_ratio_max = 3.6;
            if (dn_ratio_north > dn_ratio_max ||
                dn_ratio_south >
                    dn_ratio_max) {  // && param.seasonal_migrations[sp]){
                cout << "WARNING: dn_ratio = " << dn_ratio_north << " "
                     << dn_ratio_south << " > dn_ratio_max" << endl;
                cout << "pour latitude" << lat << endl;
                cout << "Seasonal switch will not work correctly in high "
                        "latitudes..."
                     << endl;
                cout << "exiting..." << endl;
                exit(1);
            ...
            double T_age = Topt_at_age_comp(
                param, 17, 26, sp,
                a);  // put hear estimated parameters for a species
            // TEST ALB PAPER: constant sigma: comment this line, set
            // sigma_season to a constant value
            mat.sigma_season(sp, i, a) =
                (26 - T_age);  // put here spawning temperature estimate
            double sigma_season_start = mat.sigma_season(sp, i, a);

            // TEST ALB PAPER: constant sigma : comment this block of
            // sigma_season computation
            double season_start_fix = 1.0525;
            if (dn_ratio_north >= season_start_fix)
                mat.sigma_season(sp, i, a) =
                    1.0 * (dn_ratio_north - (season_start_fix)) /
                        (dn_ratio_max - (season_start_fix)) +
                    sigma_season_start *
                        (1 - (dn_ratio_north - (season_start_fix)) /
                                 (dn_ratio_max - (season_start_fix)));
            if (dn_ratio_south >= season_start_fix)
                mat.sigma_season(sp, i, a) =
                    1.0 * (dn_ratio_south - (season_start_fix)) /
                        (dn_ratio_max - (season_start_fix)) +
                    sigma_season_start *
                        (1 - (dn_ratio_south - (season_start_fix)) /
                                 (dn_ratio_max - (season_start_fix)));
        }
    }
}

// temporal function used in the TEST: using fixed values in order to avoid
// dependence on control parameters
double VarSimtunaFunc::Topt_at_age_comp(
    CParam& param, const double teta_min, const double teta_max, const int sp,
    const int age) {
...
void VarSimtunaFunc::Seasonal_switch_comp(
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                mat.dvarSeasonSwitch(sp).elem_value(i, j) =
                    mat.season_switch(sp, jday, j);
            }
        }
    }
}

double daylength_twilight(
    double lat, double jday,
    double pi) {  // The CBM model of Forsythe et al, Ecological Modelling 80
                  // (1995) 87-95
...
double daylength_comp(
    double lat, double jday,
    double pi) {  // see original function in SimtunaFunc class
