...
void VarSimtunaFunc::Hs_comp(
    VarParamCoupled& param, CMatrices& mat, const PMap& map, dvar_matrix& Hs,
    double a, double b, double c, double d, double e, const double ssv,
    const int jday, int t_count) {
    ...
    for (int i = map.imin; i <= map.imax; i++) {
        ...
        for (int j = jmin; j <= jmax; j++) {
            if (map.carte(i, j)) {
                Hs.elem_value(i, j) = Hs_comp_elem(
                    mat, F, pp_transform, a, b, c, d, e, ssv, nb_forage, dlayer,
                    nlayer, jday, t_count, i, j);
            }
        }
    }
}

double VarSimtunaFunc::Hs_comp_elem(
    ...
    const double ssv, const int nb_forage, ivector day_layer,
    ...
    for (int n = 0; n < nb_forage; n++) F(n) = mat.forage(t, n, i, j);

    // larvae predators - micronekton at the surface
    double predators =
        pred_surface_comp(F, DL, nb_forage, day_layer, night_layer);

    // larvae preys - should be both phyto and zoo as a food of larvae. At the
    // moment phyto only
...
double lognormal(
    ...
    const double sigma) {  // 20220330: function not used currently. Only the
                           // scaled version is in use, see hs_comp.
    double f_x = exp(-pow(log(x) - mu, 2.0) / (2.0 * sigma * sigma)) /
                 (x * sigma * sqrt(2.0 * pi));
    return (f_x);
}
