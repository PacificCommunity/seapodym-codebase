	...
	switch (like_type){
		...
		case 1:{// Poisson cost function
			if (N_pred == 0.0){
				...
				lkhd = poisson_comp(L_obs, N_pred, weight_Lobszero, *param, 0, what);
			}
			break;
		}
		...
		case 2: // Truncated Poisson cost function
			if (N_pred == 0.0){
				...
				lkhd = truncated_poisson_comp(L_obs, N_pred, weight_Lobszero, *param, 0, what);
			}
			break;

		case 3: // Zero-Inflated Negative Binomial cost function
			lkhd = zinb_comp(L_obs, N_pred, *param, 0, what);
			break;
