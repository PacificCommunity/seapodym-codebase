...
//rewrote function to enable it with non-dt age structure
void CSimtunaFunc::mortality_range_age_comp(CParam& param, CMatrices& mat, const int sp)
{
        ...
        double mean_age0 = 0.5; //always half of dt for the dt-unit age structure
	double mean_age = mean_age0 * param.sp_unit_cohort[sp][0];
        for (int age=0; age<param.sp_nb_cohorts[sp]; age++){

		if (age==param.sp_nb_cohorts[sp]-1)
			mean_age += .5*param.sp_unit_cohort[sp][age]-param.sp_unit_cohort[sp][age-1];

		mat.mortality_range_age[sp][age] = p*(1.0 + pow(mean_age0,q))/(1.0 + pow(mean_age,q));
		
		mean_age +=  param.sp_unit_cohort[sp][age];
        }
}
...
/*void CSimtunaFunc::mortality_range_age_comp(CParam& param, CMatrices& mat, const int sp)
{
	...
	cout << endl << mat.mortality_range_age << endl;
	exit(1);
}*/
...
double CSimtunaFunc::function_lambda(CParam& param, CMatrices& mat, int n, int i, int j)
{
	...
	// migrant : based on the average temperature relative to time spent in day and night layers
	double function;
	...
	{ // average temperature according to time passed in both layers
		double DL = mat.daylength[i][j];
		temp = ((DL * mat.tempn[1][param.day_layer[n]][i][j]) + ((24-DL)*mat.tempn[1][param.night_layer[n]][i][j]) ) / 24 ;
	}
...
double CSimtunaFunc::daylength_twilight(double lat, int jday, const double p)
...
{
	// return the true monthly gradient of length of day (DL) for a given latitude (lat) 
	double DL = 0.0;
...
double CSimtunaFunc::f_accessibility_comp(const double Od, const double On, const double Td, const double Tn, double twosigsq, double temp_mean, double oxy_teta, double oxy_cr, const double DL)
{
