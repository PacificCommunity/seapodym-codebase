...
int seapodym_densities(const char* parfile, int cmp_regime, const bool reset_buffers)
{
	time_t time_sec;
	time(&time_sec);
...
void verify_identifier_string2(char* str1) //ASSUME str1 is not null
{
  ...
  str[num_bytes]='\0';
  extern gradient_structure gs;
  gs.get_fp()->fread(str,num_bytes);
  if(strcmp(str1,str))
  {
...
unsigned long int restore_long_int_value(void)
{
  ...
  int num_bytes = sizeof(unsigned long int);
  extern gradient_structure gs;
  gs.get_fp()->fread(&tmpout,num_bytes);
