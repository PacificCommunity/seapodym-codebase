...
void Date::init_time_variables(CParam& param, int& Tr_step, int& nbt_spinup_tuna, int& jday_run, int& jday_spinup, int& nbstot, const int info, const int flagsimu)
{
	//-------------------------------------------------------------------
...
int Date::get_nbstot(const int ndat000, const int ndatfin, const int jday_start, const int deltaT, const int date_mode, ivector& rundates)
{
	int day = 1;
	int month = 1;
	int year = 1900;
...
//do not include spinup!
	...
	while (ndatcur < ndatfin+deltaT) {
		...
		update_time_variables(t_count,deltaT,date_mode,jday_start,jday,day,month,year,newyear);
...
int Date::get_nbt_before_first_recruitment(const int first_recruitment_date, const int ndatini, const int deltaT, const int date_mode)
{
	...
	while (ndatcur <= first_recruitment_date) {
		t_count++;	
		update_time_variables(t_count+1,deltaT,date_mode,jday_run,jday,day,month,year,lastday);
		ndatcur = year*10000+month*100+day;
	}
	return t_count-1;	
}

void Date::update_time_variables(const int t_count, const int deltaT, const int date_mode, const int jday_spinup, int& jday, int& day, int& month, int& year, int& newyear)
{
        //-------------------------------------------------------------------
...
int Date::dym_startdate_run(CParam& param, const dvector zlevel_dym, const int nbstot)
{
...
//cout << param.date_mode << endl;
	...
	switch (param.date_mode){
			...
			//temporal: until the fix of date in GLORYS 7days DYM files (action: Beatriz)
			//dateini = year+(jdayini-dt)/365.0;
...
// Convert Gregorian calendar date to the corresponding Julian day for a 360 days calendar
...
// Convert Gregorian calendar date to the corresponding Julian day for a noleap year calendar
...
// Convert a Julian day number to its corresponding 360 days calendar  // Modifications Olga
