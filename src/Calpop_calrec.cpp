...
void CCalpop::calrec(const PMap& map, DMATRIX& uu, const DMATRIX& mortality)
		...
		for (int j = map.jmin; j <= map.jmax; j++){
			...
			if ((map.carte[1][j]) && (map.carte[nti-2][j])){
				for (int k=0; k<ncells; k++){
					...
					xbet1[j][k] = bm[j][nti-(ncells+1)+k] + 2*iterationNumber;
				...
				for (int k=nti-2+ncells; k<nti-2+2*ncells; k++){
					...
					xbet1[j][k] = bm[j][k-(nti-3)-ncells] + 2*iterationNumber;
					c1[j][k] = c[j][k-(nti-3)-ncells];						
				}

				for (int k=0; k<ncells; k++) rhs1[k] = rhs[nti-(ncells+1)+k];
				for (int i=ncells ; i<nti-2+ncells ; i++) rhs1[i] = rhs[i-ncells+1];
				for (int k=nti-2+ncells; k<nti-2+2*ncells; k++) rhs1[k] = rhs[k-(nti-3)-ncells];

				tridag_GO(a1[j],xbet1[j],c1[j],rhs1,uvec1,gam1,0,(nti-2)+2*ncells-1);
