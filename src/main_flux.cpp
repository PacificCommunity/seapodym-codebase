...
int seapodym_flux(const char* parfile, const int cmp_regime, const int sub_option, const bool reset_buffers);
...
int OptionToCode(char* op, int &sub_option) {
	...
	const int N = 12;
	const char *cmdop[N] = {"-s","-p","-sa","-h","-v","--simulation","--likelihood-projection","--sensitivity-analysis","--help","--version","-sa=2","-sa=3"};
...
void help(char* argv0) {
	...
	cout << "  -sa, --sensitivity-analysis \t Perform global sensitivity analysis. By default performs one AAT simulations.\n";	
