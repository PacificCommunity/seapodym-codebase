...
private:
	...
	double get_early_like(dvariable& likelihood, dvar_matrix& Agg_larvae_density_at_obs, string what);
	double get_early_like(dvariable& likelihood, dvar_matrix& Larvae_density_pred, D3_ARRAY larvae_input, int t, string what);
...
dvariable gaussian_comp(double L_obs, dvariable N_pred, double weight_Lobszero, VarParamCoupled& param, int sp, string what);
