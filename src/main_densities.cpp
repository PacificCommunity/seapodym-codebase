...
int main(int argc, char** argv) {
    ...
    // Note, other options are yet to be integrated from the seapodym_coupled
    // code Currently only simulation, optimization and Hessian calculation are
    // possible
...
int OptionToCode(char* op, int& sub_option) {
    ...
    const char* cmdop[N] = {
        ...
        "-sa=2",
        "-sa=3"};
    int cmpCode[N] = {0, 1, 2,  3,  4, -3, -2, 0, 1, 2,
                      3, 4, -3, -2, 4, 3,  4,  3, 3, 3};
    for (int i = 0; i < N; i++)
...
void help(char* argv0) {
    ...
    cout << "  -sa[=FLAG]  \t\t\t Perform sensitivity analysis. By default[=0] "
            "local sensitivity with model predictions only.\n";
    cout << "   --sensitivity-analysis[=FLAG] If FLAG=1 the sensitivity "
            "function takes both predictions and observations.\n";
    cout << "   --sensitivity-analysis[=FLAG] If FLAG=2 ONE-AT-A-TIME "
            "sensitivity analysis.\n";
    cout << "   --sensitivity-analysis[=FLAG] If FLAG=3 ALL-AT-A-TIME "
            "sensitivity analysis.\n";
    cout << "  -t[=FLAG]   \t\t\t Perform identical (by default, or FLAG=0) "
            "twin experiment.\n";
    cout << "   --twin-experiment=[FLAG] \t If FLAG=1 the noise will be added "
