...
void CCalpop::total_obs_catch_age_comp(const PMap& map, const CParam& param, CMatrices& mat, const dmatrix& uu, const dmatrix& Cobs, dvar_matrix& Ctot_age_obs, const int f, const int fne, const int k, const int age, const int sp, const double C2Dunits)
{
	...
	for (int j = b.rowmin(); j <= b.rowmax(); j++) {
		for (int i = b[j].indexmin(); i <= b[j].indexmax(); i++) {
			...
			if (map.carte(i,j)  &&  EB_tot) {
				...
				Ctot_age_obs.elem_value(i,j) += LF_est * Cobs(i,j) * C2Dunits * mat.lat_correction(j);
				...
				for (int a=0; a<nb_region_sp; a++){	
					...
					if (nb_region){ 			
						if ((i>=map.regimin[a]) && (i<map.regimax[a]) && 
							...
							mat.C_N_sp_age_fishery[sp][age][k][r] += exploited_age_class*Cobs(i,j);
					} else 
						//mat.C_N_sp_age_fishery[sp][age][k][0] += LF_est;
						mat.C_N_sp_age_fishery[sp][age][k][0] += exploited_age_class*Cobs(i,j);
				}
			}
		}
	}
...
void CCalpop::Recomp_total_obs_catch_age(const PMap& map, CParam& param, CMatrices& mat, CReadWrite& rw, dmatrix& Ctot_age_obs, const int age, const int sp, const int year, const int month, const int t_count)
{
	...
	for (int f=0; f<nb_fishery; f++){ 
		if (param.mask_fishery_sp[sp][f]){
			if (param.mask_fishery_sp_no_effort[sp][f]){
				...
				Recomp_total_exploited_biomass(map,param,mat,EB,catch_obs,Selectivity(sp,f),f,sp,t_count);
				...
				for (int i = map.imin; i <= map.imax; i++){	
					...
					for (int j = jmin; j <= jmax; j++){
						if (map.carte(i,j) && EB(i,j)){
							double LF_est = Selectivity(sp,f,age)*uu(i,j)/EB(i,j);

							Ctot_age_obs(i,j) += LF_est * catch_obs(i,j) * C2Dunits * mat.lat_correction(j);
						}
					}
				}
			}
