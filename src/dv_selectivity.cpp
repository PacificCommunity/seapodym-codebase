...
void CParam::dfselectivity(double& dfslope, double& dflength, double& dfasympt, const int sp, const int age, const int f, const int k)
{
	...
	switch (s_func_type[f]) {
		...
		case 2://sigmoid
			...
			//selectivity = 1/(1+exp(s_slope_sp_fishery(sp,k)*(s_length_sp_fishery(sp,k)-length(sp,age))));
			dff = exp(s_slope*(s_length-l))/pow(1+exp(s_slope*(s_length-l)),2);
		...
		case 3://asymmetric Gaussian
			...
			if (length(sp,age)<=s_length_sp_fishery(sp,k)){
				//selectivity = exp(-pow(length(sp,age)-s_length_sp_fishery(sp,k),2)/sigma_sq);
				dfslope   = (2*pow(l-s_length,2) / pow(s_slope,3)) * g;
				dflength = 2*((l-s_length)/sigma_sq) * g;
			} else {
				s_asympt = s_asympt_sp_fishery(sp,k);
				//selectivity = (1-s_asympt_sp_fishery(sp,k))*exp(-pow(length(sp,age)-s_length_sp_fishery(sp,k),2)/sigma_sq)+s_asympt_sp_fishery(sp,k);
				dfslope  = (1-s_asympt)*(2*pow(l-s_length,2) / pow(s_slope,3)) * g;
		...
		case 4://two-modal Gaussian: not done yet!
			...
			double dfgm1 = -1.0/pow(1+gm1+gm2,2); double dfgm2 = -1.0/pow(1+gm1+gm2,2);
