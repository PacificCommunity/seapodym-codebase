...
SRCPATH=src/utilities:src/data:src/models/control:src/models/functions:src/models/parameters:src/models/solver
INCPATH=include
BINPATH=bin
...
DEBUG= -g
#production code
CFLAGS=-DTRUE=true -DFALSE=false -D __GNUDOS__ -Dlinux -O3 -DOPT_LIB -Wall -Wno-deprecated -I$(ADMODEL)/include -I/usr/include/libxml2 -I$(INCPATH)

###### with safe library ################################
#LFLAGS=-lm -L$(ADMODEL_HOME)/lib  -ladmb -lstdc++ -lxml2 

###### with optimized library ###########################
LFLAGS= -L$(ADMODEL)/lib  -ladmbo -lstdc++ -lxml2 -lm
